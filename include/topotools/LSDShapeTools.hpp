...
//
// Only works for X,Y point shapefiles at present and it's behavious is totally
...
//
// multistem_option -> 0 = mainstem only, 1 = all tributaries, 2 specify
// tributary number (DAV 09/04/2015) DTM 11/07/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
public:
  ...
  /// 3/22/95: by ChuckGantz chuck.gantz@globalstar.com, from USGS Bulletin
  /// 1532.
  ...
  /// @date 07/12/2014
  void LLtoUTM(int eId, double Lat, double Long, double &Northing,
               double &Easting, int &Zone);

  /// @brief converts LatLong to UTM coords, but forces the data to a specific
  /// zone 3/22/95: by ChuckGantz chuck.gantz@globalstar.com, from USGS Bulletin
  /// 1532. Updated by Simon M Mudd
  /// @param eID the ellipsoid ID. Options are:
  ///  0 = "Airy1830";
  ///  1 = "AiryModified";
  ///  2 = "AustralianNational";
  ...
  /// @author SMM, modified from Chuck Gantz
  /// @date 11/04/2016
  void LLtoUTM_ForceZone(int eId, double Lat, double Long, double &Northing,
                         double &Easting, int Zone);
  ...
  /// @param isNorth is a boolean that states if the map is in the northern
  /// hemisphere
  /// @param Lat the latitude in decimal degrees.
  ...
  void DatumConvert(int dIn, double LatIn, double LongIn, double HtIn, int dTo,
