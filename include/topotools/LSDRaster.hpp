...
public:
  ...
  LSDRaster(int nrows, int ncols, float xmin, float ymin, float cellsize,
            float ndv, Array2D<float> data) {
  ...
  /// @author DAV
  LSDRaster(int nrows, int ncols, double xmin, double ymin, double cellsize,
            double ndv, Array2D<double> data) {
    create(nrows, ncols, xmin, ymin, cellsize, ndv, data);
  }
  ...
  /// @param ndv An integer of the no data value.
  /// @param data An Array2D of floats in the shape nrows*ncols,
  /// @param temp_GRS a map of strings containing georeferencing information.
  /// Used mainly with ENVI format files
  /// containing the data to be written.
  LSDRaster(int nrows, int ncols, float xmin, float ymin, float cellsize,
            float ndv, Array2D<float> data, map<string, string> temp_GRS) {
  ...
  /// @bug You can't return a TNT::Array properly in a function. See google for
  /// details.
  /// @author DAV
  /// @return Returns a 2D Array of the raster file.
  TNT::Array2D<double> get_ascii_raster(string FILENAME);

  /// @brief reads a raster from an ascii file, populates an LSDRaster object
  /// array
  /// @author DAV
  /// @details This method takes a filename (the .asc file).
  /// It reads in the ascii file and fills in the array (and header) part of the
  /// LSDRaster object with double precision values in the array. You must have
  /// declared an instance of an LSDRaster object.
  /// @example myLSDRasterobject.read_ascii_raster("elevations.asc")
  /// @result The myLSDRasterobject.RasterData_dbl data member is now populated
  /// with an array of the DEM data. The data members for NCols, NRows, etc. are
  /// also updated.
  void read_ascii_raster(string FILENAME);

  /// @brief Reads a raster of integers and populates LSDRaster integer array
  /// member data
  /// @author DAV
  /// @todo Really, one ought to modify LSDRaster so that it is a class
  /// template, and then wouldn't need different TNT Array data members for when
  /// we have floats, double, ints etc. Might be tricky though, although if done
  /// carefully it should not break peoples code.
  ...
  /// @brief This calls raster write functions, writing from Arrays of type
  /// <double> to raster format.
  /// @details Sorry for duplicating a load of code, but I couldn't think
  /// of a good way to overload the function without passing the raster data
  /// array or breaking someone elses code.
  /// @param filename a string of the filename _without_ the extension.
  /// @param extension a string of the extension _without_ the leading dot
  /// @author DAV
  /// @date 07-12-2015
  ...
  /// @author SMM
  /// @date 02/03/2015
  bool does_raster_have_same_dimensions_and_georeferencing(
  ...
  /// @brief Method which takes a new xmin and ymax value and modifys the
  /// GeoReferencingStrings map_info line to contain these new values.
  ///
  /// @details Intended for use in the rastertrimmer methods and is called from
  /// within these methods. Modifying georeferencing information by hand is
  /// messy and should be avoided if at all possible.
  /// @param NewXmin floating point value of the new minimum x value in the
  /// raster.
  /// @param NewYmax floating point value of the new maximum y value in the
  /// raster.
  /// @return An updated GeoReferencingStrings object.
  ///
  /// @author SWDG
  /// @date 6/11/14
  map<string, string> Update_GeoReferencingStrings(float NewXmin,
                                                   float NewYmax);

  /// @brief Method which updates the map info element of the georeferencing
  /// strings based on information within the datamembers of the raster
  ///
  ...
  /// @return central_meridian an integer of the central meridian of this UTM
  /// zone
  /// @author SMM
  /// @date 6/11/14
  ...
  /// @param is_North a boolean that is true if the DEM is in the northern
  /// hemisphere.
  ...
  /// @param lat the latitude of the node (in decimal degrees, replaced by
  /// function)
  ///  Note: this is a double, because a float does not have sufficient
  ///  precision relative to a UTM location (which is in metres)
  /// @param long the longitude of the node (in decimal degrees, replaced by
  /// function)
  ///  Note: this is a double, because a float does not have sufficient
  ///  precision relative to a UTM location (which is in metres)
  ...
  /// @brief This returns vectors of all the easting and northing points in the
  /// raster
  ///  Used for interpolations
  /// @param Eastings a vector of easting coordinates. Will be replaced by
  /// method.
  /// @param Northings a vector of northing coordinates. Will be replaced by
  /// method.
  /// @author SMM
  /// @date 17/03/2017
  void get_easting_and_northing_vectors(vector<float> &Eastings,
                                        vector<float> &Northings);

  /// @brief This interpolates a vector of points onto the raster. Uses bilinear
  /// interpolation.
  ...
  /// @author SMM
  /// @date 17/03/2017
  vector<float> interpolate_points_bilinear(vector<float> UTMEvec,
  ...
  /// @author SMM
  /// @date 17/02/2017
  LSDRaster fill_with_interpolated_data(vector<int> rows_of_nodes,
  ...
  /// @author SMM
  /// @date 22/01/2016
  void get_row_and_col_of_a_point(float X_coordinate, float Y_coordinate,
                                  int &row, int &col);
  ...
  /// @brief Calculate the minimum bounding rectangle for an LSDRaster Object
  /// and crop out all the surrounding NoDataValues to reduce the size and load
  /// times of output rasters.
  ///
  /// @details Ideal for use with chi analysis tools which output basin and chi
  /// m value rasters which can be predominantly no data. As an example, a 253
  /// Mb file can be reduced to ~5 Mb with no loss or resampling of data.\n
  ///
  /// Modded 6/11/14 to cope with bil files and to catch cases where some or all
  /// of the edges cannot be trimmed - SWDG
  ///
  ...
  /// and crop out all the surrounding NoDataValues to reduce the size and load
  /// times of output rasters.
  ///  Similar to RasterTrimmer but has a pixel buffer. Useful for CRN data
  ///  since
  ...
  /// @brief Make LSDRaster object using a 'template' raster and an Array2D of
  /// data.
  ...
  /// @brief Strips the edge rows/columns of a LSDRaster on each side of the
  /// array.
  /// @details This removes 1 pixel/grid cell from each side of an LSDRaster
  ///  Note that it modifies the original raster and reassigns the
  ///  RasterData_dbl data member to the new 'trimmed' raster. It then reduces
  ///  the NCols and NRows values by 2, but does not modify the ll-corner
  ///  values. It was primarily written for a special case in LSDCatchmentModel
  ///  but will work with any LSDRaster.
  /// @author DAV
  /// @date 01/04/2016
  void strip_raster_padding();

  /// @brief Buffers a raster using a circular kernel of a user-defined radius
  /// (m)
  ...
  /// @param if this ==1 then you use a circular kernal. Otherwise kernal is
  /// square
  ...
  /// @brief Calculates mean relief of a raster, it defaults to a circular
  /// kernal
  /// @return The spatially distributed relief
  /// @author JAJ (entered into trunk SMM)
  /// @date 01/02/2014   modified 09/06/2014 by SMM
  ...
  /// square algorithm.
  /// @param The first parameter is the feature size. This needs to be a power
  /// of 2, but this is set by the parent DiamondSquare function (that is, this
  /// function should not be called independantly.
  /// @param The scale is effectivly the maximum relief of the surface to be
  /// produced by the algorithm.
  /// @author SMM
  /// @date 16/02/2014
  void DSSetFeatureCorners(int featuresize, float scale);

  /// @brief This is the square sampling step of the diamond square algorithm:
  /// it takes the average of the four corners and adds a random number to set
  /// the centrepoint of a square.
  ...
  /// @brief This is the diamond sampling step of the diamond square algorithm:
  /// it takes the average of the four corners and adds a random number to set
  /// the centrepoint of a diamond.
  /// @param The row of the centrepoint.
  /// @param The column of the centrepoint.
  ...
  /// @param The scale which sets the maxmum relief within a particular square
  /// or diamond and is scaled by the stepsize (that is smaller squares have
  /// smaller scales).
  ///
  /// @author SMM
  /// @date 16/02/2014
  ...
  /// of 2. The xllocation and yllocation data values are preserved. The
  /// function returns a pseudo fractal landscape generated with the diamond
  /// square algorithm
  ///
  /// @param feature order is an interger n where the feature size consists of
  /// 2^n nodes. If the feature order is set bigger than the dimensions of the
  /// parent raster then this will default to the order of the parent raster.
  /// @param Scale is a floating point number that sets the maximum relief of
  /// the resultant raster.
  /// @return Returns a diamond square pseudo-fractal surface in and LSDRaster
  /// object.
  /// @author SMM
  /// @date 16/02/2014
  ...
  /// interfaced through LSDRaster::TopoShield and should not be called
  ...
  /// given radiation source defined by an Azimuth and Zenith following Codilean
  /// (2006).
  ///
  /// @details Performs a coordinate transformation, rotating the x,y,z
  /// coordinates about the Azimuth and Zenith such that the coordinates are
  /// aligned with the Azimuth and Zenith. Shaded cells are then found by
  /// tracking in the direction of the radiation source and looking for
  /// transformed z values greater than that at the cell of interest which would
  ...
  /// @brief This function generates a topographic shielding raster using the
  /// algorithm outlined in Codilean (2006).
  ///
  /// @details Creating a raster of values between 0 and 1 of shadowed cells
  ...
  ///        7 -> Stationary point classification (1=peak, 2=depression,
  ///        3=saddle)
  /// The program returns a vector of LSDRasters.  For options marked "false" in
  /// boolean input raster, the returned LSDRaster houses a blank raster, as
  /// this metric has not been calculated.  The desired LSDRaster can be
  /// retrieved from the output vector by using the cell reference shown in the
  /// list above i.e. it is the same as the reference in the input boolean
  /// vector.
  ...
  /// @date 28/03/2014
  vector<LSDRaster>
  calculate_polyfit_surface_metrics(float window_radius,
  ...
  /// metric has not been calculated.  The desired LSDRaster can be retrieved
  /// from the output vector by using the same cell reference shown in the list
  /// above i.e. it is the same as the reference in the input binary vector.
  ...
  /// @author DTM
  /// @date 01/04/2014
  vector<LSDRaster> calculate_polyfit_roughness_metrics(
  ...
  /// @brief This function calculates 6 coefficient matrices that allow the user
  /// to then calcualte slope, curvature, aspect, a classification for finding
  /// saddles and peaks and other metrics.
  ///
  /// @details The coefficient matrices are overwritten during the running of
  ...
  /// surface fitting.
  /// A is constructed using different combinations of x and y, thus we only
  /// need to compute this once, since the window size does not change. For 2nd
  /// order surface fitting, there are 6 coefficients, therefore A is a 6x6
  /// matrix. Updated 15/07/2013 to use a circular mask for surface fitting -
  /// DTM. Updated 24/07/2013 to check window_radius size and correct values
  /// below data resolution - SWDG.
  ...
  /// @date 01/01/12
  void
  calculate_polyfit_coefficient_matrices(float window_radius, Array2D<float> &a,
                                         Array2D<float> &b, Array2D<float> &c,
                                         Array2D<float> &d, Array2D<float> &e,
                                         Array2D<float> &f);

  // a series of functions for retrieving derived data from the polyfit
  // calculations

  /// @brief  This function calculates the elevation based on a polynomial fit.
  ///
  /// @details the window is determined by the
  /// calculate_polyfit_coefficient_matrices this function also calculates the
  /// a,b,c,d,e and f coefficient matrices.
  /// @param f coefficeint f.
  ...
  /// @brief  This function calculates the planform curvature based on a
  /// polynomial fit.
  ///
  /// @details the window is determined by the
  /// calculate_polyfit_coefficient_matrices this function also calculates the
  ...
  /// @author DTM, SMM
  /// @date 01/01/12
  LSDRaster calculate_polyfit_planform_curvature(Array2D<float> &a,
                                                 Array2D<float> &b,
                                                 Array2D<float> &c,
                                                 Array2D<float> &d,
                                                 Array2D<float> &e);

  /// @brief  This function calculates the profile curvature based on a
  /// polynomial fit.
  ///
  /// @details the window is determined by the
  /// calculate_polyfit_coefficient_matrices this function also calculates the
  ...
  /// @author DTM, SMM
  /// @date 01/01/12
  LSDRaster calculate_polyfit_profile_curvature(Array2D<float> &a,
                                                Array2D<float> &b,
                                                Array2D<float> &c,
                                                Array2D<float> &d,
                                                Array2D<float> &e);

  /// @brief  This function calculates the tangential curvature based on a
  /// polynomial fit.
  ///
  /// @details the window is determined by the
  /// calculate_polyfit_coefficient_matrices this function also calculates the
  ...
  /// @author DTM, SMM
  /// @date 01/01/12
  LSDRaster calculate_polyfit_tangential_curvature(Array2D<float> &a,
                                                   Array2D<float> &b,
                                                   Array2D<float> &c,
                                                   Array2D<float> &d,
                                                   Array2D<float> &e);

  /// @brief This function identifies approximate position of stationary points
  /// within discrete surface using a threshold slope.
  ///
  /// @details The nature of the stationary point is then determined to
  /// discriminate peaks, depressions and saddles. \n 0 = Non-stationary  \n 1 =
  /// Peak             \n 2 = Depression        \n 3 = Saddle             \n
  /// @param a coefficeint a.
  /// @param b coefficeint b.
  /// @param c coefficeint c.
  ...
  /// @author DTM
  /// @date  17/09/2012
  LSDIndexRaster calculate_polyfit_classification(Array2D<float> &a,
                                                  Array2D<float> &b,
                                                  Array2D<float> &c,
                                                  Array2D<float> &d,
                                                  Array2D<float> &e);
  ...
  /// @details Uses the hilltop curvature raster and the roughness raster. If
  /// the roughness value is greater than the threshold (set to 0.015) then the
  /// pixel is designated bedrock.
  ...
  /// @author FJC
  /// @date 01/04/15
  float get_percentage_bedrock_ridgetops(LSDRaster &roughness,
                                         LSDRaster &hilltop_curvature,
                                         float threshold);

  /// @brief Calculates slope angle in radians. Needs the slope raster
  /// @return LSDRaster containing the slope angle in radians. For use in
  /// trigonometric calculations
  /// @author SMM
  /// @date 13/11/14
  LSDRaster calculate_slope_angles();

  /// @brief Calculates the water supply rate required for saturation of the
  /// hillslope soil
  ...
  /// @param SlopeAngle the angle of the slope (you need to use the function
  ...
  /// conductivity but the lenght unit must be metres (to match contributing
  /// area)
  /// @author SMM
  /// @date 13/11/14
  LSDRaster
  calculate_water_supply_rate_for_saturation(float soil_thick, float K,
  ...
  // surface roughness
  /// @brief Algorithm that assesses surface roughness based on a polynomial
  /// fit.
  ///
  /// @details Runs a moving window across the DEM and assesses the variability
  /// of surface normals within that window.  Specifically the components of the
  ...
  /// @author DTM
  /// @date 13/09/2012
  void calculate_polyfit_directional_cosines(Array2D<float> &d,
                                             Array2D<float> &e,
                                             Array2D<float> &l,
                                             Array2D<float> &m,
                                             Array2D<float> &n);
  ...
  /// @author DTM
  /// @date 13/09/2012
  void calculate_orientation_matrix_eigenvalues(
  ...
  /// @author DTM
  /// @date 15/7/2013
  void calculate_plane_coefficient_matrices(float window_radius,
  ...
  /// @author DTM
  LSDRaster calculate_REI(Array2D<float> &a_plane, Array2D<float> &b_plane,
                          float CriticalSlope);
  ...
  /// @brief this function takes the polyfit functions and requires a window
  /// radius and a vector telling the function which rasters to print to file.
  ///
  ...
  // window of the roughness calcualtion the file codes in the vector are: 0
  // slope 1 aspect 2 curvature 3 planform curvature 4 profile curvature 6
  // tangential curvature 7 classification 8 roughness s1 9 roughness s2 10
  // roughness s3 SMM 19-12-2012
  /// @brief This function takes the combines the polyfit functions and the
  /// roughness function in one package.
  ///
  /// @details The function is data efficient since one does not need to
  ...
  /// @author SMM
  /// @date 19-12-2012
  void calculate_and_print_polyfit_and_roughness_rasters(float window_radius,
                                                         float roughness_radius,
                                                         string file_prefix,
                                                         vector<int> file_list);

  // this function combines the polyfit functions and the roughness function in
  // one package that is data efficient becasue it only requires one calculation
  // of the polyfit matrices. it takes the window radius of the polyfit and the
  // window of the roughness calcualtion the file codes in the vector are: 0
  // roughness s1 1 roughness s2 2 roughness s3 DTM 15-07-2013
  /// @brief This function takes the combines the roughness functions in one
  /// package.
  ///
  /// @details The function is data efficient since one does not need to
  ...
  /// @author DTM
  /// @date 15-07-2013
  void calculate_roughness_rasters(float window_radius, float roughness_radius,
  ...
  /// @brief This function changes any data point either above or below
  /// threshold to NoDataValue
  /// @param threshold The threshold value
  /// @param belowthresholdisnodata a boolean that if true means anything below
  /// the
  ...
  /// @author SMM
  /// @date 28/9/2016
  LSDRaster mask_to_nodata_using_threshold_using_other_raster(
      float threshold, bool belowthresholdisnodata, LSDRaster &MaskingRaster);

  /// @brief This function creats an LSDIndexRaster mask (with true == 1 and
  /// otherwise nodata) from an LSDRaster. Can mask either above or below a
  /// threshold
  /// @param threshold The threshold value
  /// @param belowthresholdisnodata a boolean that if true means anything below
  /// the
  ...
  /// @date 9/9/2016
  LSDIndexRaster
  mask_to_indexraster_using_threshold(float threshold,
                                      bool belowthresholdisnodata);
  ...
  ///@brief This function fills pits/sinks in a DEM by incrementing elevations
  ///for cells with no downslope neighbour. The process is repeated adnausium
  /// until no cells require incrementing.
  ///
  /// Inputs required are a DEM file in ascii raster format as created by ARCMap
  /// and a file name to create a filled DEM grid.
  ///
  /// This code was built ontop of code made available by Jon D. Pelletier as
  /// part of his book:
  ///
  ...
  ///
  /// @details Computes the proportion of all downslope flows for each cell in
  /// the input DEM, and weights them using the equation from Freeman et al 1991
  /// and routes the flow accordingly.
  ///
  /// Paper link:
  /// http://www.sciencedirect.com/science/article/pii/009830049190048I
  ///
  /// Cardinal Weighting = (elevation_drop/total_elevation_drop)^1.1  \n
  /// Diagonal Weighting =
  /// ((elevation_drop/total_elevation_drop)*(1/root(2)))^1.1
  ///
  /// Can <b>NOT</b> handle DEMs containing flats or pits -  must be filled
  /// using the new LSDRaster fill. Function built around original c++ code by
  /// Martin Hurst.
  /// @return LSDRaster of flow area.
  /// @author SWDG
  /// @date 18/4/13
  ...
  /// @brief Route flow from one source pixel using FreemanMDFlow.  Adapted from
  /// SWDG's code above.
  ...
  /// @brief Extracts a multi-pixel channel network, as opposed to a single
  /// thread channel, using method outlined in Pelletier, 2013, WRR; A robust,
  /// two-parameter method for the extraction of drainage networks from
  /// high-resolution digital
  /// elevation models (DEMs): Evaluation using synthetic and real-world DEMs
  ///
  /// @detail First runs Freeman MD flow routing algorithm from previously
  /// identified source.  Then removes all pixels from the channel network if
  /// the ratio of discharge to upstream channel heads is below a user-specified
  /// threshold
  /// @param vector<int> channel_heads_rows - a vector of source node row
  /// indices
  /// @param vector<int> channel_heads_cols - a vector of source node column
  /// indices
  /// @param float R_threshold - the threshold ratio of discharge:upstream
  /// channel heads; Pelletier (2013) reccomends a value of 0.1
  /// @author DTM
  /// @date 22/05/2014
  ...
  ///
  /// @details Computes the proportion of all downslope flows for each cell in
  /// the input DEM, and weights them using the equation from Quinn et al 1991
  /// and routes the flow accordingly.
  ///
  /// Paper link:
  ...
  ///
  /// Cardinal Weighting =
  /// (elevation_drop/total_elevation_drop)*DataResolution/2 \n Diagonal
  /// Weighting = ((elevation_drop/total_elevation_drop)*(1/root(2)))*
  ...
  ///
  /// @details Computes the proportion of all downslope flows for each cell in
  /// the input DEM. Finds the cell of the steepest descent and then checks the
  /// two neighbouring cells slopes. If either is also downslope proportion flow
  ...
  //  /// @brief This function is used to predict channel head locations based
  //  on the method proposed by Pelletier (2013).
  //  ///
  //  /// @details It creates a contour curvature map and identifies channel
  //  heads as pixels greater
  //  /// than a user defined contour curvature threshold value, set by default
  //  at 0.1.  The threshold curvature
  //  /// can also be defined as a multiple of the standard deviation of the
  //  curvature.  Before this function is called
  //  /// the DEM must be filtered using the wiener filter in the
  //  LSDRasterSpectral object in order to remove high frequency
  //  /// noise.
  //  ///
  //  /// Reference: Pelletier (2013) A robust, two-parameter method for the
  //  extraction of drainage
  //  /// networks from high-resolution digital elevation models (DEMs):
  //  Evaluation using synthetic and real-world
  ...
  //  /// @author FC, DTM
  //  /// @date 07/11/13
  //    LSDIndexRaster calculate_pelletier_channel_heads(float window_radius,
  //    float tan_curv_threshold, Array2D<float>& tan_curv_array);

  // some tools associated with ridgeline analyis

  /// @brief Module to sample LSDRaster values running along a ridgetop network.
  ///
  /// @details Ridge network is generated from
  /// LSDJunctionNetwork::ExtractRidges.
  ...
  /// @brief Pass a buffer over a ridge LSDRaster object to increase sampling
  /// area.
  ///
  ...
  /// @brief Module calculates an average value to each for a given raster from
  /// within a basin.
  ...
  /// @brief Write the area(in spatial units) of each basin to the basin's
  /// pixels.
  ///
  ...
  /// each run of Basin IDs. Gives a fast count of the number of pixels per
  /// basin, which is multiplied by the cellsize to get an area in spatial
  /// units.
  ...
  /// @brief Convert a basin, given by a basin ID, into a chain of xy
  /// coordinates for fast plotting of vector basin outlines.
  ///
  /// @details Produces a generalised polygon and will not cope well with
  /// complex geometries.
  /// \n\n
  /// Needs to be updated to write data into an esri ascii format so the files
  /// can be loaded into arc. Currently writes to a text file called chain.txt.
  ...
  /// @brief Punch basins out of an LSDRaster to create DEMs of a single
  /// catchment.
  ///
  /// @details Writes files in the user supplied format (flt or asc) and returns
  /// a vector of LSDRasters so they can be loaded into other functions. Updated
  /// 24/9/13 to return a vector of LSDRasters - SWDG.
  ...
  /// @author SWDG
  /// @date 27/8/13
  vector<LSDRaster> BasinPuncher(vector<int> basin_ids,
  ...
  /// @brief Takes a raster and a corresponding index raster, and calculates the
  /// mean, sd and standard error by index.  Returns four vectors: mean,
  /// st.dev., st.err and Number of points for each category.
  ...
  /// @author DTM
  /// @date 28/11/2013
  void raster_statistics_by_index(LSDIndexRaster &IndexRaster,
                                  vector<float> &mean_vector,
                                  vector<float> &sd_vector,
                                  vector<float> &serr_vector,
                                  vector<int> &Npts_vector);

  /// @brief Generate data in two text files to create a boomerang plot as in
  /// Roering et al [2007].
  ///
  /// @details Should now do all it needs to do: if it gets any more complex
  /// I'll look at refactoring it into a few methods. The latest addition is to
  /// take a bin_threshold to allow different sizes of bins to be removed from
  /// the final analysis - SWDG 11/11/13.
  ...
  /// @param log_bin_width Width (in log space) of the bins, with respect to
  /// D_inf.
  /// @param SplineResolution Number of values between each point for the spline
  /// curve.
  /// @param bin_threshold Threshold fraction of values needed to keep a bin.
  /// @return A pair of floats containing the two LH values in the order
  /// LH(bins), LH(splines).
  /// @author SWDG
  /// @date 27/8/13
  pair<float, float> Boomerang(LSDRaster &Slope, LSDRaster &D_inf,
                               string RasterFilename, float log_bin_width = 0.1,
  ...
  /// cell of the identified basin. Completely rewritten to optimise
  /// performance. Now only traverses the DEM 3 times regardless of the number
  /// of input basins, previously traversed the DEM (2*No of Basins + 2) times.
  /// \n\n Works by creating a pair of vectors of basin IDs and stream length
  /// for each pixel which are then sorted by basin ID and summed in order to
  /// calculate DD in a single pass over the DEM.
  /// @param StreamNetwork LSDIndexRaster of the stream network.
  /// @param Basins LSDIndexRaster of the basins to be analysed.
  /// @param FlowDir Array2D of flowdirections generated by
  /// FlowInfo.get_FlowDirection().
  /// @return LSDRaster of basins coded with drainage density.
  /// @author SWDG
  /// @date 19/11/13
  LSDRaster DrainageDensity(LSDIndexRaster &StreamNetwork,
  ...
  /// @author FJC
  /// @date 27/03/15
  float get_drainage_density_of_raster(LSDIndexRaster &StreamNetwork,
                                       Array2D<int> FlowDir);
  ...
  /// @param step Integer value used to thin the data, 1 preserves all the data,
  /// 2 keeps every second point and so on.
  /// @author SWDG
  /// @date 20/1/14
  void GetVectors(LSDRaster Magnitude, LSDRaster Direction, string output_file,
                  int step);

  // Smoothing tools
  // Nonlocal Means Filtering - Default values following Baudes et al [2005]
  /// @brief Perform Non-local means filtering on a DEM following Baude et al
  /// [2005].
  ///
  /// @details Smoothes non-gaussian noise. Martin Hurst, February, 2012
  /// Modified by David Milodowski, May 2012- generates grid of recording
  /// filtered noise
  ///
  ...
  /// gives the new border mirror symmetric values of the original array
  /// reflected across the boundary.
  ///
  /// @details SimilarityRadius should be the size of the window if filtering.
  /// New array has size nrows + 2*SimilarityRadius by ncols +
  /// 2*SimilarityRadius.
  /// @param PaddedRasterData Padded LSDRaster.
  /// @param SimilarityRadius similarity window radius.
  /// @author Martin Hurst
  /// @date February 2012
  void PadRasterSymmetric(Array2D<float> &PaddedRasterData,
                          int &SimilarityRadius);

  /// @brief Generate gaussian weighted kernel.
  ///
  /// @details kernel array must be predeclared of size SimilarityRadius and
  /// consist of zeros: Array2D<float>
  /// Kernel(SimilarityRadius,SimilarityRadius,0.0);
  ///
  /// Kernel generated using: G(x,y) = (1/2*pi*sigma^2) exp
  /// ((-x^2+y^2)/(2*sigma^2))
  /// @param Kernel
  /// @param sigma
  /// @param SimilarityRadius similarity window radius.
  /// @author Martin Hurst
  /// @date February 2012
  void MakeGaussianKernel(Array2D<float> &Kernel, float sigma,
                          int SimilarityRadius);

  ///  @brief Filters the raster using a square 2D Gaussian filter.  The filter
  ///  is applied using a moving kernel of gaussian weights with a radius of
  ///  3*sigma
  ///
  ///  @details Equivalent lenghtscale of smoothing (from Lashermes et al.,
  ///  2007) original data -> 4*sigma 1st derivative -> 2*pi*sigma 2nd
  ///  derivative -> sqrt(2)*pi*sigma
  ///  @param sigma
  ///  @param kr (an optional integer halfwidth for the smoothing window.  If
  ///  not supplied then the window is set to cover at least three*sigma)
  ///  @return a filtered raster
  ///  @author David Milodowski
  ///  @date Feb 2015
  LSDRaster GaussianFilter(float sigma, int kr = 0);

  ///  @brief Filters the raster using a Perona-Malik non-linear diffusion
  ///  filter
  ///
  ///  @details This follows the algorithm descibed in Passalacqua et al.
  ///  (2010), A geometric framework for channel network extraction from lidar:
  ///  Nonlinear diffusion and geodesic paths, J. Geophys. Res., 115(F1),
  ///  F01002, doi:10.1029/2009JF001254. See also  Catte et al. (1992), Image
  ///  Selective Smoothing and Edge Detection by Nonlinear Diffusion, SIAM J.
  ///  Numer. Anal., 29(1), 182-193, doi:10.1137/0729012.
  ///  @param timesteps the number of diffusion timesteps.  Suggest ~50 for 1m
  ///  LiDAR
  ///  @param lambda percentile (selects the gradient percentile that is used to
  ///  define lambda.  Suggest 90th percentile)
  ///  @param dt The timestep for each round of diffusion.  Suggest that this is
  ///  0.1 to avoid stability issues (same as Passalacqua et al. 2010)
  ///  @return a filtered raster
  ///  @author David Milodowski
  ///  @date Feb 2015
  LSDRaster PeronaMalikFilter(int timesteps, float percentile_for_lambda,
                              float dt);
  ...
  ///
  /// @details Algorithm takes a filled DEM and for each cell calculates the
  /// steepest descent based on 8 triangular facets. Flow direction is assigned
  /// as an angle from 0-360 degrees with -1 used to flag unresolved areas such
  /// as pits.
  ///
  ...
  /// @brief Main function for generating a D-infinity flow area raster after
  /// Tarboton (1997).
  ///
  /// @details Calls the recurisve D_infAccum function to get flow area for each
  ...
  /// @brief Recursive function to calculate accumulating area for a given
  /// pixel.
  ///
  ...
  /// @param Flowarea_Raster Array of the flow areas which is populated using
  /// this function.
  /// @param FlowDir_array Array of Flowdirections generated by D_inf_FlowDir().
  /// @author SWDG
  /// @date 26/07/13
  void D_infAccum(int i, int j, Array2D<float> CountGrid,
                  Array2D<float> Flowarea_Raster, Array2D<float> FlowDir_array);

  /// @brief Wrapper Function to create a D-infinity flow area raster with one
  /// function call.
  /// @return LSDRaster of D-inf flow areas in pixels.
  /// @author SWDG
  /// @date 26/07/13
  ...
  /// @brief Wrapper Function to create a D-infinity flow area raster, in
  /// spatial units, with one function call.
  /// @return LSDRaster of D-inf flow areas in spatial units.
  /// @author SWDG
  /// @date 16/10/13
  LSDRaster D_inf_units();

  ///@brief Wrapper Function to convert a D-infinity flow raster into spatial
  ///units.
  /// @return LSDRaster of D-inf flow areas in spatial units.
  ...
  /// @brief Function to generate upslope contributing area extent from the
  /// D-infinity flow directions to an LSDRaster.
  ...
  /// @author MDH (after SWDG)
  /// @date 26/08/14
  LSDIndexRaster D_inf_watershed(LSDRaster D_inf_FlowDir, int PourRow,
                                 int PourCol);
  ...
  /// @author DTM
  /// @date 01/04/2014
  LSDRaster calculate_topographic_index(LSDRaster &AccumulationArea,
                                        LSDRaster &Slope);

  /// @brief Method to turn a point shapefile into an LSDIndexRaster.
  ///
  /// @details Can be used to turn a shapefile of channel heads into a sources
  /// raster. Does not do any bounds checking or shapefile type checking.
  /// @param FileName The path and filename of the shapefile to be loaded, must
  ...
  ///
  /// @details Can be used to turn a shapefile of a river network into a raster.
  /// Does not do any bounds checking or shapefile type checking.
  ///
  /// Works by calculating points along each line spaced by less than the data
  /// resolution. This has the effect of flaggin every raster cell along a
  /// polyline.
  /// @param FileName The path and filename of the shapefile to be loaded, must
  /// include the file extension.
  ...
  /// @param OutputResolution the resolution in spatial units to be resampled
  /// to.
  ...
  ///
  /// @details The second argument (neighbourhood_switch) specifies the
  /// neighbourhood type:
  ...
  /// @brief gets mean & standard deviation value specified circular
  /// neighbourhood
  ///
  /// @details The second argument (neighbourhood_switch) specifies the
  /// neighbourhood type:
  ...
  /// @date 24/09/2014
  vector<LSDRaster>
  neighbourhood_statistics_spatial_average_and_SD(float window_radius,
                                                  int neighbourhood_switch);
  ...
  /// @brief tests neighbourhood for the fraction of values for which the
  /// specified condition is met.
  ///
  /// The second argument (neighbourhood_switch) specifies the neighbourhood
  /// type:
  ///   0 Square neighbourhood
  ///   1 Circular window
  /// @details the third argument (condition_switch) specifies the condition
  /// using an integer as follows:
  ///   0 ==
  ///   1 !=
  ///   2 >
  ///   3 >=
  ...
  /// @param float test_value -> the value to test against in the conditional
  /// statement
  /// @return an LSDRaster with the fraction of cells in neightbourhood for
  /// which condition statement is true
  /// @author DTM
  /// @date 20/06/2014
  LSDRaster neighbourhood_statistics_fraction_condition(
  ...
  /// @param int irregular_switch (0 (default) -> only creates border pixels
  /// around raster edge; 1 -> creates border around edges and nodata values
  /// e.g. for irregularly shaped datasets)
  ...
  ///  changes directions, four sweep directions in all (+ rows, - rows, + cols,
  ///  -cols) and it alternates between these directions until the raster is
  ///  filled.
  /// @param window_size the number of pixles around the centre pixel to take
  /// the
  ///  spatial average
  /// @author SMM
  /// @date 09/12/2014
  ...
  ///  changes directions, four sweep directions in all (+ rows, - rows, + cols,
  ///  -cols) and it alternates between these directions until the raster is
  ///  filled. The routine is particularly useful for data with holes that is to
  ///  be prepared for spectral analysis.
  /// @param window_size the number of pixles around the centre pixel to take
  /// the
  ///  spatial average
  /// @author SMM
  ...
  /// @brief Function to fill in no data holes in an irregular raster. Pixel
  /// must have all neighbours not equal to no data value within the specified
  /// window radius.  Data is filled based on mean of pixels within the window
  /// radius.
  LSDRaster nodata_fill_irregular_raster(int window_radius);

  /// @brief A routine that fills nodata holes. Modified by FJC to only fill
  /// holes surrounded in all directions by pixels with valid elevation values
  /// @detail The routine sweeps the raster looking for nodata and filling
  ///  this nodata with an average value from surrounding nodes. The sweeping
  ///  changes directions, four sweep directions in all (+ rows, - rows, + cols,
  ///  -cols)
  ...
  /// @date 09/12/2014
  LSDRaster
  alternating_direction_nodata_fill_irregular_raster(int window_width);

  /// @brief Function to create a masked LSDIndexRaster raster based on a
  ...
  ///
  /// @detail picks departure from gaussian behaviour, then uses this as a
  /// threshold to create a binary dataset. Lashermes, B., E. Foufoula-Georgiou,
  /// and W. E. Dietrich (2007), Channel network extraction from high resolution
  /// topography using wavelets, Geophys. Res. Lett., 34, L23S04,
  /// doi:10.1029/2007GL031140.
  /// @param string q_q_file A text file containing data to produce a q_q_plot.
  /// @return LSDIndexRaster A binary raster where the pixel value is 1 where
  /// the input raster exceeded the defined threshold
  /// @author DTM
  /// @date 11/02/2015
  LSDIndexRaster IsolateChannelsLashermesCurvature(float sigma,
                                                   string q_q_filename);
  LSDIndexRaster IsolateChannelsLashermesAspect(float sigma,
                                                string q_q_filename);
  LSDIndexRaster IsolateChannelsLashermesFull(float sigma,
                                              string q_q_filename_prefix);
  LSDIndexRaster IsolateChannelsLashermesCurvatureArea(float sigma,
                                                       float area_threshold,
                                                       string q_q_filename);
  /// @brief method to locate channel pixels outlined by Passalacqua et al.
  /// (2010).
  ///
  /// @detail Isolate channelised portions of the landscape using the method
  /// proposed by Passalacqua et al. (2010) A geometric framework for channel
  /// network extraction from lidar: Nonlinear diffusion and geodesic paths, J.
  /// Geophys. Res., 115(F1), F01002, doi:10.1029/2009JF001254.
  ///
  /// @param number of timesteps for non-linear filtering steps - suggest 20 in
  /// complex terrain
  /// @param a catchment area threshold for pruning
  /// @param a window radius for surface fitting from which curvature
  /// calculation is performed
  /// @return LSDIndexRaster A binary raster where the pixel value is 1 where
  /// the input raster exceeded the defined threshold
  /// @author DTM
  /// @date 21/04/2015
  LSDIndexRaster IsolateChannelsGeonet(float timesteps, float area_threshold,
                                       float window_radius,
                                       string q_q_filename);
  /// @brief uses quantile-quantile analysis to pick departure from gaussian
  /// behaviour, then uses this as a threshold to create a binary dataset.
  /// @param string q_q_file A text file containing data to produce a q_q_plot.
  /// @return LSDIndexRaster A binary raster where the pixel value is 1 where
  /// the input raster exceeded the defined threshold
  /// @author DTM     /// @return LSDIndexRaster A binary raster where the pixel
  /// value is 1 where the input raster exceeded the defined threshold
  /// @author DTM
  /// @date 10/02/2015
  ...
  /// @brief Function to calculate the curvature threshold used for DrEICH
  /// channel extraction which varies across the landscape
  ...
  /// @brief Method to convert an LSDRaster hilltop file into a series of
  /// contigiuous hilltop patches.
  ///
  /// @details Connects all contiguous patches of hilltop via a unique ID,
  /// allowing hilltop patches to be generated in a manner similar to the
  /// methodology employed by Hurst et al. (2012) and (2013) to spatially
  /// average data for E*R* calculations. Currently has no method to limit the
  /// maximum size of segments.
  /// @param minimum_patch_size Integer of the minimum number of pixels required
  /// for a patch to be created
  ...
  /// @brief Simple method to flatten an LSDRaster to a text file, with a sigle
  /// value on each line.
  /// @param FileName The name of the file to write, if no path is included it
  /// will write to the current directory.
  /// @author SWDG
  /// @date 9/2/15
  void FlattenToFile(string FileName);

  /// @brief Method to flatten an LSDRaster and place the non NDV values in a
  /// csv file.
  ///
  /// @detail Each value is placed on its own line, so that it can be read more
  /// quickly in python etc.
  ///   It includes the x and y locations so it can be read by GIS software
  /// @param FileName_prefix The prefix of the file to write, if no path is
  /// included it will write to the current directory.
  ...
  /// @brief Simple method to remove any values below a user supplied value from
  /// an LSDRaster.
  ...
  /// @brief Simple method to remove any values above a user supplied value from
  /// an LSDRaster.
  ...
  /// @brief apply a mask to a raster dataset, converts pixels to nodata where
  /// mas=k=1
  /// @author DTM
  /// @date 25/08/2015
  ...
  /// @param Value, an integer value that will be assigned to every non NDV cell
  /// in the raster.
  ...
  /// @brief Method to merge data from two LSDRasters WITH SAME EXTENT together.
  /// /// The data from the raster specified as an argument will be added (will
  ...
  /// @brief Method to merge data from two LSDRasters WITH SAME EXTENT together.
  /// /// The data from the raster specified as an argument will be added (will
  /// overwrite the original raster if there is a conflict). Overloaded function
  /// to rewrite original raster rather than creating a new one
  /// @param RasterToAdd second raster to add to original raster
  /// @author FJC
  /// @date 07/04/17
  void OverwriteRaster(LSDRaster &RasterToAdd);

  /// @brief Function to get potential floodplain patches using a slope and
  /// relief threshold
  /// @param Relief raster with relief values
  /// @param Slope raster with slope values
  /// @param relief_threshold threshold value for relief (floodplain must be
  /// lower than this)
  /// @param slope_threshold threshold value for slope (floodplain must be lower
  /// than this)
  /// @author FJC
  /// @date 20/10/15
  LSDIndexRaster get_potential_floodplain_patches(LSDRaster &Relief,
                                                  LSDRaster &Slope,
                                                  float relief_threshold,
                                                  float slope_threshold);

  /// @brief Function to get threshold from a raster using a histogram of
  /// Probability-Density values. The function identifies the peaks in the PDF,
  /// and sets the threshold to the peak with the lowest value.  This can be
  /// used to set the floodplain threshold.
  /// @param bin_width Bin width for histogram
  /// @param peak_threshold Threshold probability-density value above which
  /// peaks will be selected
  /// @param peak_distance Threshold distance between identified peaks (will
  /// remove smaller peaks close to the largest ones)
  /// @author FJC
  /// @date 16/11/15
  float get_threshold_for_floodplain(float bin_width, float peak_threshold,
                                     int peak_distance);

  /// @brief Function to get threshold from a raster using qq plots
  /// @param q_q_filename Filename for the txt file with the data to visualise
  /// the qq plot
  /// @author FJC
  /// @date 16/11/15
  ...
  /// @brief Get the lengths in spatial units of each part of the channel
  /// network, divided by strahler order.
  ...
  /// @author SWDG
  /// @date 17/1/16
  string ChannelLengthByOrder(LSDIndexRaster &StreamNetwork,
                              Array2D<int> FlowDir);

  /// @brief Populate a raster with random noise drawn from a gaussian
  /// distribution of given mean and minimum values.
  ...
  /// @brief Write CHT and hilltop gradient data to a *.csv file, coded by UTM
  /// coordinates as well as lat/long.
  ///
  /// @detail Pass in a CHT raster, CHT raster that has been filtered by
  /// gradient, a slope raster, a filename and some spatial reference
  /// information used to generate Lat Long values.
  ///
  /// The code outputs a csv to the filename specified with the header
  /// structure:
  ///
  ///  "_ID,Easting,Northing,Lat,Long,CHT,gradient_flag,gradient"
  ///
  /// Where the gradient_flag is a binary switch where 0 == above threshold and
  /// 1 == below threshold.
  /// @param CHT LSDRaster of the hilltop curvature.
  /// @param CHT_gradient LSDRaster of the hilltop curvature, filtered by a
  /// gradient threshold.
  /// @param gradient LSDRaster of the topographic gradient.
  /// @param UTMZone The UTM zone the data falls in.
  /// @param isNorth Boolean set to true if the data are in the northern
  /// hemisphere and false if not.
  /// @param eId The ellipsoid ID, see LSDCoordinateConverterLLandUTM for
  /// details, WGS84 is 22.
  /// @param filename A string containing a path and filename to write the data
  /// to.
  /// @author SWDG
  /// @date 2/11/16
  void HilltopsToCSV(LSDRaster &CHT, LSDRaster &CHT_gradient,
                     LSDRaster &gradient, int UTMZone, bool isNorth, int eId,
                     string filename);

  /// @brief Sample the values of 3 input rasters that intersect with the point
  /// a,b within the area defined by threshold.
  ///
  ...
  /// @return A vector of vectors of floats, containing the sampled values for
  /// each raster.
  /// @author SWDG
  /// @date 23/1/17
  vector<vector<float>> Sample_Along_Ridge(LSDRaster &Raster1,
                                           LSDRaster &Raster2,
                                           LSDRaster &Raster3, int a, int b,
                                           int threshold);
...
private:
  ...
  void create(int ncols, int nrows, float xmin, float ymin, float cellsize,
              float ndv, Array2D<float> data);
  void create(int ncols, int nrows, double xmin, double ymin, double cellsize,
              int ndv, Array2D<double> data);
  void create(int ncols, int nrows, float xmin, float ymin, float cellsize,
              float ndv, Array2D<float> data, map<string, string> GRS);
};
