...
// replaces data in residuals with residuals and returns a 4 element vector,
// which has slope, intercept, r^2 and the Durbin-Watson test statistic which
// looks for autocorrelation of the residuals
...
// this function gets the difference between nearest neighbours in a vector of y
// data FJC 10/11/15
vector<float> difference(vector<float> &y_data);

// this function gets the main peaks from a vector of y data using the first
// order difference FJC 13/11/15
...
// variate, returning an (evenly spaced) sorted subsample of N_points, their
// corresponding normal variate and the reference value  from the standard
// normal distribution.  Test for departures from normality within the given
// distribution.
void generate_q_q_plot(vector<float> &data, vector<float> &values,
...
void populate_segment_matrix(int start_node, int end_node, float no_data_value,
                             ...
                             int maximum_segment_length, float sigma,
...
void calculate_segment_matrices(
    ...
    int maximum_segment_length, float sigma, Array2D<float> &like_array,
    Array2D<float> &m_array, Array2D<float> &b_array,
...
// this gets the number of segments for several different values of sigma bassed
// in the vector sigma_values
...
// these functions manipulate likelihood matrices and vectors for use with the
// segment tool
...
// functions for partitioning and permutation (to be used with linear segment
// finding
...
// this generates random segments for use in testing the segment finding
// algorithm
void generate_random_segments(float sigma, int minimum_n_nodes,
                              ...
                              vector<int> &segment_length, vector<float> &slope,
...
// look for empty bins output from the log binning function and removes them to
// avoid plotting several empty bins at 0,0 in some cases. SWDG 6/11/13
...
// Load in a vector of data and convert into a histogram with a specified bin
// width that is printed to file containing:
...
void matlab_int_sort(vector<int> &unsorted, vector<int> &sorted,
                     vector<size_t> &index_map); // added 27/11/13 SWDG
...
// and gets the angle between them. It assumes the starting point of each of the
// vectors is the first node
...
/// Splits a string delimited by a character, c, into a sequence of strings,
/// here stored in a vector, v.
/// @author DAV, but taken out of C++ Cookbook (Stevens, Digins, Turkanis, and
/// Coswell. O'Reilly)
...
// Computing, CUP Inverse Complementary error function.  Returns x such that
// erfc(x)=p within limits 0<p<2
float inverfc(float p);
// Inverse Complementary error function.  Returns x such that erf(x)=p within
// limits -1<p<1
