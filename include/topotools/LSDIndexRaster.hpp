...
public:
  ...
  /// @author SMM
  /// @date 01/01/12
  LSDIndexRaster(int nrows, int ncols, float xmin, float ymin, float cellsize,
                 int ndv, Array2D<int> data) {
  ...
  /// @date 09/09/14
  LSDIndexRaster(int nrows, int ncols, float xmin, float ymin, float cellsize,
                 int ndv, Array2D<int> data, map<string, string> GRS_map) {
  ...
  /// @date 20/05/16
  LSDIndexRaster(int nrows, int ncols, float xmin, float ymin, float cellsize,
                 int ndv, map<string, string> GRS_map, int ConstValue) {
    create(nrows, ncols, xmin, ymin, cellsize, ndv, GRS_map, ConstValue);
  }

  /// @brief Create an LSDIndexRaster from an LSDRaster object, rounding to
  /// nearest int
  /// @return LSDIndexRaster
  ...
  /// @brief Method to flatten an LSDIndexRaster and place the non NDV values in
  /// a csv file.
  ...
  /// @brief Calculate the minimum bounding rectangle for an LSDIndexRaster
  /// Object and crop out all the surrounding NoDataValues to reduce the size
  /// and load times of output rasters.
  ///
  ...
  /// @brief This function runs the hole finding algorithm but instead of
  /// printing
  ...
  /// @brief This is a brute force method for finding all the nodata regions
  /// connected to
  ...
  /// @param Visited and array of numbers for the number of times a pixel is
  /// visited
  ...
  /// @author SMM
  /// @date 17/03/2017
  void release_random_bot(Array2D<int> &Visited, int startrow, int startcol,
                          int NSteps);

  /// @brief Make LSDIndexRaster object using a 'template' raster and an Array2D
  /// of data.
  ...
  /// @author SWDG
  /// @date 17/6/14
  LSDIndexRaster CombineBinaryNetwork(LSDIndexRaster &Network1,
  ...
  /// This allows the preservation of the stream order in addition to the
  /// floodplain geometry.
  /// @param FloodPlain an LSDIndexRaster of the floodplains coded with any
  /// integer value.
  ...
  /// Labeling Algorithm," Image Processing, IEEE Transactions on , vol.17,
  /// no.5, pp.749,756, doi: 10.1109/TIP.2008.919369
  /// @return an LSDRaster with labelled connected components
  /// @author DTM
  /// @date 13/07/2015
  LSDIndexRaster ConnectedComponents();

  /// @brief Method to filter a binary array according to a connected components
  /// threshold
  ///
  /// @author DTM
  /// @date 22/07/2015
  LSDIndexRaster
  filter_by_connected_components(int connected_components_threshold);

  /// @brief A method to thin a multipixel feature (binary) to a single thread
  /// skeleton
  ///
  /// @details Takes a binary array in which features are identified as 1, and
  /// background 0, and thins the features down to a skeleton, using the
  /// thinning algorithm described by Zhang and Suen, 1984, "A fast algorithm
  ...
  /// @brief Method to convert all values in an LSDIndexRaster to a single
  /// value.
  ...
  /// @brief Method to remove patches generated by the connected components
  /// analysis that are smaller than a user defined threshold.
  /// @param minimum_segment_size Size in pixels below which a patch should be
  /// removed.
  /// @author SWDG
  /// @date 17/9/15
  ...
  /// @param window_radius radius over which to search to remove holes (size of
  /// hole)
  /// @author FJC
  /// @date 22/10/15
  LSDIndexRaster remove_holes_in_patches(int window_radius);

  /// @brief Method to remove small holes in patches from a connected components
  /// raster. Holes will only be filled if surrounded by pixels with the same CC
  /// value.
  /// @param window_radius radius over which to search to remove holes (size of
  /// hole)
  /// @author FJC
  /// @date 20/01/16
  LSDIndexRaster
  remove_holes_in_patches_connected_components(int window_radius);
...
private:
  ...
  void create(int ncols, int nrows, float xmin, float ymin, float cellsize,
              int ndv, Array2D<int> data, map<string, string> GRS_map);
  void create(int nrows, int ncols, float xmin, float ymin, float cellsize,
              int ndv, map<string, string> GRS_map, int ConstValue);
