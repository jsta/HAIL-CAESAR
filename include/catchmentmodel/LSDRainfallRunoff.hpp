...
/// @detail It can be used to interpolate or downscale rainfall data from
/// coarser resolutions/grid spacings.
/// @author DAV
class rainGrid {
public:
  ...
  rainGrid(std::vector<std::vector<float>> &rain_data,
           TNT::Array2D<int> &hydroindex, int imax, int jmax,
           int current_rainfall_timestep, int rf_num) {
    //std::cout << "Creating a LSD rainGrid object from a rainfall timeseries and hydroindex..." \
    //             << std::endl;
    create(rain_data, hydroindex, imax, jmax, current_rainfall_timestep,
           rf_num);
  }
  ...
  void interpolateRainfall_RectTrivariateSpline(
      rainGrid &raingrid, const TNT::Array2D<double> &elevation);
  ...
  void write_rainGrid_to_raster_file(double xmin, double ymin, double cellsize,
...
protected:
  /// For a single instance of a 2D rainfall grid, matching the dimensions of
  /// the model domain.
...
private:
  ...
  /// Creates a raingrid for the current timestep by extracting from the netCDF
  /// data file. TO DO
  void create(TNT::Array3D<double> &rain_data, int current_raindata_timestep,
              int imax, int jmax);
};
...
public:
  ...
  runoffGrid(int imax, int jmax) { create(imax, jmax); }

  /// Create a rainfallrunoffGrid from passing params and refs to params
  runoffGrid(int current_rainfall_timestep, int imax, int jmax, int rain_factor,
             double M, const rainGrid &current_rainGrid,
             const TNT::Array2D<double> &elevations) {
    create(current_rainfall_timestep, imax, jmax, rain_factor, M,
           current_rainGrid, elevations);
  }

  // void calculate_catchment_water_inputs(); // Left this is CatchmentModel
  // object for now
  ...
  /// Introduced to be able to create an empty runoff object and then later
  /// initialise it, or update an exisiting runoff grid for a new timestep,
  /// @params Takes a ref to a rainGrid object and the elevations array from
  /// LSDCatchmentModel
  void calculate_runoff(int rain_factor, double M, int jmax, int imax,
                        const rainGrid &current_rainGrid,
                        const TNT::Array2D<double> &elevations);

  void write_runoffGrid_to_raster_file(double xmin, double ymin,
                                       double cellsize,
  ...
  void set_j_mean(int m, int n, double cell_j_mean) {
    j_mean_array[m][n] = cell_j_mean;
  }

protected:
  TNT::Array2D<double> j_array, jo_array, j_mean_array, old_j_mean_array,
      new_j_mean_array;

private:
  void create(int imax, int jmax);
  void create(int current_rainfall_timestep, int imax, int jmax,
              int rain_factor, double M, const rainGrid &current_rainGrid,
              const TNT::Array2D<double> &elevations);
};
