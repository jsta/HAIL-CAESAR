...
/// @brief This class is used primarily for the LSDCatchmentModel, to package up
/// data about the stratigraphy and grain fraction data into neat objects.
...
public:
  ...
  */
  /// Create a GrainMatrix object from references to arrays (in
  /// LSDCatchmentModel, though needn't be this object)
  LSDGrainMatrix(int imax, int jmax, int NoDataVal, int G_MAX,
                 TNT::Array2D<int> &indexes, TNT::Array2D<double> &graindatas,
  ...
  /// Writes the GrainMatrix object to an output text file (Warning: large
  /// file!)
  void write_grainMatrix_to_ascii_file(std::string filename,
