...
void LSDIndexRaster::create(int nrows, int ncols, float xmin, float ymin,
  ...
  if (RasterData.dim1() != NRows) {
    cout << "LSDIndexRaster create::dimension of data is not the same as "
            "stated in NRows!"
         << endl;
    exit(EXIT_FAILURE);
  }
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDIndexRaster::create(int nrows, int ncols, float xmin, float ymin,
                            float cellsize, int ndv,
                            map<string, string> GRS_map, int ConstValue) {
  // cout << "YOYOYOYOY" << endl;
  // cout << "NR: " << NRows << " NC: " << NCols << endl;
...
void LSDIndexRaster::read_raster(string filename, string extension) {
  ...
  if (extension == "asc") {
    ...
    data_in >> str >> NCols >> str >> NRows >> str >> XMinimum >> str >>
        YMinimum >> str >> DataResolution >> str >> NoDataValue;
    ...
    cout << "Loading asc file; NCols: " << NCols << " NRows: " << NRows << endl
         ...
         << " and No Data Value: " << NoDataValue << endl;
    ...
    if (ifs.fail()) {
      ...
      string str;
      ifs >> str >> NCols >> str >> NRows >> str >> XMinimum >> str >>
          YMinimum >> str >> DataResolution >> str >> NoDataValue;
    }
    ifs.close();
    ...
    if (ifs_data.fail()) {
      ...
      if (DataType == 2) {
        ...
        cout << "WARNING loading ENVI raster with unusual data type. " << endl
             << "If you get a crazy DEM go to LINE 604 of LSDIndexRaster.cpp "
                "to debug"
             << endl;
    ...
    cout << "You did not enter and approprate extension!" << endl
         << "You entered: " << extension << " options are .flt and .asc"
         << endl;
    exit(EXIT_FAILURE);
  }
}
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDIndexRaster::get_x_and_y_locations(int row, int col, double &x_loc,
                                           double &y_loc) {
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDIndexRaster::get_x_and_y_locations(int row, int col, float &x_loc,
                                           float &y_loc) {
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDIndexRaster::get_lat_and_long_locations(
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
bool LSDIndexRaster::does_raster_have_same_dimensions(
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
bool LSDIndexRaster::does_raster_have_same_dimensions_and_georeferencing(
    LSDRaster &Compare_raster) {
...
LSDIndexRaster::clip_to_smaller_raster(LSDIndexRaster &smaller_raster) {
  ...
  LSDIndexRaster TrimmedRaster(New_NRows, New_NCols, NewR_XMinimum,
                               NewR_YMinimum, DataResolution, NoDataValue,
                               NewData, GeoReferencingStrings);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
map<string, string>
LSDIndexRaster::Update_GeoReferencingStrings(float NewXmin, float NewYmax) {
...
bool LSDIndexRaster::check_if_point_is_in_raster(float X_coordinate,
  ...
  float X_coordinate_shifted_origin =
      X_coordinate - XMinimum - DataResolution * 0.5;
  float Y_coordinate_shifted_origin =
      Y_coordinate - YMinimum - DataResolution * 0.5;
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Calculate the minimum bounding rectangle for an LSDIndexRaster Object and
// crop out all the surrounding NoDataValues to reduce the size and load times
// of output rasters.
//
...
LSDIndexRaster LSDIndexRaster::RasterTrimmer() {
  ...
  LSDIndexRaster TrimmedRaster(new_row_dimension, new_col_dimension, new_XLL,
                               new_YLL, DataResolution, NoDataValue,
                               TrimmedData);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDIndexRaster::get_points_in_holes_for_interpolation(
    int NSteps, int NSweeps, vector<float> &UTME, vector<float> &UTMN,
    ...
    for (int row = 0; row < NRows; row++) {
      for (int col = 0; col < NCols; col++) {
        ...
        if (Visited[row][col] > 0 && Visited[row][col] < 25) {
          // cout << "Getting this one, visted is: " << Visited[row][col] <<
          // endl;
  ...
  // Note that there cannot be a hole on the edge so we do not loop over the
  // edge
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// This is a brute force method to try and find nodata at edges rather than in
// holes.
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// This is a little algorithm to release random bots that move around in nodata
// areas
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDIndexRaster::release_random_bot(Array2D<int> &Visited, int startrow,
                                        int startcol, int NSteps) {
  long seed = time(NULL);
...

{
  ...
  if (InputData.dim1() == NRows && InputData.dim2() == NCols) {
    LSDIndexRaster OutputRaster(NRows, NCols, XMinimum, YMinimum,
                                DataResolution, NoDataValue, InputData,
                                GeoReferencingStrings);
    return OutputRaster;
  } else {
    cout << "Array dimensions do not match template LSDIndexRaster object"
         << endl;
    exit(EXIT_FAILURE);
  }
}
...
LSDIndexRaster LSDIndexRaster::Resample(float OutputResolution) {
  ...
  LSDIndexRaster OutputRaster(NewNRows, NewNCols, XMinimum, YMinimum,
...
LSDIndexRaster LSDIndexRaster::CombineBinaryNetwork(LSDIndexRaster &Network1,
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      ...
      if (Network1.get_data_element(i, j) == NoDataValue &&
          Network2.get_data_element(i, j) == NoDataValue) {
        CombinedNetworkArray[i][j] = NoDataValue;
      } else if ((Network1.get_data_element(i, j) == NoDataValue &&
                  Network2.get_data_element(i, j) != NoDataValue)) {
        ...
        CombinedNetworkArray[i][j] = Network1.get_data_element(i, j);
                  ...
                  Network2.get_data_element(i, j) != NoDataValue)) {
        CombinedNetworkArray[i][j] =
            Network2.get_data_element(i, j) + Network1.get_data_element(i, j);
      }
    }
  }

  LSDIndexRaster CombinedNetwork(NRows, NCols, XMinimum, YMinimum,
                                 DataResolution, NoDataValue,
                                 CombinedNetworkArray, GeoReferencingStrings);
  return CombinedNetwork;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Method to merge a floodplain raster with a channel raster. Creates an output
// LSDIndexRaster which is coded channel == input channel index, floodplain ==
// 500, NDV == hillslopes. SWDG 05/03/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDIndexRaster
LSDIndexRaster::MergeChannelWithFloodplain(LSDIndexRaster FloodPlain) {
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      ...
      if (ChannelArray[i][j] != NoDataValue &&
          FloodPlain.get_data_element(i, j) != NoDataValue) {
        ChannelArray[i][j] = 1;
      } else if (ChannelArray[i][j] == NoDataValue &&
                 FloodPlain.get_data_element(i, j) != NoDataValue) {
  ...
  LSDIndexRaster CombinedOutput(NRows, NCols, XMinimum, YMinimum,
                                DataResolution, NoDataValue, ChannelArray,
                                GeoReferencingStrings);
...
// Closely follows the algorithm described by He et al. (2008), "A Run-Based
// Two-Scan Labeling Algorithm," Image Processing, IEEE Transactions on ,
// vol.17, no.5, pp.749,756, doi: 10.1109/TIP.2008.919369 Components must be
// identified by the number 1. DTM 13/07/2015
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDIndexRaster LSDIndexRaster::ConnectedComponents() {
  ...
  // Now
  LSDIndexRaster ConnectedComponentsRaster(
      NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
      LabelledComponents, GeoReferencingStrings);
  return ConnectedComponentsRaster;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// The following two functions are used to thin a multi-pixel binary feature
// into a single pixel skeleton.  It uses the algorithm described by Zhang and
// Suen (1984), A fast algorithm for thinning digital patterns, Communications
// of the ACM. Thinning algorithm
...
LSDIndexRaster LSDIndexRaster::thin_to_skeleton() {
  ...
  while (finish_flag == 0) {
    ...
    cout << "removed " << removed << " pixels; " << total_removed
         << "; removed in total     \r";
    binary_old = binary_new.copy();
  }
  cout << "\nDone" << endl;
  LSDIndexRaster skeleton(NRows, NCols, XMinimum, YMinimum, DataResolution,
                          NoDataValue, binary_new.copy(),
                          GeoReferencingStrings);
  return skeleton;
}
...
LSDIndexRaster LSDIndexRaster::find_end_points() {
  ...
  for (int i = 1; i < NRows - 1; ++i) {
    ...
    for (int j = 1; j < NCols - 1; ++j) {
      if (RasterData[i][j] == 1) {
        test = RasterData[i - 1][j] + RasterData[i - 1][j + 1] +
               RasterData[i][j + 1] + RasterData[i + 1][j + 1] +
               RasterData[i + 1][j] + RasterData[i + 1][j - 1] +
               RasterData[i][j - 1] + RasterData[i - 1][j - 1];
        if (test <= 1) {
          EndPoints[i][j] = 1;
        }
  ...
  LSDIndexRaster Ends(NRows, NCols, XMinimum, YMinimum, DataResolution,
                      NoDataValue, EndPoints, GeoReferencingStrings);
  return Ends;
}

void LSDIndexRaster::remove_downstream_endpoints(LSDIndexRaster CC,
                                                 LSDRaster Topo) {
  // first loop through the array to find the number of different components to
  // check
  int max_segment_ID = 0;
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (RasterData[i][j] != NoDataValue &&
          CC.get_data_element(i, j) > max_segment_ID)
  ...
  // Now sort end points by elevation, and remove the lowest elevation point in
  // each group
  ...
  for (int i = 0; i < max_segment_ID + 1; ++i) {
    ...
    if (N > 0) {
      ...
      matlab_float_sort(end_point_elevations[i], end_point_elevations[i],
                        index_map);
...
LSDIndexRaster LSDIndexRaster::filter_by_connected_components(
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (ConnectedComponentsRaster.get_data_element(i, j) != NoDataValue) {
        if (count[ConnectedComponentsRaster.get_data_element(i, j)] >=
  ...
  LSDIndexRaster filtered_raster(NRows, NCols, XMinimum, YMinimum,
                                 DataResolution, NoDataValue, BinaryArray,
                                 GeoReferencingStrings);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Method to remove patches generated by the connected components analysis that
// are smaller than a user defined threshold, minimum_segment_size. SWDG 17/9/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Method to remove small holes in patches from an integer raster (at the moment
// set to run on a raster made up of 0s, 1s, and 2s). FJC 22/10/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDIndexRaster LSDIndexRaster::remove_holes_in_patches(int window_radius) {
  ...
  LSDIndexRaster FilledPatches(NRows, NCols, XMinimum, YMinimum, DataResolution,
                               NoDataValue, FilledRaster,
                               GeoReferencingStrings);
  return FilledPatches;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Method to remove small holes in patches from a connected components raster
// (holes will only be removed if surrounded by pixels with the same CC value).
// FJC 20/01/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDIndexRaster LSDIndexRaster::remove_holes_in_patches_connected_components(
    int window_radius) {
  int pixel_radius = int(window_radius / DataResolution);
  if (window_radius < DataResolution)
  ...
  // search the neighbours of each pixel for NoData values within the window
  // radius
  for (int row = 0; row < NRows; row++) {
    for (int col = 0; col < NCols; col++) {
      if (RasterArray[row][col] == NoDataValue) {
        ...
        // if surrounding pixels all have the same CC value, then fill in the
        // pixel
  ...
  // create new LSDIndexRaster with the filled patches
  LSDIndexRaster FilledPatches(NRows, NCols, XMinimum, YMinimum, DataResolution,
                               NoDataValue, RasterArray, GeoReferencingStrings);
  return FilledPatches;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Method to remove checkerboard pattern from an integer raster (at the moment
// set to run on a raster made up of 0s and 1s). FJC 22/10/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDIndexRaster LSDIndexRaster::remove_checkerboard_pattern() {
  ...
  // create new LSDIndexRaster with the filled patches
  LSDIndexRaster FilledRaster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                              NoDataValue, FilledArray, GeoReferencingStrings);
  return FilledRaster;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// QUALITY ANALYSIS ROUTINES
// These functions can be used to compare two rasters and calculate the
// reliability, sensitivity, etc. of a method FJC 18/01/17
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Function to calculate the quality analysis of floodplain method based on true
// positives and false postivies. Need to pass in a raster of predicted values
// and a raster of actual values Predicted values: 1 = floodplain, NoDataValue =
// not floodplain Actual values: 1 = floodplain, 0 = not floodplain results are
// in a vector: 0 - reliability 1 - sensitvity 2 - false positive rate 3 - true
// negative rate 4 - false negative rate FJC 29/06/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
vector<float> LSDIndexRaster::AnalysisOfQuality(LSDIndexRaster &ActualRaster) {
  ...
  cout << "SumTP = " << SumTP << " SumFP = " << SumFP << " SumTN = " << SumTN
       << " SUM FN = " << SumFN << endl;
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Function to calculate the percentage area difference between the predicted
// and mapped rasters FJC 18/01/17
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
float LSDIndexRaster::GetAreaDifference(LSDIndexRaster &ActualRaster) {
  ...
  cout << "N predicted pixels: " << NPredictedPixels
       << " N actual pixels: " << NActualPixels << endl;
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Method to merge data from two LSDIndexRasters WITH SAME EXTENT together.  The
// data from the raster specified as an argument will be added (will overwrite
// the original raster if there is a conflict). FJC 07/04/17
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
