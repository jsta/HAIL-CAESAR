...
LSDRaster &LSDRaster::operator=(const LSDRaster &rhs) {
  if (&rhs != this) {
    create(rhs.get_NRows(), rhs.get_NCols(), rhs.get_XMinimum(),
           rhs.get_YMinimum(), rhs.get_DataResolution(), rhs.get_NoDataValue(),
           rhs.get_RasterData(), rhs.get_GeoReferencingStrings());
  }
  return *this;
}
...
void LSDRaster::create(int nrows, int ncols, float xmin, float ymin,
  ...
  if (RasterData.dim1() != NRows) {
    cout << "LSDRaster line 89 dimension of data is not the same as stated in "
            "NRows!"
         << endl;
    exit(EXIT_FAILURE);
  }
  if (RasterData.dim2() != NCols) {
    cout << "LSDRaster line 94 dimension of data is not the same as stated in "
            "NRows!"
         << endl;
    exit(EXIT_FAILURE);
  }
...
void LSDRaster::create(int nrows, int ncols, float xmin, float ymin,
  ...
  // cout << "Hey I am making a new raster using some data you gave me!" <<
  // endl;
...
void LSDRaster::read_raster(string filename, string extension) {
  ...
  // cout << "\n\nLoading an LSDRaster, the filename is " << string_filename <<
  // endl;
  ...
  if (extension == "asc") {
    ...
    data_in >> str >> XMinimum >> str >> YMinimum >> str >> DataResolution >>
        str >> NoDataValue;

    // cout << "Loading asc file; NCols: " << NCols << " NRows: " << NRows <<
    // endl
    ...
    if (ifs.fail()) {
      ...
      ifs >> str >> NRows;
      // cout << "NRows: " << NRows << " str: " << endl;
      ifs >> str >> XMinimum >> str >> YMinimum >> str >> DataResolution >>
          str >> NoDataValue;
    }
    ifs.close();

    // cout << "Loading flt file; NCols: " << NCols << " NRows: " << NRows <<
    // endl
    //      << "X minimum: " << XMinimum << " YMinimum: " << YMinimum << endl
    //      << "Data Resolution: " << DataResolution << " and No Data Value: "
    //      << NoDataValue << endl;
    ...
    if (ifs.fail()) {
      ...
      ifs >> str;
      if (str != "ENVI") {
        cout << "\nFATAL ERROR: this is not an ENVI header file!, first line "
                "is: "
             << str << endl;
        exit(EXIT_FAILURE);
      } else {
        ...
        int NChars =
            5000; // need a big buffer beacause of the projection string
        ...
        while (counter < NLines) {
          ...
          if (found != string::npos) {
            ...
            // cout << "startpos: " << start_pos << " and end pos: " << end_pos
            // << endl;
            string info_str =
                lines[counter].substr(start_pos + 1, end_pos - start_pos - 1);
            ...
            // get Y minimum
            // IMPORTANT THIS USES CONVENTION THAT THE MINIMUM AND MAXIMUM
            // VALUES ARE AT THE PIXEL EDGES AS IN QGIS!!!
            YMinimum = YMax - (NRows)*DataResolution;

            // using a string comparison as float(X) != float(X) in many cases
            // due to floating point math
            //  http://www.cygnus-software.com/papers/comparingfloats/comparingfloats.htm
            //  - SWDG
            if (mapinfo_strings[5] != mapinfo_strings[6]) {
              cout << "Warning! Loading ENVI DEM, but X and Y data spacing are "
                      "different!"
                   << endl;
            }

            // cout << "Xmin: " << XMinimum << " YMin: " << YMinimum << "
            // spacing: "
            //      << DataResolution << endl;
    ...
    if (ifs_data.fail()) {
      ...
      if (DataType == 2) {
        ...
        double temp;
        cout << "I am trying to load a double precision raster. Wish me luck!"
             << endl;
        for (int i = 0; i < NRows; ++i) {
          for (int j = 0; j < NCols; ++j) {
            ifs_data.read(reinterpret_cast<char *>(&temp), sizeof(temp));
        ...
        cout
            << "WARNING loading ENVI raster with unusual data type. " << endl
            << "If you get a crazy DEM go to LINE 625 of LSDRaster.cpp to debug"
            << endl;
    ...
    // cout << "Loading ENVI bil file; NCols: " << NCols << " NRows: " << NRows
    // << endl
    //    << "X minimum: " << XMinimum << " YMinimum: " << YMinimum << endl
    //      << "Data Resolution: " << DataResolution << " and No Data Value: "
    //      << NoDataValue << endl;
    ...
    cout << "You did not enter and appropriate extension!" << endl
         << "You entered: " << extension << " options are .flt, .asc and .bil"
         << endl;
    exit(EXIT_FAILURE);
  }
}
...
// TNT::Array2D<double> LSDRaster::read_ascii_raster(string FILENAME)
{
  ...
  if (data_in.fail()) {
    cout << "\nFATAL ERROR: the data file \"" << FILENAME << "\" doesn't exist"
         << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  std::cout << "Loading asc file; NCols: " << NCols << " NRows: " << NRows
            << std::endl
            << "X minimum: " << XMinimum << " YMinimum: " << YMinimum
            << std::endl
            << "Data Resolution: " << DataResolution
            << " and No Data Value: " << NoDataValue << std::endl;
...
void LSDRaster::write_raster(string filename, string extension) {
  ...
  if (extension == "asc") {
    ...
    data_out << "ncols\t" << NCols << "\nnrows\t" << NRows << "\nxllcorner\t"
             << setprecision(14) << XMinimum << "\nyllcorner\t"
             << setprecision(14) << YMinimum << "\ncellsize\t" << DataResolution
    ...
    if (iter != GeoReferencingStrings.end()) {
      ...
      cout << "Warning, writing ENVI file but no coordinate system string"
           << endl;
    }
    ...
    cout << "You did not enter and approprate extension!" << endl
         << "You entered: " << extension << " options are flt, bil and asc"
         << endl;
    exit(EXIT_FAILURE);
  }
}
...
// DAV 2015
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void LSDRaster::write_double_flt_raster(string filename,
                                        string string_filename) {
  // float data (a binary format created by ArcMap) has a header file
  // this file must be opened first
  string dot = ".";
...
void LSDRaster::write_double_raster(string filename, string extension) {
  ...
  if (extension == "asc") {
    ...
    cout << "You did not enter an approprate extension!" << endl
         << "You entered: " << extension
         << " options are asc, flt, bil (bil might not work yet though...)"
         << endl;
    exit(EXIT_FAILURE);
  }
}
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
bool LSDRaster::does_raster_have_same_dimensions(
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
bool LSDRaster::does_raster_have_same_dimensions_and_georeferencing(
    LSDRaster &Compare_raster) {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// Method which takes a new xmin and ymax value and modifys the
// GeoReferencingStrings map_info line to contain these new values. Intended for
// use in the rastertrimmer methods and is called from within these methods.
//
// Modifying georeferencing information by hand is messy and should be avoided
// if at all possible.
//
...
map<string, string> LSDRaster::Update_GeoReferencingStrings(float NewXmin,
  ...
  if (iter != GeoReferencingStrings.end()) {
    ...
    CombineMapinfo << mapinfo_strings[0] << "," << mapinfo_strings[1] << ","
                   ...
                   << "," << mapinfo_strings[5] << "," << mapinfo_strings[6]
                   << "," << mapinfo_strings[7] << "," << mapinfo_strings[8]
                   << "," << mapinfo_strings[9];
...
void LSDRaster::Update_GeoReferencingStrings() {
  ...
  if (iter != GeoReferencingStrings.end()) {
    ...
    cout << "New string is: " << endl
         << GeoReferencingStrings[mi_str_key] << endl;
  } else {
...
void LSDRaster::impose_georeferencing_UTM(int zone, string NorS) {
  ...
  if (NorS.find("N") == 0 || NorS.find("n") == 0) {
    ...
    cout << "imposing georeferencing, but I didn't understand N or S, "
            "defaulting to North."
         << endl;
    str_NorS = "N";
    str_NorSlong = "North";
  ...
  string new_string = str_UTM + delim + str_x + delim + str_y + delim + xmin +
                      delim + ymax + delim + DR + delim + DR + delim +
                      str_UTMZ + delim + str_hemis + delim + str_spheroid;
  ...
  string cs_str = cs_string_firstbit + cs_string_secondbit +
                  cs_string_thirdbit + cs_string_central_merid +
                  cs_string_fifthbit + cs_string_fnbit + cs_string_seventhbit;
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDRaster::get_x_and_y_locations(int row, int col, double &x_loc,
                                      double &y_loc) {
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDRaster::get_x_and_y_locations(int row, int col, float &x_loc,
                                      float &y_loc) {
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDRaster::get_lat_and_long_locations(
    int row, int col, double &lat, double &longitude,
...
void LSDRaster::get_easting_and_northing_vectors(vector<float> &Eastings,
  ...
  for (int row = 0; row < NRows; row++) {
    this_northing.push_back(YMinimum + float(NRows - row) * DataResolution -
                            0.5 * DataResolution);
  }
  for (int col = 0; col < NCols; col++) {
    this_easting.push_back(XMinimum + float(col) * DataResolution +
                           0.5 * DataResolution);
  }
...
vector<float> LSDRaster::interpolate_points_bilinear(vector<float> UTMEvec,
  ...
  // This is quite annoying since the number of rows in the raster is the first
  // dimension and the number of columns is the second dimension. Also the
  // raster is inverted so we need to change the direction of the x vector
  // cout << "The size of the easting is: " << Eastings.size() << " N: " <<
  // Northings.size() << endl; cout << "D1: " << RasterData.dim1() << " D2: " <<
  // RasterData.dim2() << endl;

  if (RasterData.dim2() != int(Eastings.size())) {
    cout << "Something has gone wrong with the dimensions of the x and y data "
            "for interpolation"
         << endl;
  ...
  if (UTMEvec.size() != UTMNvec.size()) {
    cout << "LSDRaster::interpolate_points_bilinear you x and y vecs are not "
            "the same size, prepare for segmentation."
         << endl;
  }
  ...
  for (int i = 0; i < n_samples; i++) {
    ...
    // cout << "Northings: " << Northings[0] << " " << Northings[NCols-1] <<
    // endl; cout << "e: " << UTMEvec[i] << " n: " << UTMNvec[i] << endl;

    // this stupid ordering is due to the fact that the rows are first dimension
    // and the
    this_data = interp2D_bilinear(Eastings, New_northings, flipped, UTMEvec[i],
                                  UTMNvec[i]);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDRaster
LSDRaster::fill_with_interpolated_data(vector<int> node_rows,
...
int LSDRaster::Find_UTM_central_meridian(int UTM_zone) {
  ...
  if (UTM_zone < 1 || UTM_zone > 60) {
    cout << "Trying to assign central meridian but you have chosen an invalid "
            "UTM zone"
         << endl;
...
bool LSDRaster::check_if_point_is_in_raster(float X_coordinate,
  ...
  int row_point =
      (NRows - 1) - int(round(Y_coordinate_shifted_origin / DataResolution));

  if (col_point < 0 || col_point > NCols - 1 || row_point < 0 ||
      row_point > NRows - 1) {
    is_in_raster = false;
  }
...
LSDRaster LSDRaster::create_raster_nodata() {
  ...
  LSDRaster NDV_Raster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                       NoDataValue, NDV_Array, GeoReferencingStrings);
  return NDV_Raster;
}
...
// Right now it can have a circular window with kernalType == 1 or square
// otherwise
//
            ...
            for (int sub_j = j - kr; sub_j <= j + kr; ++sub_j) {
              ...
              if (sub_j >= 0 && sub_j < NCols) {
                ...
                if (RasterData[sub_i][sub_j] != NoDataValue) {
                  ...
                  {
                    if ((pow(sub_i - i, 2) + pow(sub_j - j, 2)) *
                            DataResolution >
                        kernelWidth / 2) {
...
float LSDRaster::WrapSample(int row, int col) {
  ...
  // cout << "Nrows: " << NRows << " row: " << row << " wraprow: " << wraprow <<
  // endl; cout << "Ncols: " << NCols << " col: " << col << " wrapcol: " <<
  // wrapcol << endl;
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// This function start with the corners of the algorithm to make individual
// 'features' note: the scale gives the range, so it should be a random value
// between -scale*0.5 and scale*0.5 SMM 9/1/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void LSDRaster::DSSetFeatureCorners(int featuresize, float scale) {
  ...
  for (int row = 0; row < NRows; row += featuresize) {
    for (int col = 0; col < NCols; col += featuresize) {
      ...
      // cout << "setting feature corners, row: " << row << " and col: " << col
      // << endl;
...
void LSDRaster::DSSampleDiamond(int row, int col, int size, float value) {
  ...
  if (a == -9999 || b == -9999 || c == -9999 || d == -9999) {
    cout << "got a nodata DSSampleDiamond; row: " << row << " and col: " << col
         << endl;
...
void LSDRaster::DiamondSquare_SampleStep(int stepsize, float scale) {
  ...
  for (int row = -stepsize; row < NRows; row += stepsize) {
    for (int col = -stepsize; col < NCols; col += stepsize) {
      ...
      DSSampleDiamond(row + halfstep, col, stepsize,
                      ((ran3(&seed) - 0.5) * scale));
      DSSampleDiamond(row, col + halfstep, stepsize,
                      ((ran3(&seed) - 0.5) * scale));
    }
  }
}
...
LSDRaster LSDRaster::DiamondSquare(int feature_order, float scale) {
  ...
  if (featuresize > max_feature_size) {
    cout << "Your featuresize is too big for the DEM. Changing to max feature "
            "size"
         << endl;
  }
  ...
  cout << "Starting diamond square algorithm, samplesize is: " << samplesize
       << endl;
...
// spatial_analyst_tools/how_hillshade_works.htm. Does not allow for drop
// shadows.
//
...
LSDRaster LSDRaster::hillshade(float altitude, float azimuth, float z_factor) {
  ...
  cout << "Hillshading with altitude: " << altitude << ", azimuth: " << azimuth
       << " and z-factor: " << z_factor << endl;
  ...
  for (int i = 1; i < NRows - 1; ++i) {
    for (int j = 1; j < NCols - 1; ++j) {
      ...
      if (RasterData[i][j] != NoDataValue) {
        dzdx = ((RasterData[i][j + 1] + 2 * RasterData[i + 1][j] +
                 RasterData[i + 1][j + 1]) -
                (RasterData[i - 1][j - 1] + 2 * RasterData[i - 1][j] +
                 RasterData[i - 1][j + 1])) /
               (8 * DataResolution);
        dzdy = ((RasterData[i - 1][j + 1] + 2 * RasterData[i][j + 1] +
                 RasterData[i + 1][j + 1]) -
                (RasterData[i - 1][j - 1] + 2 * RasterData[i][j - 1] +
                 RasterData[i + 1][j - 1])) /
               (8 * DataResolution);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// This function generates a hillshade derivative raster using the algorithm
// outlined in Codilean (2006) (excluding cast shadows), identifying areas in
// shadow as 1 and all other values as 0. This is now obselete since we have the
// CastShadows algorithm.
//
// Is interfaced through LSDRaster::TopoShield and should not be called
// directly, to generate a hillshade use LSDRaster::hillshade instead.
//
// Takes 2 ints, representing theta, the zenith angle of the illumination source
// in degrees and the azimuth angle, phi, of the illumination source in degrees.
//
...
Array2D<float> LSDRaster::Shadow(int theta, int phi) {
  ...
  for (int i = 1; i < NRows - 1; ++i) {
    for (int j = 1; j < NCols - 1; ++j) {
      ...
      if (data[i][j] != NoDataValue) {
        dzdx =
            ((data[i][j + 1] + 2 * data[i + 1][j] + data[i + 1][j + 1]) -
  ...
  // create LSDRaster hillshade object
  // LSDRaster hillshade_raster(NRows, NCols, XMinimum, YMinimum,
  // DataResolution,
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// This function generates a topographic sheilding raster using the algorithm
// outlined in Codilean (2006), creating a raster of values between 0 and 1
// which can be used as a scaling factor in Cosmo analysis.
//
// Goes further than the original algorithm allowing a theoretical theta, phi
// pair of 1,1 to be supplied and although this will increase the computatin
// time significantly, it is much faster than the original Avenue and VBScript
// implementations. Default parameters are 5 and 5 if no arguments supplied
//
// Takes 2 ints, representing the Elevation/Zenith Angle, and Azimuth Angle step
// sizes required. Codilean (2006) used 5,5 as the standard values, but in
// reality values of
// 10,15 are often preferred to save processing time. **steps must be a factor
// of 360**
//
...
LSDRaster LSDRaster::TopographicShielding() {
  ...
  printf("LSDRaster::%s: No input argumments provided, defaulting to "
         "Azimuth/Zenith stepsize of 5 degrees\n",
         __func__);
  int AzimuthStep = 5;
  int PhiStep = 5;
...
LSDRaster LSDRaster::TopographicShielding(int AzimuthStep, int ZenithStep) {
  ...
  // loop through all the theta, phi pairs and increment the FinalArray with the
  // scaled values start at ZenithStep and AzimuthStep and continue up to Max

  for (int ZenithAngle = ZenithStep; ZenithAngle <= 90;
       ZenithAngle += ZenithStep) {
    for (int AzimuthAngle = AzimuthStep; AzimuthAngle <= 360;
         AzimuthAngle += AzimuthStep) {
      fflush(stdout);
      ...
      // Calculate Weighting
      float Weighting = (AzimuthStep * (M_PI / 180.)) *
                        (ZenithStep * (M_PI / 180.)) *
                        cos(ZenithAngle * (M_PI / 180.)) *
                        pow(sin(ZenithAngle * (M_PI / 180.)), m);
  ...
  // write LSDRaster
  LSDRaster Shielding(NRows, NCols, XMinimum, YMinimum, DataResolution,
                      NoDataValue, FinalShieldingFactor, GeoReferencingStrings);
  return Shielding;
}
...
//  //loop through all the theta, phi pairs and increment the FinalArray with
//  the scaled values for(int theta = 10; theta < 90; theta += theta_step)
//  {
...
//  //deal with nodata values on border of dem - more efficient than doing it in
//  the hillshade function 10s of times. for (int i = 0; i < NRows; ++i)
//  {
//    for (int j = 0; j < NCols; ++j)
//    {
...
/*=======================================================================================
  ...
  This function generates a shadows raster containing drop shadows using the
algorithm outlined in Codilean (2006), identifying areas in shadow as 1 and all
other values as 0. It includes the coordinate transformation in order to look
for shadow casting.

  Algorithm works by adjusting the coordiantes of the DEM to be relative to the
azimuth and zenith i.e. X-coordinates should be equal along the Azimuth
direction and the DEM itself is tilted by the angle Zenith toward Azimuth.

  Is interfaced through LSDRaster::TopoShield and LSDRaster::Hillshade, and
should not be called directly,  to generate a hillshade use
LSDRaster::Hillshade.

  Takes 2 ints, the zenith angle of the illumination source in degrees from
horizontal and the azimuth angle, of the illumination source in degrees.
...
LSDRaster LSDRaster::CastShadows(int Azimuth, int ZenithAngle) {
  ...
  return LSDRaster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                   NoDataValue, Shadows, GeoReferencingStrings);
}
...
Array2D<float> LSDRaster::Shadows(int Azimuth, int ZenithAngle) {
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (RasterData[i][j] != NoDataValue) {
        ...
        // Modified from Codilean 2006 whose equation 6 appears to be wrong (may
        // be due to differrent origins?)
        XCoords_Transform[i][j] = XCoords[i][j] * sin(AzimuthRadians) -
                                  YCoords[i][j] * cos(AzimuthRadians);
        // YCoords_Transform[i][j] =
        // XCoords[i][j]*cos(AzimuthRadians)+YCoords[i][j]*sin(AzimuthRadians);

        // Modified from Codilean 2006 whose equation 8 is missing a + sign and
        // needs rederiving following the new equation 6
        ZCoords_Transform[i][j] = (RasterData[i][j] * cos(ZenithRadians) -
                                   (XCoords[i][j] * cos(AzimuthRadians) +
                                    YCoords[i][j] * sin(AzimuthRadians)) *
                                       sin(ZenithRadians));
      }
    }
  }

  // Check direction to start looking and generate search indices for looking in
  // each quadrant to determine if cells down the look direction are in shadow
  // Also determine directions to loop across DEM, e.g. if Az = 0-90 start in
  // upper right corner
  int Reversei = 0;
  int Reversej = 0;
  if (Azimuth >= 0 && Azimuth <= 45) {
    ...
    printf("LSDRaster:FATAL ERROR: Encountered Azimuth out of range. In %s at "
           "line %d\n",
           __func__, __LINE__);
    exit(EXIT_FAILURE);
  }
  ...
  for (int ii = 0; ii < NRows; ++ii) {
    ...
    for (int jj = 0; jj < NCols; ++jj) {
      ...
      // Parameters declared internally to avoid dependencies in parallel
      // processing

      // pull out vector of indices for searching the line in direction of
      // azimuth
      int a = i;
      int b = j;
      ...
      float DiffX, DiffZ;
      int a_temp = 0; // assigned meaningless value to stop compiler warnings,
                      // value will always be updated.
      int b_temp = 0;
      int i_temp, j_temp;

      while (true) {
        // check the three search cells for a minumum X value in rotated
        // coordinate mode
        MinX = 2 * DataResolution;
        bool Initialized =
            false; // this flag tests if a_temp and b_temp have a real value.
        for (int k = 0; k < NSearch; ++k) {
          ...
          DiffX =
              fabs(XCoords_Transform[i_temp][j_temp] - XCoords_Transform[i][j]);
          if (DiffX < MinX) {
            MinX = DiffX;
            Initialized = true;
            a_temp = i_temp;
        ...
        if (Initialized ==
            true) { // without this condition a and b can be assigned values
                    // from unallocated memory, causing crashes.
          // update a and b
          a = a_temp;
          b = b_temp;
        } else
        ...
        if (ShadowFlag > 10)
          break;
        else if (a <= 0 || b == 0 || a == NRows - 1 || b == NCols - 1 ||
                 RasterData[a][b] == NoDataValue)
          break;
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
vector<LSDRaster>
LSDRaster::calculate_polyfit_surface_metrics(float window_radius,
                                             vector<int> raster_selection) {
  Array2D<float> void_array(1, 1, NoDataValue);
  LSDRaster VOID(1, 1, NoDataValue, NoDataValue, NoDataValue, NoDataValue,
                 void_array, GeoReferencingStrings);
  ...
  if (window_radius < sqrt(2) * DataResolution) {
    cout << "Supplied window radius: " << window_radius
         << " is less than the data resolution * sqrt(2), i.e. the diagonal of "
            "a single grid cell: "
         << sqrt(2) * DataResolution
         << ".\nWindow radius has been set to sqrt(2) * data resolution."
         << endl;
  ...
  Array2D<float> elevation_raster, slope_raster, aspect_raster,
      curvature_raster, planform_curvature_raster, profile_curvature_raster,
      tangential_curvature_raster, classification_raster, s1_raster, s2_raster,
      s3_raster;
  ...
  for (int i = 0; i < kw; ++i) {
    for (int j = 0; j < kw; ++j) {
      ...
      radial_dist = sqrt(y_kernel[i][j] * y_kernel[i][j] +
                         x_kernel[i][j] * x_kernel[i][j]);
  ...
  for (int i = 0; i < NRows; ++i) {
    ...
    for (int j = 0; j < NCols; ++j) {
      // Avoid edges
      if ((i - kr < 0) || (i + kr + 1 > NRows) || (j - kr < 0) ||
          (j + kr + 1 > NCols) || RasterData[i][j] == NoDataValue) {
        if (raster_selection[0] == 1)
        ...
        if (ndv_present == 0) // test for nodata values within the selection
        {
          ...
          if (raster_selection[2] == 1) {
            if (d == 0 && e == 0)
              ...
              aspect_raster[i][j] =
                  270. - (180. / M_PI) * atan(e / d) + 90. * (d / abs(d));
          ...
          if (raster_selection[4] == 1 || raster_selection[5] == 1 ||
              raster_selection[6] == 1 || raster_selection[7] == 1) {
            ...
            if (raster_selection[4] == 1) {
              if (q > 0)
                planform_curvature_raster[i][j] =
                    (fxx * fy * fy - 2 * fxy * fx * fy + fyy * fx * fx) /
                    (sqrt(q * q * q));
              else
                planform_curvature_raster[i][j] = NoDataValue;
            }
            if (raster_selection[5] == 1) {
              if ((q * q * q > 0) && ((p * sqrt(q * q * q)) != 0))
                profile_curvature_raster[i][j] =
                    (fxx * fx * fx + 2 * fxy * fx * fy + fyy * fy * fy) /
                    (p * sqrt(q * q * q));
              else
                profile_curvature_raster[i][j] = NoDataValue;
            }
            if (raster_selection[6] == 1) {
              if (q > 0 && (p * sqrt(q)) != 0)
                tangential_curvature_raster[i][j] =
                    (fxx * fy * fy - 2 * fxy * fx * fy + fyy * fx * fx) /
                    (p * sqrt(q));
              else
                tangential_curvature_raster[i][j] = NoDataValue;
            ...
            if (raster_selection[7] == 1) {
              ...
              if (slope < 0.1) {
                if (fxx < 0 && fyy < 0 && fxy * fxy < fxx * fxx)
                  ...
                  classification_raster[i][j] =
                      2; // Conditions for a depression
  ...
  if (raster_selection[0] == 1) {
    LSDRaster Elevation(NRows, NCols, XMinimum, YMinimum, DataResolution,
                        NoDataValue, elevation_raster, GeoReferencingStrings);
    raster_output[0] = Elevation;
  }
  if (raster_selection[1] == 1) {
    LSDRaster Slope(NRows, NCols, XMinimum, YMinimum, DataResolution,
                    NoDataValue, slope_raster, GeoReferencingStrings);
    raster_output[1] = Slope;
  }
  if (raster_selection[2] == 1) {
    LSDRaster Aspect(NRows, NCols, XMinimum, YMinimum, DataResolution,
                     NoDataValue, aspect_raster, GeoReferencingStrings);
    raster_output[2] = Aspect;
  }
  if (raster_selection[3] == 1) {
    LSDRaster Curvature(NRows, NCols, XMinimum, YMinimum, DataResolution,
                        NoDataValue, curvature_raster, GeoReferencingStrings);
    raster_output[3] = Curvature;
  }
  if (raster_selection[4] == 1) {
    LSDRaster PlCurvature(NRows, NCols, XMinimum, YMinimum, DataResolution,
                          NoDataValue, planform_curvature_raster,
                          GeoReferencingStrings);
    raster_output[4] = PlCurvature;
  }
  if (raster_selection[5] == 1) {
    LSDRaster PrCurvature(NRows, NCols, XMinimum, YMinimum, DataResolution,
                          NoDataValue, profile_curvature_raster,
                          GeoReferencingStrings);
    raster_output[5] = PrCurvature;
  }
  if (raster_selection[6] == 1) {
    LSDRaster TanCurvature(NRows, NCols, XMinimum, YMinimum, DataResolution,
                           NoDataValue, tangential_curvature_raster,
                           GeoReferencingStrings);
    raster_output[6] = TanCurvature;
  }
  if (raster_selection[7] == 1) {
    LSDRaster Class(NRows, NCols, XMinimum, YMinimum, DataResolution,
                    NoDataValue, classification_raster, GeoReferencingStrings);
...
// DTM 01/04/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
vector<LSDRaster> LSDRaster::calculate_polyfit_roughness_metrics(
    float window_radius1, float window_radius2, vector<int> raster_selection) {
  Array2D<float> void_array(1, 1, NoDataValue);
  LSDRaster VOID(1, 1, NoDataValue, NoDataValue, NoDataValue, NoDataValue,
                 void_array, GeoReferencingStrings);
  ...
  if (window_radius1 < sqrt(2) * DataResolution) {
    cout << "Supplied window radius: " << window_radius1
         << " is less than sqrt(2) * data resolution: "
         << sqrt(2) * DataResolution
  ...
  if (window_radius2 < sqrt(2) * DataResolution) {
    cout << "Supplied window radius: " << sqrt(2) * window_radius2
         << " is less than the data resolution: " << sqrt(2) * DataResolution
         << ".\nWindow radius has been set to data resolution." << endl;
  ...
  for (int i = 0; i < NRows; ++i) {
    ...
    for (int j = 0; j < NCols; ++j) {
      ...
      if ((i - kr < 0) || (i + kr + 1 > NRows) || (j - kr < 0) ||
        ...
        // Fit polynomial surface, avoiding nodata values ==================>
        // Could change this, as can fit polynomial surface as long as there are
        // 6 data points.
        if (ndv_present == 0) // test for nodata values within the selection
        {
          Array1D<float> bb(6, 0.0);
  ...
  // EIGENVALUES.
  // Prepare new kernel
  kr = ceil(window_radius2 /
            DataResolution); // Set radius of kernel as >= specified radius
  ...
  for (int i = 0; i < kw; ++i) {
    for (int j = 0; j < kw; ++j) {
      ...
      radial_dist = sqrt(
          y_kernel_ref * y_kernel_ref +
          x_kernel_ref * x_kernel_ref); // distance from centre to this point.
  ...
  cout << "Finding eigenvalues for local surface. Search radius = " << kr << "m"
       << endl;
  for (int i = 0; i < NRows; ++i) {
    cout << flush << "\tRow = " << i + 1 << " / " << NRows << "    \r";
    for (int j = 0; j < NCols; ++j) {
      ...
      if ((i - kr < 0) || (i + kr + 1 > NRows) || (j - kr < 0) ||
        ...
        for (int i_kernel = 0; i_kernel < kw; ++i_kernel) {
          for (int j_kernel = 0; j_kernel < kw; ++j_kernel) {
            pheta_kernel[i_kernel][j_kernel] =
                pheta[i - kr + i_kernel][j - kr + j_kernel];
            phi_kernel[i_kernel][j_kernel] =
                phi[i - kr + i_kernel][j - kr + j_kernel];
            // check for nodata values nearby
        ...
        int N = 0;
        //         if(ndv_present == 0)  // test for nodata values within the
        //         selection
        ...
        for (int i_kernel = 0; i_kernel < kw; ++i_kernel) {
          for (int j_kernel = 0; j_kernel < kw; ++j_kernel) {
            if (mask2[i_kernel][j_kernel] == 1) {
              ...
              mi = 0;
              ni = 0;
              if (phi_kernel[i_kernel][j_kernel] != 0 &&
                  phi_kernel[i_kernel][j_kernel] != NoDataValue &&
                  pheta_kernel[i_kernel][j_kernel] != NoDataValue) {
                li = sin(pheta_kernel[i_kernel][j_kernel]) *
                     cos(phi_kernel[i_kernel][j_kernel]);
                mi = sin(pheta_kernel[i_kernel][j_kernel]) *
                     sin(phi_kernel[i_kernel][j_kernel]);
  ...
  vector<LSDRaster> raster_output = output_rasters_temp;
  if (raster_selection[0] == 1) {
    LSDRaster s1(NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
                 s1_raster, GeoReferencingStrings);
    raster_output[0] = s1;
  }
  if (raster_selection[1] == 1) {
    LSDRaster s2(NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
                 s2_raster, GeoReferencingStrings);
    raster_output[1] = s2;
  }
  if (raster_selection[2] == 1) {
    LSDRaster s3(NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
                 s3_raster, GeoReferencingStrings);
    raster_output[2] = s3;
  }
  return raster_output;
}
...
// then calcualte slope, curvature, aspect, a classification for finding saddles
// and peaks and other metrics
//
// The coefficient matrices are overwritten during the running of this member
// function
//
// DTM
//
// Updated 15/07/2013 to use a circular mask for surface fitting. DTM
// Updated 24/07/2013 to check window_radius size and correct values below data
// resolution. SWDG
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void LSDRaster::calculate_polyfit_coefficient_matrices(
    float window_radius, Array2D<float> &a, Array2D<float> &b,
    Array2D<float> &c, Array2D<float> &d, Array2D<float> &e,
    Array2D<float> &f) {
  ...
  if (window_radius < DataResolution) {
    cout << "Supplied window radius: " << window_radius
         << " is less than the data resolution: " << DataResolution
         << ".\nWindow radius has been set to data resolution." << endl;
    ...
    for (int j = 0; j < NCols; ++j) {
      ...
      // Avoid nodata values
        ...
        if (ndv_present == 0) // test for nodata values within the selection
        {
          ...
          // Solve matrix equations using LU decomposition using the TNT JAMA
          // package: A.coefs = b, where coefs is the coefficients vector.
          LU<float> sol_A(A); // Create LU object
          coeffs = sol_A.solve(bb);
...
LSDRaster LSDRaster::calculate_polyfit_elevation(Array2D<float> &f) {
  ...
  LSDRaster elevation_raster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                             NoDataValue, elevation_data,
                             GeoReferencingStrings);
  ...
  for (int row = 0; row < NRows; row++) {
    for (int col = 0; col < NCols; col++) {
      if (d[row][col] != NoDataValue) {
        slope_data[row][col] =
            sqrt(d[row][col] * d[row][col] + e[row][col] * e[row][col]);
      }
    }
  }
    ...
    for (int col = 0; col < NCols; col++) {
      if (d[row][col] != NoDataValue) {
        if (d[row][col] == 0 || e[row][col] == 0) {
          ...
          aspect_data[row][col] = 180 -
                                  57.29578 * atan(e[row][col] / d[row][col]) +
...
LSDRaster LSDRaster::calculate_polyfit_planform_curvature(Array2D<float> &a,
  ...
  LSDRaster planform_curvature_raster(NRows, NCols, XMinimum, YMinimum,
                                      DataResolution, NoDataValue,
                                      pl_curvature_data, GeoReferencingStrings);
...
LSDRaster LSDRaster::calculate_polyfit_profile_curvature(Array2D<float> &a,
    ...
    for (int col = 0; col < NCols; col++) {
      if (a[row][col] != NoDataValue) {
        ...
        if (qqq > 0) {
          denom = (p * sqrt(qqq));
          if (denom != 0) {
            profile_curvature_data[row][col] =
                (fxx * fx * fx + 2 * fxy * fx * fy + fyy * fy * fy) / denom;
          } else {
  ...
  LSDRaster profile_curvature_raster(
      NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
      profile_curvature_data, GeoReferencingStrings);
...
LSDRaster LSDRaster::calculate_polyfit_tangential_curvature(Array2D<float> &a,
    ...
    for (int col = 0; col < NCols; col++) {
      ...
      if (a[row][col] != NoDataValue) {
        ...
        if (q > 0) {
          denom = (p * sqrt(q));
          if (denom != 0) {
            ta_curvature_data[row][col] =
                (fxx * fy * fy - 2 * fxy * fx * fy + fyy * fx * fx) / denom;
          } else {
  ...
  LSDRaster tangential_curvature_raster(
      NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
      ta_curvature_data, GeoReferencingStrings);
...
LSDIndexRaster LSDRaster::calculate_polyfit_classification(Array2D<float> &a,
  ...
  for (int row = 0; row < NRows; row++) {
    for (int col = 0; col < NCols; col++) {
      if (d[row][col] != NoDataValue) {
        ...
        if (slope < 0.1) // Threshold for assessing whether point is close to a
                         // stationary point
        {
          d2z_dx2 = 2 * a[row][col];
          d2z_dy2 = 2 * b[row][col];
          d2z_dxdy = c[row][col];
          if (d2z_dx2 < 0 && d2z_dy2 < 0 &&
              d2z_dxdy * d2z_dxdy < d2z_dx2 * d2z_dy2) // Conditions for peak
          {
            classification[row][col] = 1;
          } else if (d2z_dx2 > 0 && d2z_dy2 > 0 &&
                     d2z_dxdy * d2z_dxdy <
                         d2z_dx2 * d2z_dy2) // Conditions for a depression
          {
            classification[row][col] = 2;
          } else if (d2z_dx2 * d2z_dy2 < 0 ||
                     d2z_dxdy * d2z_dxdy >
                         d2z_dx2 * d2z_dy2) // Conditions for a saddle
          {
  ...
  LSDIndexRaster sp_class_raster(NRows, NCols, XMinimum, YMinimum,
                                 DataResolution, intNoDataValue, classification,
                                 GeoReferencingStrings);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// this function takes the polyfit functions and requires a window radius and a
// vector telling the function which rasters to print to file. The function is
// data efficient since one does not need to recalucalte the polyfit coefficeint
// matrices it also takes a string which is the prename of the data files the
// file codes in the vector are: 0 slope 1 aspect 2 curvature 3 planform
// curvature 4 profile curvature 5 tangential curvature 6 classification SMM
// 18-Dec-2012
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void LSDRaster::calculate_and_print_polyfit_rasters(float window_radius,
  ...
  if (n_vec_entries != 7) {
    cout << endl
         << "LSDRaster.calcualte_and_print_polyfit_rasters error" << endl;
    cout << "You have the wrong number of entries in the file code vector; "
            "taking no action!!!"
         << endl
         << endl;
  } else {
    ...
    if (file_code[3] == 1) {
      LSDRaster PolyPlCurv =
          calculate_polyfit_planform_curvature(a, b, c, d, e);
    ...
    if (file_code[5] == 1) {
      LSDRaster PolyTaCurv =
          calculate_polyfit_tangential_curvature(a, b, c, d, e);
      string CTa_name = "_ptacurv_";
      CTa_name = file_prefix + CTa_name + window_size_str;
      PolyTaCurv.write_raster(CTa_name, DEM_flt_extension);
    }
    if (file_code[6] == 1) {
      LSDIndexRaster PolyClass =
          calculate_polyfit_classification(a, b, c, d, e);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// this function combines the polyfit functions and the roughness function in
// one package that is data efficient becasue it only requires one calcualtion
// of the polyfit matrices. it takes the window radius of the polyfit and the
// window of the roughness calcualtion the file codes in the vector are: 0 slope
// 1 aspect
// 2 curvature
...
// 8 roughness s2
// 9 roughness s3
// SMM 19-12-2012 -> adapted by DTM 15/07/2013 since it really makes sense just
// to kick out s1,s2,s3 rather than derivatives for options 7-9
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void LSDRaster::calculate_and_print_polyfit_and_roughness_rasters(
    float window_radius, float roughness_radius, string file_prefix,
    vector<int> file_code) {
  // set up polyfit arrays
  Array2D<float> a;
  Array2D<float> b;
  ...
  if (n_vec_entries != 10) {
    ...
    string roughness_size_str =
        roughness_number_str + p_str + remainderroughness_str;
    roughness_size_str = polystring + window_size_str + underscore +
                         roughstring + roughness_size_str;
    ...
    if (file_code[7] == 1 || file_code[8] == 1 || file_code[9] == 1) {
      ...
      calculate_orientation_matrix_eigenvalues(roughness_radius, l, m, n, s1,
                                               s2, s3);

      if (file_code[7] == 1) {
        LSDRaster s1_raster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                            NoDataValue, s1, GeoReferencingStrings);
        string s1_name = "_s1_";
        s1_name = file_prefix + s1_name + roughness_size_str;
        s1_raster.write_raster(s1_name, DEM_flt_extension);
      }
      if (file_code[8] == 1) {
        LSDRaster s2_raster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                            NoDataValue, s2, GeoReferencingStrings);
        string s2_name = "_s2_";
        s2_name = file_prefix + s2_name + roughness_size_str;
        s2_raster.write_raster(s2_name, DEM_flt_extension);
      }
      if (file_code[9] == 1) {
        LSDRaster s3_raster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                            NoDataValue, s3, GeoReferencingStrings);
        string s3_name = "_s3_";
...
LSDRaster LSDRaster::get_hilltop_curvature(LSDRaster &curvature,
  ...
  LSDRaster hilltop_curvature_raster(NRows, NCols, XMinimum, YMinimum,
                                     DataResolution, NoDataValue,
                                     hilltop_curvature, GeoReferencingStrings);
...
// This function removes positive hilltop curvature values from the hilltop
// curvature raster (to remove noise)
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDRaster
LSDRaster::remove_positive_hilltop_curvature(LSDRaster &hilltop_curvature) {
  ...
  LSDRaster CHT(NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
                CHT_array, GeoReferencingStrings);
...
// This function gets the percentage of pixels that are bedrock from the hilltop
// curvature raster FJC 01/04/15
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
float LSDRaster::get_percentage_bedrock_ridgetops(LSDRaster &roughness,
  ...
  cout << "There are " << bedrock_pixels << " bedrock pixels, out of "
       << total_pixels << " total" << endl;
...
LSDRaster LSDRaster::calculate_slope_angles() {
  cout << "I am calculating the slope angles; make sure you are using a slope "
          "raster!"
       << endl;
  ...
  LSDRaster Slope_angle(NRows, NCols, XMinimum, YMinimum, DataResolution,
                        NoDataValue, slope_angle, GeoReferencingStrings);
  return Slope_angle;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDRaster LSDRaster::calculate_water_supply_rate_for_saturation(
    float soil_thick, float K, LSDRaster &ContributingArea,
    ...
    for (int col = 0; col < NCols; col++) {
      if (RasterData[row][col] != NoDataValue) {
        if (ContributingArea.get_data_element(row, col) == 0) {
          ...
          W_for_sat[row][col] =
              (soil_thick * K * sin(SA) * cos(SA) * DataResolution) /
  ...
  LSDRaster w_for_sat(NRows, NCols, XMinimum, YMinimum, DataResolution,
                      NoDataValue, W_for_sat, GeoReferencingStrings);
  return w_for_sat;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
// This function calculates the water supply rate that causes overland flow
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDRaster LSDRaster::calculate_factor_of_safety_at_saturation(
    float C_r, float rho_s, float soil_thick, float tan_phi,
    LSDRaster &SlopeAngle) {
  ...
  if (rho_s < 700) {
    cout << "Calculating factor of Safety. You soil density is really low"
         << endl
         << "Perhaps you entered it in g/cm^3? It should be in kg/m^3" << endl;
    if (rho_s < 5) {
      cout << "Assuming density was given in g/cm^3 so muliplying by 1000"
           << endl;
      rho_s = rho_s * 1000;
    }
  }
    ...
    for (int col = 0; col < NCols; col++) {
      if (RasterData[row][col] != NoDataValue) {
        ...
        if (sin_SA == 0) {
          ...
          FS_for_sat[row][col] =
              C_r / (g * rho_s * soil_thick * cos_SA * sin_SA) +
  ...
  LSDRaster fs_for_sat(NRows, NCols, XMinimum, YMinimum, DataResolution,
                       NoDataValue, FS_for_sat, GeoReferencingStrings);
  return fs_for_sat;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
LSDRaster LSDRaster::calculate_REI(Array2D<float> &a_plane,
  ...
  for (int row = 0; row < NRows; row++) {
    for (int col = 0; col < NCols; col++) {
      if (a_plane[row][col] != NoDataValue) {
        SlopeOfPlane = sqrt(a_plane[row][col] * a_plane[row][col] +
                            b_plane[row][col] * b_plane[row][col]);
  ...
  LSDRaster REI_raster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                       NoDataValue, REI_data, GeoReferencingStrings);
  return REI_raster;
}
...
void LSDRaster::calculate_polyfit_directional_cosines(Array2D<float> &d,
  ...
  for (int row = 0; row < NRows; row++) {
    for (int col = 0; col < NCols; col++) {
      if (d[row][col] != NoDataValue) {
        pheta =
            atan(sqrt(d[row][col] * d[row][col] + e[row][col] * e[row][col]));
        ...
        if (pheta != 0) { // l and m are undefined if pheta = 0 because phi is
                          // undefined for a vertical surface normal
          l[row][col] = sin(pheta) * cos(phi);
          m[row][col] = sin(pheta) * sin(phi);
        }

        // Can define n for vertical surface normal, since pheta is always
        // defined
        n[row][col] = cos(pheta);
      }
    }
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void LSDRaster::calculate_orientation_matrix_eigenvalues(
    float window_radius, Array2D<float> &l, Array2D<float> &m,
    Array2D<float> &n, Array2D<float> &s1, Array2D<float> &s2,
    Array2D<float> &s3) {
  ...
  // kernel
  int kr =
      int(ceil(window_radius /
               DataResolution)); // Set radius of kernel => suggest = 1 cell
  int kw = 2 * kr + 1;           // width of kernel
  float li, mi, ni;
  ...
  for (int i = 0; i < kw; ++i) {
    for (int j = 0; j < kw; ++j) {
      ...
      radial_dist =
          sqrt(y_kernel * y_kernel +
               x_kernel * x_kernel); // distance from centre to this point.
      if (floor(radial_dist) <= window_radius) {
        mask[i][j] = 1;
      }
    }
  ...
  for (int row = 0; row < NRows; ++row) {
    for (int col = 0; col < NCols; ++col) {
      // Avoid edges
      if (row - kr < 0 || row + kr + 1 > NRows || col - kr < 0 ||
          col + kr + 1 > NCols) {
        ...
        if (l[row][col] != NoDataValue) {
          // Construct orientation matrix and solve to retrieve eigenvalues for
          // data window Build orientation matrix
...

{
  ...
  if (n_vec_entries != 3) {
    ...
    Array2D<float> s3;
    calculate_polyfit_directional_cosines(d, e, l, m, n);
    calculate_orientation_matrix_eigenvalues(roughness_radius, l, m, n, s1, s2,
                                             s3);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDRaster
LSDRaster::mask_to_nodata_using_threshold(float threshold,
  ...
  for (int row = 0; row < NRows; row++) {
    for (int col = 0; col < NCols; col++) {
      ...
      if (NewArray[row][col] != NoDataValue) {
        ...
        } else // this logic is for if you are changing to nodata if above
               // threshold
        {
          if (NewArray[row][col] >= threshold) {
            NewArray[row][col] = NoDataValue;
  ...
  LSDRaster NDR(NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
                NewArray, GeoReferencingStrings);
  return NDR;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDRaster LSDRaster::mask_to_nodata_using_threshold_using_other_raster(
    float threshold, bool belowthresholdisnodata, LSDRaster &MaskingRaster) {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDIndexRaster
LSDRaster::mask_to_indexraster_using_threshold(float threshold,
                                               bool belowthresholdisnodata) {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDRaster
LSDRaster::mask_to_nodata_with_mask_raster(LSDIndexRaster &Mask_raster,
                                           int mask_value) {
    ...
    for (int row = 0; row < NRows; row++) {
      for (int col = 0; col < NCols; col++) {
        ...
        if (Mask_raster.get_data_element(row, col) == IR_NDV ||
          ...
          // cout << "r: " << row << " c: " << col << " mask: "
          // <<Mask_raster.get_data_element(row,col) << endl;
  ...
  LSDRaster NDR(NRows, NCols, XMinimum, YMinimum, DataResolution, NoDataValue,
                new_data_raster, GeoReferencingStrings);
  return NDR;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//------------------------------------------------------------------------------
//
// This function fills pits/sinks in a DEM by incrementing elevations for cells
// with no downslope neighbour. The process is repeated adnausium until no cells
// require incrementing.
//
...
// of his book:
//
// Pelletier,J.D.,'Quantitative Modelling of Landscapes' Cambridge University
// Press
//
//---------------------------------------------------------------------------------
//
...
//---------------------------------------------------------------------------------
//
// v1.1 function incorporated to allow the tool to fill adjacent pixels
// immediately after filling a given pixel, should speed things up.
//
...
LSDRaster LSDRaster::fill() {
  ...
  cout << "Data rows: " << RasterData.dim1() << " cols: " << RasterData.dim2()
       << endl;
  for (int i = 1; i < NRows - 1; i++) {
    cout << "\rRow = " << i + 1 << " / " << NRows << "    ";
    for (int j = 1; j < NCols - 1; j++) {
      ...
      if (FilledRasterData[i][j] == NoDataValue ||
          FilledRasterData[i - 1][j - 1] == NoDataValue ||
          FilledRasterData[i - 1][j] == NoDataValue ||
          FilledRasterData[i - 1][j + 1] == NoDataValue ||
      ...
      // if (RasterData[i][j] == NoDataValue || RasterData[i-1][j-1] ==
      // NoDataValue
      //          || RasterData[i-1][j] == NoDataValue || RasterData[i-1][j+1]
      //          == NoDataValue
      //          || RasterData[i][j+1] == NoDataValue || RasterData[i+1][j+1]
      //          == NoDataValue
      //          || RasterData[i+1][j] == NoDataValue || RasterData[i+1][j-1]
      //          == NoDataValue
...
void LSDRaster::fill_iterator(Array2D<float> &fill_data, int i, int j) {
  ...
  } else if (fill_data[a][b] == NoDataValue ||
             fill_data[a - 1][b - 1] == NoDataValue ||
             fill_data[a - 1][b] == NoDataValue ||
             fill_data[a - 1][b + 1] == NoDataValue ||
             fill_data[a][b + 1] == NoDataValue ||
             fill_data[a + 1][b + 1] == NoDataValue ||
             fill_data[a + 1][b] == NoDataValue ||
             fill_data[a + 1][b - 1] == NoDataValue ||
             fill_data[a][b - 1] == NoDataValue) {
...
//  New fill function
//
//  This function fills pits/sinks in a DEM by checking for pits from lowest to
//  highest elevation, starting at the DEM boundary (raster edge or adjacent to
//  NDVs). Utilises a priority queue to progressively populate the stack and pop
//  out the the lowest value before checking that the neighbouring cells that
//  are yet to be visited must be higher in a hydrologically correct DEM. This
//  method is substantially faster on datasets with pits consisting of multiple
//  cells since each cell only needs to be visited once.
//
//  Input argument required -> MinSlope the minimum slope between two Nodes once
//  filled If set to zero will create flats.
//
...
//(I've coded this but not yet added to LSDRaster, it's only faster than
//presorting when applied to pretty large datasets).
...
LSDRaster LSDRaster::fill(float &MinSlope) {
  ...
  // Declare a temporary FillNode structure which we populate before adding to
  // the PQ Declare a central node or node of interest
    ...
    for (int j = 0; j < NCols; ++j) {
      if (FilledZeta[i][j] != NoDataValue) {
        ...
        if (i == 0 || j == 0 || i == NRows - 1 || j == NCols - 1 ||
            FilledZeta[i - 1][j - 1] == NoDataValue ||
            FilledZeta[i - 1][j] == NoDataValue ||
            FilledZeta[i - 1][j + 1] == NoDataValue ||
            FilledZeta[i][j - 1] == NoDataValue ||
            FilledZeta[i][j + 1] == NoDataValue ||
            FilledZeta[i + 1][j - 1] == NoDataValue ||
            FilledZeta[i + 1][j] == NoDataValue ||
            FilledZeta[i + 1][j + 1] == NoDataValue) {
  ...
  while (!PriorityQueue.empty()) {
    ...
    for (int Neighbour = 0; Neighbour < 8; ++Neighbour) {
      // If the neighbour has data and is not already in the priority queue and
      // has not been processed
      if (row_kernal[Neighbour] == NoDataValue ||
          col_kernal[Neighbour] == NoDataValue ||
          FillIndex[row_kernal[Neighbour]][col_kernal[Neighbour]] == 1 ||
          FillIndex[row_kernal[Neighbour]][col_kernal[Neighbour]] == 2 ||
          FillIndex[row_kernal[Neighbour]][col_kernal[Neighbour]] ==
              NoDataValue) {
      } else {
        // check if neighbour is equal/lower and therefore needs filling
        if (FilledZeta[row_kernal[Neighbour]][col_kernal[Neighbour]] <=
            CentreFillNode.Zeta) {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Main function for generating a D-infinity flow area raster after Tarboton
// (1997). Calls the recurisve D_infAccum function to get flow area for each
// pixel. Returns flow area in pixels.
//
...
LSDRaster LSDRaster::D_inf_FlowArea(Array2D<float> FlowDir_array) {
  ...
  Array2D<float> CountGrid(
      NRows, NCols, NoDataValue); // array to hold no of inflowing neighbours
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      ...
      if (flowDir != NoDataValue) {
        ...
        for (int c = 0; c < 8;
             ++c) { // loop through the 8 neighbours of the target cell
...
// required in the original Tarboton (1997) implementation. For more detail on
// the recursive algorithm following channels see Mark (1998) "Network Models in
// Geomorphology".
//
// Code is ported and optimised from a Java implementation of the algorithm
// supplied under the GNU GPL licence through WhiteBox GAT:
...
void LSDRaster::D_infAccum(int i, int j, Array2D<float> CountGrid,
  ...
  float FD_High_361[] = {
      45,  90,  135, 180,
      225, 270, 315, 361}; // this array ends with 361 to catch angles up to 360
  ...
  // indexes to store the coordinates of the neighbours where flow is to be
  // routed
  int a1 = 0;
  int b1 = 0;
  int a2 = 0;
  ...
  if (flowDir >= 0) { // avoids flagged pits
    ...
    if (proportion1 > 0 && Flowarea_Raster[a1][b1] != NoDataValue) {
      Flowarea_Raster[a1][b1] =
          Flowarea_Raster[a1][b1] + flowAccumVal * proportion1;
      CountGrid[a1][b1] = CountGrid[a1][b1] - 1;
      if (CountGrid[a1][b1] == 0) {
        D_infAccum(a1, b1, CountGrid, Flowarea_Raster,
                   FlowDir); // recursive call
      }
    }
    if (proportion2 > 0 && Flowarea_Raster[a2][b2] != NoDataValue) {
      Flowarea_Raster[a2][b2] =
          Flowarea_Raster[a2][b2] + flowAccumVal * proportion2;
      CountGrid[a2][b2] = CountGrid[a2][b2] - 1;
      if (CountGrid[a2][b2] == 0) {
        D_infAccum(a2, b2, CountGrid, Flowarea_Raster,
                   FlowDir); // recursive call
      }
    }
  }
...
//
// Algorithm takes a filled DEM and for each cell calculates the steepest
// descent based on 8 triangular facets. Flow direction is assigned as an angle
// from 0-360 degrees with -1 used to flag unresolved areas such as pits.
//
// Code is ported and optimised from a Java implementation of the algorithm
// supplied under the GNU GPL licence through WhiteBox GAT:
// http://www.uoguelph.ca/~hydrogeo/Whitebox/ and provides identical results
...
Array2D<float> LSDRaster::D_inf_FlowDir() {
  ...
  float flowDir =
      0; // temp variable to hold flowdirections while looping thru the 8 facets
  ...
  for (int i = 1; i < NRows - 1; ++i) {
    for (int j = 1; j < NCols - 1; ++j) {
      ...
      if (e0 == NoDataValue) {
        FlowDir_Array[i][j] =
            NoDataValue; // if there is no elevation data we cant have a flowdir
      } else {
        maxSlope = -9999999; // set to a low value that != NDV so any slope will
                             // be bigger than it
        ...
        for (int a = 0; a < 8; ++a) { // loop through the 8 facets
          ...
          if (e1 != NoDataValue &&
              e2 != NoDataValue) { // avoid facets lyng in no data
            if (e0 > e1 && e0 > e2) {
              ...
              if (r < 0 || r > atan(1)) {
                if (r < 0) {
                  ...
                  r = atan(1);
                  s = (e0 - e2) /
                      (DataResolution * sqrt(2)); // diagonal cell length
                }
              }
        ...
        if (maxSlope <= 0) {
          FlowDir_Array[i][j] =
              -1; // unresolved - Tarboton uses D8 to fill these pits - we have
                  // a better fill algorithm
        } else {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Wrapper Function to create a D-infinity flow area raster with one function
// call.
//
// SWDG - 26/7/13
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Wrapper Function to create a D-infinity flow area raster, in spatial units,
...
LSDRaster LSDRaster::D_inf_units() {
  ...
  LSDRaster Dinf_area_units_raster =
      Dinf_area.LSDRasterTemplate(Dinf_area_units);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// Function to multiply D_inf flow accumulation data to drainage area by
// multiplying through by the data resolution. (This could be a duplicate
// function but I don't have time to check!).
//
// MDH - 27/8/14
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=--=-=-=-=-=--=-
...
LSDRaster LSDRaster::D_inf_ConvertFlowToArea() {
  ...
  return LSDRaster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                   NoDataValue, Dinf_area_units, GeoReferencingStrings);
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// Function to trace upstream from a pour point and extract watershed associated
// with D_inf flow routing
//
// Uses a priority queue ordered by elevation to collect every pixel that drains
// to a particular pour point and returns the resultant basin as an
// LSDIndexRaster mask
//
// MDH - 26/8/14
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=--=-=-=-=-=--=-
LSDIndexRaster LSDRaster::D_inf_watershed(LSDRaster D_inf_FlowDir, int PourRow,
                                          int PourCol) {
  // Declare the priority Queue with greater than comparison
  priority_queue<FillNode, vector<FillNode>, greater<FillNode>> PriorityQueue;
  // Declare a temporary FillNode structure which we populate before adding to
  ...
  while (!PriorityQueue.empty()) {
    ...
    for (int c = 0; c < 8; ++c) {
      // handle edges here
      if ((row + dY[c] > NRows - 1) || (col + dX[c] > NCols - 1) ||
          (row + dY[c] < 0) || (col + dY[c] < 0))
        continue;
  ...
  LSDIndexRaster Watershed(NRows, NCols, XMinimum, YMinimum, DataResolution,
                           NoDataValue, WatershedArray, GeoReferencingStrings);
  return Watershed;
}
...
LSDRaster LSDRaster::calculate_topographic_index(LSDRaster &AccumulationArea,
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (RasterData[i][j] != NoDataValue)
        TopographicIndexArray[i][j] =
            log(AccumulationArea.get_data_element(i, j) /
                Slope.get_data_element(i, j));
    }
  }
  LSDRaster TopographicIndex(NRows, NCols, XMinimum, YMinimum, DataResolution,
                             NoDataValue, TopographicIndexArray,
                             GeoReferencingStrings);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// Generate data in three text files to create a boomerang plot as in Roering et
// al [2007]. Added spline curves and return of a pair of LH values - 1/11/13
// SWDG. SWDG 27/8/13
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
pair<float, float> LSDRaster::Boomerang(LSDRaster &Slope, LSDRaster &Dinf,
  ...
  log_bin_data(area, slope, log_bin_width, Mean_x_out, Mean_y_out,
               Midpoints_out, STDDev_x_out, STDDev_y_out, STDErr_x_out,
               STDErr_y_out, number_observations, NoDataValue);

  // remove empty bins
  RemoveSmallBins(Mean_x_out, Mean_y_out, Midpoints_out, STDDev_x_out,
                  STDDev_y_out, STDErr_x_out, STDErr_y_out, number_observations,
                  bin_threshold);

  // index value of max slope
  int slope_max_index = distance(
      Mean_y_out.begin(), max_element(Mean_y_out.begin(), Mean_y_out.end()));
  ...
  PlotCubicSplines(Mean_x_out, Mean_y_out, SplineResolution, Spline_X,
                   Spline_Y);

  // index value of max spline slope
  int slope_max_index_spline =
      distance(Spline_Y.begin(), max_element(Spline_Y.begin(), Spline_Y.end()));

  // hillslope length from spline curve - maybe get a range of values about the
  // maximum?
  ...
  file.open(
      ss_bin.str().c_str()); // needs a null terminated character array, not a
                             // string. See pg 181 of accelerated c++

  for (int q = 0; q < int(Mean_x_out.size()); q++) {
    file << Mean_x_out[q] << " " << Mean_y_out[q] << " " << STDDev_x_out[q]
         << " " << STDDev_y_out[q] << " " << STDErr_x_out[q] << " "
         << STDErr_y_out[q] << endl;
  }
  file.close();
  ...
  SplineFile.open(
      ss_spline.str().c_str()); // needs a null terminated character array, not
                                // a string. See pg 181 of accelerated c++

  for (int q = 0; q < int(Spline_X.size());
       q++) { // fixed bug here where I looped over the wrong vector - SWDG
              // 7/11/13
  ...
  cloud.open(
      ss_cloud.str().c_str()); // needs a null terminated character array, not a
                               // string. See pg 181 of accelerated c++
  ...
  // create a pair containing the two LH values, first is the binned value and
  // second is the spline value
...
vector<LSDRaster> LSDRaster::BasinPuncher(vector<int> basin_ids,
  ...
  vector<LSDRaster> BasinVector; // vector to contain individual basin
                                 // LSDRasters
  ...
  for (int a = 0; a < int(basin_ids.size()); ++a) {
    ...
    if (Flag == true) { // only write the raster if there is data to write
      LSDRaster Basin(NRows, NCols, XMinimum, YMinimum, DataResolution,
                      NoDataValue, BasinDEM, GeoReferencingStrings);
...
LSDRaster LSDRaster::CookieCutRaster(LSDRaster Cutter) {
  ...
  LSDRaster Cookie(NRows, NCols, XMinimum, YMinimum, DataResolution,
                   NoDataValue, cookie, GeoReferencingStrings);
...
// hillslope_length mean_slope hilltop_relief hilltop_aspect E* R* LH_bins
// LH_splines LH_density" SWDG 27/8/13
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDRaster::CollectBasinMetrics(
    LSDIndexRaster &Basins, LSDRaster &Slope, LSDRaster &Elevation,
    LSDRaster &Aspect, LSDRaster &Area, LSDRaster &DrainageDensity,
    LSDRaster &Cht, LSDRaster &HillslopeLength, LSDRaster &MeanSlope,
    LSDRaster &Relief, LSDRaster &MeanAspect, LSDRaster &LH_drainage_density,
  ...
  for (vector<int>::iterator it = basin_index.begin(); it != basin_index.end();
       ++it) {
    ...
    for (int i = 0; i < NRows; ++i) {
      for (int j = 0; j < NCols; ++j) {
        ...
        if (Elevation.get_data_element(i, j) != NoDataValue &&
            basin_ids[i][j] == *it) {
        ...
        if (Area.get_data_element(i, j) != NoDataValue &&
            basin_ids[i][j] == *it) {
        ...
        if (Cht.get_data_element(i, j) != NoDataValue &&
            basin_ids[i][j] == *it) {
          ChtSum += Cht.get_data_element(i, j);
          ++ChtCounter;
        }
        if (HillslopeLength.get_data_element(i, j) != NoDataValue &&
            basin_ids[i][j] == *it) {
          HillslopeLengthSum += HillslopeLength.get_data_element(i, j);
          ++HillslopeLengthCounter;
        }
        if (MeanSlope.get_data_element(i, j) != NoDataValue &&
            basin_ids[i][j] == *it) {
          MeanSlopeSum += MeanSlope.get_data_element(i, j);
          ++MeanSlopeCounter;
        }
        if (Relief.get_data_element(i, j) != NoDataValue &&
            basin_ids[i][j] == *it) {
          ReliefSum += Relief.get_data_element(i, j);
          ++ReliefCounter;
        }
        if (MeanAspect.get_data_element(i, j) != NoDataValue &&
            basin_ids[i][j] == *it) {
          MeanAspectSum += MeanAspect.get_data_element(i, j);
          ++MeanAspectCounter;
        }
        if (LH_drainage_density.get_data_element(i, j) != NoDataValue &&
            basin_ids[i][j] == *it) {
          LHFromDD = LH_drainage_density.get_data_element(
              i, j); // this is already a basin average value
        }
      }
    }
  ...
  file << "basin_id slope elevation aspect area drainage_density "
          "hilltop_curvature hillslope_length mean_slope hilltop_relief "
          "hilltop_aspect E* R* LH_bins LH_splines LH_density"
       << endl;
  ...
  for (int q = 0; q < int(BasinIDVector.size()); q++) {
    ...
    for (int k = 0; k < int(LH_Data.dim1()); ++k) {
      if (LH_Data[k][0] ==
          BasinIDVector[q]) { // if we have LH data for this basin id then write
                              // it to the variables
        LH_bins = LH_Data[k][1];
    ...
    file << BasinIDVector[q] << " " << SlopeVector[q] << " "
         << ElevationVector[q] << " " << AspectVector[q] << " " << AreaVector[q]
         << " " << DrainageDensityVector[q] << " " << ChtVector[q] << " "
         << HillslopeLengthVector[q] << " " << MeanSlopeVector[q] << " "
         << ReliefVector[q] << " " << MeanAspectVector[q] << " "
         << EStarVector[q] << " " << RStarVector[q] << " " << LH_bins << " "
         << LH_splines << " " << LHFromDDVector[q] << endl;
  }
...
void LSDRaster::raster_statistics_by_index(LSDIndexRaster &IndexRaster,
  ...
  // sort our two vectors based on the Basin IDs: has the effect of grouping
  // each basin together in 1D space
...
LSDRaster LSDRaster::RidgeSample(Array2D<float> &Ridges) {
  ...
  LSDRaster Sample(NRows, NCols, XMinimum, YMinimum, DataResolution,
                   NoDataValue, Sample_data, GeoReferencingStrings);
  return Sample;
}
...
LSDRaster LSDRaster::RidgeSmoother(int WindowRadius) {
  ...
  LSDRaster Smooth(NRows, NCols, XMinimum, YMinimum, DataResolution,
                   NoDataValue, Smoothed, GeoReferencingStrings);
  return Smooth;
}
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// Module calculates an average value to each for a given raster from within a
// basin. Returns a Float of the averaged value. SWDG 24/3/2015
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--=-=-=-=-=-=-=-=-=-=-=-=
float LSDRaster::SingleBasinAverager(LSDIndexRaster &Basin) {
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      ...
      if (RasterData[i][j] != NoDataValue &&
          basin_ids[i][j] == single_basin_index) {
        sum += RasterData[i][j];
        ++counter;
      }
    }
...
LSDRaster LSDRaster::BasinArea(LSDIndexRaster Basins) {
  ...
  int q = 0;
  float area_sum = 0;
  map<int, float> Basin_Areas; // structure to hold pairs of area values with a
                               // basin ID as a key
  ...
  // sort our vectors based on the Basin IDs: has the effect of grouping each
  // basin together in 1D space
  //  so if we count each pixel and multiply it by the pixel area we get basin
  //  area
  matlab_int_sort(IDs, IDs_sorted, index_map);
...
//
// Produces a generalised polygon and will not cope well with complex
// geometries.
//
  ...
  for (int i = 1; i < NRows - 1; ++i) {
    for (int j = 1; j < NCols - 1; ++j) {
      ...
      if (basin_ids[i][j] == BasinOfInterest) {
        ...
        if (NDVCount >= 4 &&
            NDVCount < 8) { // increase the first value to get a simpler polygon
          // edge pixel
          I.push_back(i);
          J.push_back(j);
        }
...
LSDRaster LSDRaster::DrainageDensity(LSDIndexRaster &StreamNetwork,
  ...
  float area_sum = 0;
  map<int, float>
      Basin_DD; // structure to hold pairs of DD values with a basin ID as a key
  Array2D<float> Density(NRows, NCols, NoDataValue); // Output raster

  // convert Basin Raster to an Array
  Array2D<int> basin_ids = Basins.get_RasterData();

  // Loop over every pixel and record it's stream length and basin ID in two
  // vectors
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (basin_ids[i][j] != NoDataValue) {
        if (StreamNetwork.get_data_element(i, j) != NoDataValue) {
          if ((FlowDir[i][j] % 2) != 0 &&
              (FlowDir[i][j] != -1)) { // is odd but not -1
  ...
  while (q < int(IDs_sorted.size())) {
    if (start_id == IDs_sorted[q]) {
      ...
      Basin_DD[start_id] =
          (length_sum / area_sum); // record the DD density in the map using the
                                   // Basin ID as a key
      length_sum = 0;
      area_sum = 0;
      start_id = IDs_sorted[q];
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Simple method to convert a drainage density raster into a hillslope length
// raster. Returns a Raster of basin average hillslope lengths. The LH value is
// calculated using LH = 1/2*DD [Tucker et al 2001]. SWDG - 7/11/13
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDRaster
LSDRaster::HillslopeLengthFromDrainageDensity(LSDIndexRaster &StreamNetwork,
  ...
  LSDRaster DrainageDensity =
      LSDRaster::DrainageDensity(StreamNetwork, Basins, FlowDir);
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (DrainageDensity.get_data_element(i, j) != NoDataValue) {
        HillslopeLength[i][j] =
            1 / (2 * DrainageDensity.get_data_element(i, j));
      }
    }
  }

  LSDRaster HillslopeLengthRaster =
      DrainageDensity.LSDRasterTemplate(HillslopeLength);
  return HillslopeLengthRaster;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Function built around original c++ code by Martin Hurst to generate a
// flowarea raster.
//
// Computes the proportion of all downslope flows for each cell in the input
// DEM and routes the flow accordingly. Consequently the dem is sorted and
// indexed using LSDStatsTools.
//
...
// Needs to be able to handle the boundary cond vector and only reflect
// bondaries when needed.
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  for (int q = 0; q < int(flat.size()); ++q) {
    ...
    if (sorted[q] != NoDataValue) {
      ...
      if (i != 0 && j != 0 && i != NRows - 1 && j != NCols - 1) {
        ...
        // Get sum of magnitude of downslope flow, total, and store the
        // magnitude of each of the 8 downslope cells as slope1->8 *Avoids NDVs*
        if (elev_pad[i][j] > elev_pad[i - 1][j - 1] &&
            elev_pad[i - 1][j - 1] != NoDataValue) {
          slope1 = (elev_pad[i][j] - elev_pad[i - 1][j - 1]) * one_ov_root_2;
          total += slope1;
        }
        if (elev_pad[i][j] > elev_pad[i - 1][j] &&
            elev_pad[i - 1][j] != NoDataValue) {
        ...
        if (elev_pad[i][j] > elev_pad[i][j + 1] &&
            elev_pad[i][j + 1] != NoDataValue) {
        ...
        // if no slope is found (total == 0), pass flow to all equal elevation
        // cells]
        if (total == 0) {
        ...
        // divide slope by total to get the proportion of flow directed to each
        // cell and increment the downslope cells. If no downslope flow to a
        // node, 0 is added, so no change is seen.
...
//
// Computes the proportion of all downslope flows for each cell in the input
// DEM, and weights them using the equation from Freeman et al 1991 and routes
...
//
// Can *NOT* handle DEMs containing flats or pits -  must be filled using the
// new LSDRaster fill.
//
// Outputs an LSDRaster
//
  ...
  for (int q = 0; q < int(flat.size()); ++q) {
    ...
    if (sorted[q] != NoDataValue) {
      ...
      if (i != 0 && j != 0 && i != NRows - 1 && j != NCols - 1) {
        ...
        // Get sum of magnitude of downslope flow, total, and store the
        // magnitude of each of the 8 downslope cells as slope1->8 *Avoids NDVs*
        if (RasterData[i][j] > RasterData[i - 1][j - 1] &&
            RasterData[i - 1][j - 1] != NoDataValue) {
          slope1 = pow(
              ((RasterData[i][j] - RasterData[i - 1][j - 1]) * one_ov_root_2),
              p);
          total += slope1;
        }
        if (RasterData[i][j] > RasterData[i - 1][j] &&
            RasterData[i - 1][j] != NoDataValue) {
        ...
        if (RasterData[i][j] > RasterData[i][j + 1] &&
            RasterData[i][j + 1] != NoDataValue) {
...
LSDRaster LSDRaster::FreemanMDFlow_SingleSource(int i_source, int j_source) {
  ...
  LSDRaster FreemanMultiFlowSingleSource(NRows, NCols, XMinimum, YMinimum,
                                         DataResolution, NoDataValue, area,
                                         GeoReferencingStrings);
...
// DTM 27/06/2014
LSDRaster
LSDRaster::FMDChannelsFromChannelHeads(vector<int> &channel_heads_rows,
...
//
// Paper link:
// http://onlinelibrary.wiley.com/doi/10.1002/hyp.3360050106/abstract
//
// Cardinal Weighting = (elevation_drop/total_elevation_drop)*DataResolution/2
// Diagonal Weighting =
//      ((elevation_drop/total_elevation_drop)*(1/root(2)))*
//      DataResolution*0.354
//
// Can *NOT* handle DEMs containing flats or pits -  must be filled using the
// new LSDRaster fill.
//
    ...
    if (sorted[q] != NoDataValue) {
      ...
      if (i != 0 && j != 0 && i != NRows - 1 && j != NCols - 1) {
        ...
        if (int(slopes.size()) >
          ...
          // partition flow following the steepest slope and it's steepest
          // neighbour
...
// proposed by Pelletier (2013).  It creates a contour curvature map and
// identifies channel heads as pixels greater than a user defined contour
// curvature threshold value, set by default at 0.1.  The threshold curvature
// can also be defined as a multiple of the standard deviation of the curvature.
// Before this function is called the DEM must be filtered using the wiener
// filter in the LSDRasterSpectral object in order to remove high frequency
// noise.
//
// Reference: Pelletier (2013) A robust, two-parameter method for the extraction
// of drainage networks from high-resolution digital elevation models (DEMs):
// Evaluation using synthetic and real-world DEMs, Water Resources Research 49:
// 1-15
//
...
// elevation and ii) routing flow from each potential source using Freeman MD
// flow.  Any potential sources that are located on ANY down-slope pathway from
// previously visited source pixels are excluded from the final source map.
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// LSDIndexRaster LSDRaster::calculate_pelletier_channel_heads(float
// tan_curv_threshold, Array2D<float>& tan_curv_array)
// {
...
//   // Get all the locations where the tan curvature is greater than the user
//   defined threshold for (int row = 0; row < NRows; row++)
//   {
//     for(int col = 0; col < NCols; col++)
//     {
...
//   }
//   //LSDRaster
//   AboveThreshold(NRows,NCols,XMinimum,YMinimum,DataResolution,NoDataValue,curv_array);
...
// //   else
// //   {
// //     cout << "Found " << n_possible_sources << " possible source pixels" <<
// endl;
// //   }
//   vector<size_t> index_map;
//
//   // sort
//   matlab_float_sort_descending(possible_sources_elev, possible_sources_elev,
//   index_map); matlab_int_reorder(possible_sources_row, index_map,
//   possible_sources_row); matlab_int_reorder(possible_sources_col, index_map,
//   possible_sources_col); LSDIndexRaster SourcesRaster =
//   IdentifyFurthestUpstreamSourcesWithFreemanMDFlow(possible_sources_row,possible_sources_col,tan_curv_array);
//
...
// rasters.
//
// Ideal for use with chi analysis tools which output basin and chi m value
// rasters which can be predominantly no data. As an example, a 253 Mb file can
// be reduced to ~5 Mb with no loss or resampling of data.
//
// Modded 6/11/14 to cope with bil files and to catch cases where some or all of
// the edges cannot be trimmed - SWDG
//
...
LSDRaster LSDRaster::RasterTrimmer() {
  ...
  if (min_row == 0 && min_col == 0 && max_row == (NRows - 1) &&
      max_col == (NCols - 1)) {
    cout << "Raster cannot be trimmed! \nReturning original raster.\n" << endl;

    LSDRaster Output(NRows, NCols, XMinimum, YMinimum, DataResolution,
                     NoDataValue, RasterData, GeoReferencingStrings);
  ...
  // loop over min bounding rectangle and store it in new array of shape
  // new_row_dimension x new_col_dimension
  int TrimmedRow = 0;
  int TrimmedCol = 0;

  // check if the north or west edges cannot be trimmed and stop an out of
  // bounds error
  if (min_row == 0) {
    min_row = 1;
  }
  ...
  // Check if the file is in *.bil format and if it is update the
  // GeoReferencingStrings
...
LSDRaster LSDRaster::RasterTrimmerSpiral() {
  ...
  while (North_node < South_node && East_node > West_node &&
         AllBordersFound == false) {
    // cout << "Nn: " << North_node << " En: " << East_node << " Sn: " <<
    // South_node << " Wn: " << West_node << endl; cout << "Nb: " <<
    // North_border << " Eb: " << East_border << " Sb: " << South_border << "
    // Wb: " << West_border << endl;
  ...
  if (AllBordersFound == false) {
    cout << "Couldn't trim your raster, sorry! Returning original raster"
         << endl;
    LSDRaster Output(NRows, NCols, XMinimum, YMinimum, DataResolution,
                     NoDataValue, RasterData, GeoReferencingStrings);
    return Output;
    ...
    // cout << "Nn: " << North_node << " En: " << East_node << " Sn: " <<
    // South_node << " Wn: " << West_node << endl;
    cout << "Nb: " << North_border << " Eb: " << East_border
         << " Sb: " << South_border << " Wb: " << West_border << endl;
    cout << "If these are very close together it probably means you need to do "
            "a better job of clipping your raster with GDAL."
         << endl;
    ...
    cout << "New dimensions are: rows: " << new_row_dimension
         << " cols: " << new_col_dimension << endl;
    cout << "Min row: " << min_row << " max_row: " << max_row << endl;
    cout << "Min col: " << min_col << " max_col: " << max_col << endl;
    cout << "NRows: " << NRows << " and NCols: " << NCols << endl;

    Array2D<float> TrimmedData(new_row_dimension, new_col_dimension,
                               NoDataValue);
    ...
    cout << "Nrows: " << new_row_dimension << " NCols: " << new_col_dimension
         << endl;
    cout << "XMinimum: " << new_XLL << " YMinimum: " << new_YLL << endl;
    cout << "DataResolution: " << DataResolution << " NDV: " << NoDataValue
         << endl;
    // cout << GeoReferencingStrings[0] << endl;
    // cout << GeoReferencingStrings[1] << endl;

    cout << "Size of data, Rows " << TrimmedData.dim1()
         << " Cols: " << TrimmedData.dim2() << endl;
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//
// Trims the raster array by one pixel around the edge. This was primarily
// written for the LSDCatchmentModel which has a padding of zeros around most of
// the surface data arrays. This removes that padding and reduces the size of
// the array.
//
// It also modifies the NCols and NRows data members, reducing them by 2 to
// account for the new size. Note that it does *not* alter the xll and yll
// values, because these are not modified during the reading in of the original
// DEM and padding with zeroes.
//
// DAV 2016
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void LSDRaster::strip_raster_padding() {
  ...
  // Calculate the dimensions of the trimmed array (n-2, since there is 1px
  // either edge)
  ...
  // Wasn't sure about this simple assignment, thought it might do weird stuff
  // but it seems to be ok?. DAV. Check the TNT Documentation under "Operator=":
          ...
          if (sub_i >= 0 && sub_i < NRows) {
            for (int sub_j = j - kr; sub_j <= j + kr; ++sub_j) {
              ...
              if (sub_j >= 0 && sub_j < NCols) {
                // circular kernel
                float radial_dist =
                    sqrt(pow(((sub_i - i) * DataResolution), 2) +
                         pow(((sub_j - j) * DataResolution), 2));
                if (radial_dist <= window_radius) {
...
LSDRaster LSDRaster::clip_to_smaller_raster(LSDRaster &smaller_raster) {
  ...
  cout << "Small Xmin: " << SR_XMinimum << " YMin: " << SR_YMinimum
       << " Xmax: " << SR_XMaximum << " YMax: " << SR_YMaximum << endl;

  cout << "This data resolution: " << DataResolution
       << " and smaller raster data resolution: " << SR_DataR << endl;

  // find the col of old raster that has the same Xlocations as the XLL of
  // smaller raster the 0.5*DataResolution is in case of rounding errors
  int XLL_col =
      int((SR_XMinimum - XMinimum + 0.5 * DataResolution) / DataResolution);
  int XUL_col =
      int((SR_XMaximum - XMinimum + 0.5 * DataResolution) / DataResolution);
  ...
  // find the row of old raster that has the same Xlocations as the XLL of
  // smaller raster the 0.5*DataResolution is in case of rounding errors
  // Slightly different logic for y because the DEM starts from the top corner
  int YLL_row = NRows - int((SR_YMinimum - YMinimum + 0.5 * DataResolution) /
                            DataResolution);
  int YUL_row = NRows - int((SR_YMaximum - YMinimum + 0.5 * DataResolution) /
                            DataResolution);

  // check on the lower row:
  cout << "Checking lower left row." << endl;
  cout << "integer subtraction: "
       << int((SR_YMinimum - YMinimum + 0.5 * DataResolution) / DataResolution)
       << endl;
  cout << "float subtraction: "
       << (SR_YMinimum - YMinimum + 0.5 * DataResolution) / DataResolution
       << endl;

  // this catches a weird rounding error.
  double int_sub = double(
      int((SR_YMinimum - YMinimum + 0.5 * DataResolution) / DataResolution));
  double flt_sub =
      (SR_YMinimum - YMinimum + 0.5 * DataResolution) / DataResolution;
  ...
  cout << "Small XLLCol: " << XLL_col << " XLR_col: " << XUL_col
       << " YLLrow: " << YLL_row << " YUL_row: " << YUL_row << endl;
  ...
  LSDRaster TrimmedRaster(New_NRows, New_NCols, NewR_XMinimum, NewR_YMinimum,
                          DataResolution, NoDataValue, NewData,
                          GeoReferencingStrings);
...
LSDRaster LSDRaster::clip_to_smaller_raster(LSDIndexRaster &smaller_raster) {
  ...
  cout << "Small XLLCol: " << XLL_col << " XLR_col: " << XUL_col
       << " XLLrow: " << YLL_row << " YUL_row: " << YUL_row << endl;
...
// SWDG 20/1/14
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDRaster::GetVectors(LSDRaster Magnitude, LSDRaster Direction,
                           string output_file, int step) {
  ...
  for (int i = 0; i < NRows; i += step) {
    for (int j = 0; j < NCols; j += step) {
      ...
      if (Magnitude.get_data_element(i, j) != NoDataValue &&
          Direction.get_data_element(i, j) != NoDataValue) {
        stringstream output_line;
        output_line << i << " " << j << " " << Magnitude.get_data_element(i, j)
                    << " " << Direction.get_data_element(i, j);
...
//  Date: 09-03-2006
//
//  Implementation of the Non local filter proposed for A. Buades, B. Coll and
//  J.M. Morel in "A non-local algorithm for image denoising"
//
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDRaster LSDRaster::NonLocalMeansFilter(int WindowRadius, int SimilarityRadius,
                                         int DegreeFiltering, float Sigma) {
  ...
  Array2D<float> PaddedRasterData(NRows + 2 * SimilarityRadius,
                                  NCols + 2 * SimilarityRadius, 0.0);
    ...
    for (int j = 0; j < NCols; ++j) {
      ...
      for (int a = 0; a < (KernelDimension); ++a) {
        for (int b = 0; b < (KernelDimension); ++b) {
          W1[a][b] = PaddedRasterData[i1 - SimilarityRadius + a]
                                     [j1 - SimilarityRadius + b];
        }
      }
      ...
      for (int row = rowmin; row < rowmax + 1; ++row) {
        for (int col = colmin; col < colmax + 1; ++col) {
          ...
          // Otherwise do the calculations
          {
            ...
            for (int a = 0; a < (KernelDimension); ++a) {
              for (int b = 0; b < (KernelDimension); ++b) {
                W2[a][b] = PaddedRasterData[row + a - SimilarityRadius]
                                           [col + b - SimilarityRadius];
                d += Kernel[a][b] * (W1[a][b] - W2[a][b]) *
                     (W1[a][b] - W2[a][b]);
              }
            }
  ...
  LSDRaster NLFilteredDEM(NRows, NCols, XMinimum, YMinimum, DataResolution,
                          NoDataValue, FilteredRasterData,
                          GeoReferencingStrings);
  // LSDRaster
  // NLFilteredNoise(NRows,NCols,XMinimum,YMinimum,DataResolution,NoDataValue,FilteredNoise);
  return NLFilteredDEM; //, NLFilteredNoise;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//  Creates a buffer around an array (of size SimilarityRadius) and gives the
//  new border mirror symmetric values of the original array reflected across
//  the boundary. SimilarityRadius should be the size of the window if filtering
//
    ...
    for (int j = 0; j < PaddedNCols; ++j) {
      ...
      if (i < SimilarityRadius) {
        if (j < SimilarityRadius) {
          PaddedRasterData[i][j] =
              RasterData[SimilarityRadius - i][SimilarityRadius - j];
        } else if (j > (NCols - 1 + SimilarityRadius)) {
          PaddedRasterData[i][j] = RasterData[SimilarityRadius - i]
                                             [j - SimilarityRadius -
                                              2 * (SimilarityRadius - minus_j)];

        } else {
          PaddedRasterData[i][j] =
              RasterData[SimilarityRadius - i][j - SimilarityRadius];
        }
      }
      // south boundary
      else if (i > NRows - 1 + SimilarityRadius) {
        if (j < SimilarityRadius) {
          PaddedRasterData[i][j] = RasterData[i - SimilarityRadius -
                                              2 * (SimilarityRadius - minus_i)]
                                             [SimilarityRadius - j];
      ...
      // west boundary
        ...
        PaddedRasterData[i][j] =
            RasterData[i - SimilarityRadius][SimilarityRadius - j];
      }
      // east boundary
      else if (j > NCols - 1 + SimilarityRadius) {
        PaddedRasterData[i][j] =
            RasterData[i - SimilarityRadius]
                      [j - SimilarityRadius - 2 * (SimilarityRadius - minus_j)];
      }
      // copy rest of RasterData
      else {
        PaddedRasterData[i][j] =
            RasterData[i - SimilarityRadius][j - SimilarityRadius];
      }
    }
  }
}
...
//  kernel array must be predeclared of size SimilarityRadius and consist of
//  zeros: Array2D<float> Kernel(SimilarityRadius,SimilarityRadius,0.0);
//
...
//  Martin Hurst, Feb 2012
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void LSDRaster::MakeGaussianKernel(Array2D<float> &Kernel, float sigma,
                                   int SimilarityRadius) {
  ...
  for (int i = 0; i < 2 * SimilarityRadius + 1; ++i) {
    for (int j = 0; j < 2 * SimilarityRadius + 1; ++j) {
      right_side = -(((j - SimilarityRadius) * (j - SimilarityRadius) +
                      (i - SimilarityRadius) * (i - SimilarityRadius)) /
                     twosigma2);
...
LSDRaster LSDRaster::GaussianFilter(float sigma, int kr) {
  ...
  if (kr == 0)
    kr = int(ceil(
        3 * sigma /
        DataResolution)); // Set radius of kernel (default if not specified)
  int kw = 2 * kr + 1;    // width of kernel
  ...
  for (int i = 0; i < kw; ++i) {
    for (int j = 0; j < kw; ++j) {
      ...
      //       gaussian_kernel_weights[i][j]= (1/(2*M_PI*sigma*sigma)) *
      //       exp(-(x*x+y*y)/(2*sigma*sigma));
      gaussian_kernel_weights[i][j] =
          exp(-(x * x + y * y) / (2 * sigma * sigma));
    }
  }
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      // Avoid edges
      //       if((i-kr < 0) || (i+kr+1 > NRows) || (j-kr < 0) || (j+kr+1 >
      //       NCols) || RasterData[i][j]==NoDataValue)
      if (RasterData[i][j] == NoDataValue) {
        ...
        for (int i_kernel = 0; i_kernel < kw; ++i_kernel) {
          for (int j_kernel = 0; j_kernel < kw; ++j_kernel) {
            if (i - kr + i_kernel >= 0 && i - kr + i_kernel < NRows &&
                j - kr + j_kernel >= 0 && j - kr + j_kernel < NCols) {
              if (RasterData[i - kr + i_kernel][j - kr + j_kernel] !=
                  NoDataValue) // at edges, this takes the one sided gaussian
              {
                summed_weights += gaussian_kernel_weights[i_kernel][j_kernel];
                summed_values +=
                    RasterData[i - kr + i_kernel][j - kr + j_kernel] *
                    gaussian_kernel_weights[i_kernel][j_kernel];
              }
            }
          }
        }
  ...
  LSDRaster FilteredRaster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                           NoDataValue, filtered);
...
LSDRaster LSDRaster::PeronaMalikFilter(int timesteps,
                                       float percentile_for_lambda, float dt) {
  float sigma = 0.05;
  // Calculating lambda
  float lambda = 1;
    ...
    for (int j = 0; j < NCols; ++j) {
      if (i - 1 >= 0 && i + 1 < NRows && j - 1 >= 0 && j + 1 < NCols) {
        if (RasterData[i][j] != NoDataValue &&
            ...
            RasterData[i][j + 1] != NoDataValue &&
            RasterData[i][j - 1] != NoDataValue) {
          slope_NS = (RasterData[i + 1][j] - RasterData[i - 1][j]) /
                     (2 * DataResolution);
          slope_EW = (RasterData[i][j + 1] - RasterData[i][j - 1]) /
                     (2 * DataResolution);
          finite_difference_slopes.push_back(
              sqrt(slope_NS * slope_NS + slope_EW * slope_EW));
        }
      }
    }
  }
  ...
  if (N_slopes > 0) {
    vector<size_t> index_map;
    matlab_float_sort(finite_difference_slopes, finite_difference_slopes,
                      index_map);
  ...
  LSDRaster PM_FilteredTopo(NRows, NCols, XMinimum, YMinimum, DataResolution,
                            NoDataValue, Topography.copy(),
                            GeoReferencingStrings);

  for (int t = 0; t < timesteps; ++t) {
    cout << flush << "\t\t\t Perona-Malik Filter; timestep " << t + 1 << " of "
         << timesteps << "\r";
    // Gaussian filter
    int kr = 2;
    LSDRaster GaussianFilteredTopo =
        PM_FilteredTopo.GaussianFilter(sqrt(sigma), kr);
    ...
    for (int i = 0; i < NRows; ++i) {
      for (int j = 0; j < NCols; ++j) {
        if (i - 1 >= 0 && i + 1 < NRows && j - 1 >= 0 && j + 1 < NCols) {
          if (PM_FilteredTopo.get_data_element(i, j) == NoDataValue ||
              PM_FilteredTopo.get_data_element(i + 1, j) == NoDataValue ||
              PM_FilteredTopo.get_data_element(i - 1, j) == NoDataValue ||
            ...
            slope_n_g = (GaussianFilteredTopo.get_data_element(i - 1, j) -
                         GaussianFilteredTopo.get_data_element(i, j)) /
                        DataResolution;
            slope_s_g = (GaussianFilteredTopo.get_data_element(i + 1, j) -
                         GaussianFilteredTopo.get_data_element(i, j)) /
                        DataResolution;
            slope_e_g = (GaussianFilteredTopo.get_data_element(i, j + 1) -
                         GaussianFilteredTopo.get_data_element(i, j)) /
                        DataResolution;
            slope_w_g = (GaussianFilteredTopo.get_data_element(i, j - 1) -
                         GaussianFilteredTopo.get_data_element(i, j)) /
                        DataResolution;

            p_n =
                1 / (1 + (abs(slope_n_g) / lambda) * (abs(slope_n_g) / lambda));
            p_s =
                1 / (1 + (abs(slope_s_g) / lambda) * (abs(slope_s_g) / lambda));
            p_e =
                1 / (1 + (abs(slope_e_g) / lambda) * (abs(slope_e_g) / lambda));
            p_w =
                1 / (1 + (abs(slope_w_g) / lambda) * (abs(slope_w_g) / lambda));
            //             p_n = exp(-((abs(slope_n_g)/lambda
            //             )*(abs(slope_n_g)/lambda))); p_s =
            //             exp(-((abs(slope_s_g)/lambda
            //             )*(abs(slope_s_g)/lambda))); p_e =
            //             exp(-((abs(slope_e_g)/lambda
            //             )*(abs(slope_e_g)/lambda))); p_w =
            //             exp(-((abs(slope_w_g)/lambda
            //             )*(abs(slope_w_g)/lambda)));

            // slope_n =
            // (PM_FilteredTopo.get_data_element(i-1,j)-PM_FilteredTopo.get_data_element(i,j))/DataResolution;
            // slope_s =
            // (PM_FilteredTopo.get_data_element(i+1,j)-PM_FilteredTopo.get_data_element(i,j))/DataResolution;
            // slope_e =
            // (PM_FilteredTopo.get_data_element(i,j+1)-PM_FilteredTopo.get_data_element(i,j))/DataResolution;
            // slope_w =
            // (PM_FilteredTopo.get_data_element(i,j-1)-PM_FilteredTopo.get_data_element(i,j))/DataResolution;

            dh = dt * (p_n * slope_n_g + p_s * slope_s_g + p_e * slope_e_g +
                       p_w * slope_w_g);
            Topography[i][j] += dh;
          }
        } else
...
//
// Can be used to turn a shapefile of channel heads into a sources raster. Does
// not do any bounds checking or shapefile type checking.
//
// SWDG 13/3/14
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
//
// Works by calculating points along each line spaced by less than the data
// resolution. This has the effect of flagging every raster cell along a
// polyline.
//
// SWDG 21/3/14
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDIndexRaster LSDRaster::PolylineShapefileToRaster(string FileName) {
  ...
  vector<PointData> Polylines =
      LoadPolyline(FileName); // load the coordinates of the shapefile
  ...
  for (int w = 0; w < int(Polylines.size()); ++w) {
    for (int q = 0; q < int(Polylines[w].X.size()) - 1; ++q) {
      ...
      length = sqrt(((Polylines[w].X[q + 1] - Polylines[w].X[q]) *
                     (Polylines[w].X[q + 1] - Polylines[w].X[q])) +
                    ((Polylines[w].Y[q + 1] - Polylines[w].Y[q]) *
                     (Polylines[w].Y[q + 1] - Polylines[w].Y[q])));
      fraction = length / DataResolution;

      for (int n = 0; n <= fraction; ++n) {

        k = n / fraction; // calculate the next step along the line to get the
                          // coordinates of

        Xm = Polylines[w].X[q] +
             (k * (Polylines[w].X[q + 1] -
                   Polylines[w].X[q])); // get the next point along the line
        Ym = Polylines[w].Y[q] +
             (k * (Polylines[w].Y[q + 1] - Polylines[w].Y[q]));

        j = (Xm - XMinimum) /
            DataResolution; // convert the x,y coordinates into raster coords
        i = (YMax - Ym) / DataResolution;

        i_new = trunc(
            i); // convert the floating point raster coordinates into integers
        j_new = ceil(j) - 1;
...
LSDRaster LSDRaster::Resample(float OutputResolution) {
  ...
  if (OutputResolution < DataResolution) {
    cout << "Your resample resolution of " << OutputResolution
         << " is lower that the current data resolution " << DataResolution
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  LSDRaster OutputRaster(NewNRows, NewNCols, XMinimum, YMinimum,
                         OutputResolution, NoDataValue, Resampled,
                         GeoReferencingStrings);
  return OutputRaster;
}
...
// BASIC TOOLS
//---------------------------------------------------------------------------------------
// Basic functions e.g. spatial averaging, which may be useful for loads of
// stuff
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Creates a mask for neighbourhood functions.   Uses a neighbourhood switch to
// select circular (1) vs square window (0).  Default is a square window. DTM
// 19/06/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
Array2D<int> LSDRaster::create_mask(float window_radius,
  ...
  if ((neighbourhood_switch != 1) && (neighbourhood_switch != 0)) {
    ...
    cout << "\t\t incorrect specification of neighbourhood type, so set to "
            "square (default).\n"
         << " Note that correct neighbourhood_switch values are: 0 = square, 1 "
            "= circular"
         << endl;
  }
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// spatial_average
// Calculates a spatial average using a specified moving window.  Uses a
// neighbourhood switch to select circular (1) vs square window (0) DTM
// 19/06/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDRaster
LSDRaster::neighbourhood_statistics_spatial_average(float window_radius,
  ...
  LSDRaster SpatialAverage(NRows, NCols, XMinimum, YMinimum, DataResolution,
                           NoDataValue, SpatialAverageArray,
                           GeoReferencingStrings);
  return SpatialAverage;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// overloaded function to kick out 2 rasters -> local standard deviation &
// average
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
vector<LSDRaster> LSDRaster::neighbourhood_statistics_spatial_average_and_SD(
    float window_radius, int neighbourhood_switch) {
  Array2D<float> SpatialAverageArray(NRows, NCols, NoDataValue);
  Array2D<float> StandardDeviationArray(NRows, NCols, NoDataValue);
  ...
  LSDRaster SpatialAverage(NRows, NCols, XMinimum, YMinimum, DataResolution,
                           NoDataValue, SpatialAverageArray,
                           GeoReferencingStrings);
  LSDRaster SpatialSD(NRows, NCols, XMinimum, YMinimum, DataResolution,
                      NoDataValue, StandardDeviationArray,
                      GeoReferencingStrings);
...
LSDRaster::neighbourhood_statistics_local_relief(float window_radius,
  ...
  for (int i = 0; i < NRows; ++i) {
    ...
    for (int j = 0; j < NCols; ++j) {
      ...
      if ((i - kr < 0) || (i + kr + 1 > NRows) || (j - kr < 0) ||
        ...
        min_in_window =
            100000; // set to a large value to make sure it is reset lower
  ...
  LSDRaster SpatialRelief(NRows, NCols, XMinimum, YMinimum, DataResolution,
                          NoDataValue, SpatialReliefArray,
                          GeoReferencingStrings);
...
// A function that determines the fraction of cells in a circular neighbourhood
// that satisfy a given condition options 0 == 1 != 2 > 3 >= 4 < 5 <=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDRaster LSDRaster::neighbourhood_statistics_fraction_condition(
    float window_radius, int neighbourhood_switch, int condition_switch,
    float test_value) {
...
LSDRaster LSDRaster::border_with_nodata(int border_width,
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (i < border_width || i > NRows - border_width - 1 ||
          j < border_width || j > NCols - border_width - 1) {
        Data[i][j] = NoDataValue;
          ...
          for (int i_kernel = 0; i_kernel < kw; ++i_kernel) {
            for (int j_kernel = 0; j_kernel < kw; ++j_kernel) {
              ...
              if (value == NoDataValue) {
                i_kernel =
                    kw; // as soon as NoDataValue found, skip to next cell
                j_kernel = kw;
                Data[i][j] = NoDataValue;
              }
            }
...
// This function generates an index raster comprising a mask where values of an
// LSDRaster have met a specified condition (e.g. where all values exceed a
// threshold slope)
//
// MDH, 27/8/14
//
...
LSDIndexRaster LSDRaster::Create_Mask(string Condition, float TestValue) {
  ...
  LSDIndexRaster MaskRaster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                            NoDataValue, Mask, GeoReferencingStrings);
  return MaskRaster;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Extract values by mask
// This function generates a masked raster from an LSDIndexRaster masks,
// retaining values where the Mask == 1
//
// MDH, 27/8/14
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
LSDRaster LSDRaster::ExtractByMask(LSDIndexRaster Mask) {
  ...
  LSDRaster MaskedRaster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                         NoDataValue, MaskedArray, GeoReferencingStrings);
  return MaskedRaster;
}
...
void LSDRaster::check_isolated_nodata() {
  for (int row = 0; row < NRows; ++row) {
    for (int col = 0; col < NCols; ++col) {
      ...
      if (RasterData[row][col] == NoDataValue) {
        cout << "LSDRaster::check_isolated_nodata found nodata: row: " << row
             << " col: " << col << " data: " << RasterData[row][col];
      }
    }
  }
  cout << "Done!" << endl;
...
LSDRaster LSDRaster::alternating_direction_nodata_fill(int window_width) {
  ...
  do {
    ...
    cout << "Line 8285, the updated raster has " << begin_ndv
         << " no data nodes " << endl;
            ...
            for (int r = -window_width; r <= window_width; r++) {
              for (int c = -window_width; c <= window_width; c++) {
                ...
                if (window_row > 0 && window_row < NRows - 1 &&
                  ...
                  // cout << "wr: " << window_row << " wc: " << window_col << "
                  // data: " << updated_raster[window_row][window_col] << endl;
  ...
  LSDRaster Hole_filled_Raster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                               int(NoDataValue), this_sweep_data,
                               GeoReferencingStrings);
...
LSDRaster::alternating_direction_nodata_fill_with_trimmer(int window_width) {
  ...
  LSDRaster nodata_filled =
      Trimmed_raster.alternating_direction_nodata_fill(window_width);
...
// No data values must have all neighbours within the window radius != NDV.
// Local mean of all pixels in the window radius. FJC 04/11/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDRaster LSDRaster::nodata_fill_irregular_raster(int window_radius) {
    ...
    for (int col = 0; col < NCols; col++) {
      ...
      if (RasterArray[row][col] == NoDataValue) {
        ...
        for (int i = 1; i <= pixel_radius; i++) {
          ...
          if (counts.at(0) > 0 && counts.at(1) > 0 && counts.at(2) > 0 &&
              counts.at(3) > 0 && counts.at(4) > 0 && counts.at(5) > 0 &&
              counts.at(6) > 0 && counts.at(7) > 0) {
  ...
  LSDRaster FilledDEM(NRows, NCols, XMinimum, YMinimum, DataResolution,
                      NoDataValue, FilledRaster, GeoReferencingStrings);
  return FilledDEM;
}
...
// 09/12/2014
//
// UPDATE - modification of original function to only fill internal no data
// voids This is because the raster trimmer won't work on irregular rasters.
// Holes are only filled if they are surrounded in all directions by pixels with
// a valid elevation value within the given window width.
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDRaster LSDRaster::alternating_direction_nodata_fill_irregular_raster(
    int window_width) {
        ...
        for (int col = 0; col < NCols; ++col) {
          ...
          if (updated_raster[row][col] == NoDataValue) {
            ...
            for (int i = 1; i < window_width; i++) {
              ...
              // check whether surrounding pixels in all directions are equal to
              // 0
              ...
              if (counts.at(0) > 0 && counts.at(1) > 0 && counts.at(2) > 0 &&
                  counts.at(3) > 0 && counts.at(4) > 0 && counts.at(5) > 0 &&
                  counts.at(6) > 0 && counts.at(7) > 0) {
                // cout << "total elev: " << total_elev << " N obs: " << n_obs
                // << endl;
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Isolate channelised portions of the landscape using the method proposed by
// Lashermes et al. (2007) Lashermes, B., E. Foufoula-Georgiou, and W. E.
// Dietrich (2007), Channel network extraction from high resolution topography
// using wavelets, Geophys. Res. Lett., 34, L23S04, doi:10.1029/2007GL031140.
// This function (i) filters the DEM using a Gaussian filter; (ii) calculates
// the curvature and/or aspect; (iii) uses quantile quantile analysis to define
// a curvature threshold for channel initiation. DTM 06/02/2015
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDIndexRaster
LSDRaster::IsolateChannelsLashermesCurvature(float sigma, string q_q_filename) {
  ...
  output_rasters = FilteredTopo.calculate_polyfit_surface_metrics(
      window_radius, raster_selection);
  LSDRaster curvature = output_rasters[3];
  // use q-q plot to isolate the channels
  cout << "\t\t Finding threshold using q-q plot" << endl;
  LSDIndexRaster channels =
      curvature.IsolateChannelsQuantileQuantile(q_q_filename);
  return channels;
}
LSDIndexRaster LSDRaster::IsolateChannelsLashermesAspect(float sigma,
                                                         string q_q_filename) {
  // filter
  LSDRaster FilteredTopo = GaussianFilter(sigma);
  // calculate curvature
  ...
  // aspect is not a continuous function, so need to repeat with aspect origin
  ...
  // calculate variability of slope direction d(pheta) = sqrt(d(pheta)/dy^2 +
  // d(pheta)/dx^2)
  ...
  for (int i = 1; i < NRows - 1; ++i) {
    for (int j = 1; j < NCols - 1; ++j) {
      if (aspect.get_data_element(i, j) != NoDataValue &&
          aspect.get_data_element(i - 1, j) != NoDataValue &&
          aspect.get_data_element(i + 1, j) != NoDataValue &&
          aspect.get_data_element(i, j + 1) != NoDataValue &&
          aspect.get_data_element(i, j - 1) != NoDataValue) {
        d_pheta_dx = (aspect.get_data_element(i, j + 1) -
                      aspect.get_data_element(i, j - 1)) /
                     (2 * DataResolution);
        d_pheta_dy = (aspect.get_data_element(i + 1, j) -
                      aspect.get_data_element(i - 1, j)) /
                     (2 * DataResolution);
        d_pheta1 = sqrt(d_pheta_dx * d_pheta_dx + d_pheta_dy * d_pheta_dy);
        // and for transformed aspects
        d_pheta_dx = (aspect_temp[i][j + 1] - aspect_temp[i][j - 1]) /
                     (2 * DataResolution);
        d_pheta_dy = (aspect_temp[i + 1][j] - aspect_temp[i - 1][j]) /
                     (2 * DataResolution);
  ...
  LSDRaster dpheta(NRows, NCols, XMinimum, YMinimum, DataResolution,
                   NoDataValue, d_pheta);
  // use q-q plot to isolate the channels
  LSDIndexRaster channels =
      dpheta.IsolateChannelsQuantileQuantile(q_q_filename);
  return channels;
}
LSDIndexRaster
LSDRaster::IsolateChannelsLashermesFull(float sigma,
  ...
  LSDIndexRaster Channels1 =
      IsolateChannelsLashermesAspect(sigma, q_q_filename_aspect);
  LSDIndexRaster Channels2 =
      IsolateChannelsLashermesCurvature(sigma, q_q_filename_curvature);
  Array2D<int> binary_array(NRows, NCols, int(NoDataValue));
  for (int i = 1; i < NRows - 1; ++i) {
    for (int j = 1; j < NCols - 1; ++j) {
      if (Channels1.get_data_element(i, j) == 1 &&
          Channels2.get_data_element(i, j) == 1)
        binary_array[i][j] = 1;
      else if (Channels1.get_data_element(i, j) == 0 ||
               Channels2.get_data_element(i, j) == 0)
        binary_array[i][j] = 0;
    }
  }

  LSDIndexRaster channels(NRows, NCols, XMinimum, YMinimum, DataResolution,
                          NoDataValue, binary_array, GeoReferencingStrings);
  return channels;
}
// This version uses curvature and an upstream contributing area threshold
LSDIndexRaster LSDRaster::IsolateChannelsLashermesCurvatureArea(
    float sigma, float area_threshold, string q_q_filename) {
  cout << "\t Isolation of channelised pixels using curvature" << endl;
  // filter
  cout << "\t\t Gaussian filter" << endl;
  LSDRaster FilteredTopo = GaussianFilter(sigma);
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (channels_init.get_data_element(i, j) !=
          channels_init.get_NoDataValue()) {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Isolate channelised portions of the landscape using the method proposed by
// Passalacqua et al. (2010) A geometric framework for channel network
// extraction from lidar: Nonlinear diffusion and geodesic paths, J. Geophys.
// Res., 115(F1), F01002, doi:10.1029/2009JF001254.
//
// This function (i) filters the DEM using a Perona Malik filter; (ii)
// calculates the curvature; (iii) uses quantile quantile analysis to define a
// curvature threshold for channel initiation.
//
// DTM 21/04/2015
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
//=-=-=-=-=
...
LSDIndexRaster LSDRaster::IsolateChannelsQuantileQuantile(string q_q_filename) {
  ...
  quantile_quantile_analysis(values, quantile_values, normal_variates,
                             mn_values, N_points);
  ofstream ofs;
  ...
  for (int i = 0; i < n_values; ++i) {
    ofs << normal_variates[i] << " " << quantile_values[i] << " "
        << mn_values[i] << "\n";
  }
  ofs.close();

  // Find q-q threshold
  cout << "\t finding deviation from Gaussian distribution to define q-q "
          "threshold"
       << endl;
  vector<int> indices;
  int flag = 0;
  ...
  curvature_threshold =
      mean_curvature + normal_variates[threshold_index] * sd_curvature;
  cout
      << "\t Creating channel raster based on curvature threshold (threshold = "
      << curvature_threshold << ")" << endl;
  ...
  cout << "DONE" << endl;
  LSDIndexRaster ChannelMask(NRows, NCols, XMinimum, YMinimum, DataResolution,
                             NoDataValue, binary_raster, GeoReferencingStrings);
  return ChannelMask;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
LSDIndexRaster
LSDRaster::IsolateChannelsQuantileQuantileAdaptive(int half_width) {
  ...
  for (int i = 0; i < NRows; ++i) {
    ...
    for (int j = 0; j < NCols; ++j) {
      if (RasterData[i][j] != NoDataValue) {
        vector<float> values;
        for (int i_kernel = i - half_width; i_kernel < i + half_width;
             ++i_kernel) {
          for (int j_kernel = j - half_width; j_kernel < j + half_width;
               ++j_kernel) {
            if (i_kernel >= 0 && i_kernel < NRows && j_kernel >= 0 &&
                j_kernel < NCols) {
        ...
        // Find q-q threshold
        // cout << "\t finding deviation from Gaussian distribution to define
        // q-q threshold" << endl;
        vector<int> indices;
        int flag = 0;
        float threshold_condition = 0.99;
        ...
        for (int i_val = 0; i_val < n_values; ++i_val) {
          if (normal_variates[i_val] >= 0) {
            if (mn_values[i_val] <
                threshold_condition * quantile_values[i_val]) {
              if (flag == 0) {
                flag = 1;
                //               curvature_threshold = quantile_values[i];
        ...
        // cout << "\t Creating channel raster based on curvature threshold
        // (threshold = " << curvature_threshold << ")" << endl;
      }
    }
  }
  Array2D<int> binary_raster(NRows, NCols, NoDataValue);
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
Array2D<float>
LSDRaster::CalculateAdaptiveCurvatureThresholdSD(int half_width) {
  cout << "Calculating adaptive curvature threshold" << endl;
  Array2D<float> curvature_threshold_array(NRows, NCols, NoDataValue);
  for (int i = 0; i < NRows; ++i) {
    cout << flush << "processing row " << i + 1 << " of " << NRows << "\r";
...
// Each value is placed on its own line, so that it can be read more quickly in
// python etc. SWDG 9/2/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
// Each value is placed on its own line, so that it can be read more quickly in
// python etc. It includes the x and y locations so it can be read by GIS
// software SMM 29/6/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (RasterData[i][j] != NoDataValue) {
        ...
        WriteData << x_loc << "," << y_loc << "," << RasterData[i][j] << ","
...
// Method to flatten an LSDRaster and place the non NDV values in a csv file.
// Each value is placed on its own line, so that it can be read more quickly in
// python etc. It includes only lat long in WGS1984 so it can be read by GIS
// software SMM 12/11/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  ...
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
      if (RasterData[i][j] != NoDataValue) {
        ...
        WriteData << latitude << "," << longitude << "," << RasterData[i][j]
                  << endl;
      }
    }
  }
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Method to convert an LSDRaster hilltop file into a series of contiguous
// hilltop patches.
//
// Connects all contiguous patches of hilltop via a unique ID, allowing hilltop
// patches to be generated in a manner similar to the methodology employed by
// Hurst et al. (2012) and (2013) to spatially average data for E*R*
// calculations. Currently has no method to limit the maximum size of segments.
// Requires an integer minimum_patch_size, the minimum number of pixels required
// for a patch to be created SWDG 5/6/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
LSDIndexRaster LSDRaster::CreateHilltopPatchesNEW(int minimum_patch_size) {
  ...
  LSDIndexRaster HilltopPatches(NRows, NCols, XMinimum, YMinimum,
                                DataResolution, int(NoDataValue), hilltops);
  HilltopPatches =
      HilltopPatches.filter_by_connected_components(minimum_patch_size);
...
LSDIndexRaster LSDRaster::CreateHilltopPatches(int minimum_patch_size) {
  ...
  // cycle over every cell and find connected patches, giving each patch a
  // unique ID
  for (int i = 1; i < NRows - 1; ++i) {
    for (int j = 1; j < NCols - 1; ++j) {

      // if the pixel has data it is a hilltop and if it has not been assigned a
      // patchID we must visit it
      if (RasterData[i][j] != NoDataValue && PatchIDs[i][j] == NoDataValue) {
        ...
        // need this set to true to start the loop, does not matter if the
        // initial cell has no neighbours, this will be caught on the first
        // iteration of the loop
        Neighbours = true;

        // edge checking now in every initial neighbour search.
        // if the index is not changing eg i-1,j do we need to check if j is at
        // an edge?
        ...
        while (Neighbours == true && PatchIDs[a][b] == NoDataValue) {
          ...
          if (RasterData[a - 1][b - 1] != NoDataValue &&
              PatchIDs[a - 1][b - 1] == NoDataValue &&
              (((a - 1) != (NRows - 1) || (a - 1) != 0) ||
               (b - 1) != (NCols - 1) || (b - 1) != 0)) {
            --a;
            --b;
          } else if (RasterData[a][b - 1] != NoDataValue &&
                     PatchIDs[a][b - 1] == NoDataValue &&
                     (((a) != (NRows - 1) || (a) != 0) ||
                      (b - 1) != (NCols - 1) || (b - 1) != 0)) {
            --b;
          } else if (RasterData[a + 1][b - 1] != NoDataValue &&
                     PatchIDs[a + 1][b - 1] == NoDataValue &&
                     (((a + 1) != (NRows - 1) || (i + 1) != 0) ||
                      (b - 1) != (NCols - 1) || (b - 1) != 0)) {
            ++a;
            --b;
          } else if (RasterData[a + 1][b] != NoDataValue &&
                     PatchIDs[a + 1][b] == NoDataValue &&
                     (((a + 1) != (NRows - 1) || (a + 1) != 0) ||
                      (b) != (NCols - 1) || (b) != 0)) {
            ++a;
          } else if (RasterData[a + 1][b + 1] != NoDataValue &&
  ...
  for (int i = 1; i < NRows - 1; ++i) {
    for (int j = 1; j < NCols - 1; ++j) {
      if (PatchIDs[i][j] != NoDataValue) {
        ...
        if (PatchIDs[i - 1][j - 1] != center_value &&
            PatchIDs[i - 1][j - 1] != NoDataValue &&
            (((i - 1) != (NRows - 1) || (i - 1) != 0) ||
             (j - 1) != (NCols - 1) || (j - 1) != 0)) {
          value_to_replace = PatchIDs[i - 1][j - 1];
          clashes = true;
        } else if (PatchIDs[i][j - 1] != center_value &&
                   PatchIDs[i][j - 1] != NoDataValue &&
                   (((i) != (NRows - 1) || (i) != 0) ||
                    (j - 1) != (NCols - 1) || (j - 1) != 0)) {
          value_to_replace = PatchIDs[i][j - 1];
          clashes = true;
        } else if (PatchIDs[i + 1][j - 1] != center_value &&
                   PatchIDs[i + 1][j - 1] != NoDataValue &&
                   (((i + 1) != (NRows - 1) || (i + 1) != 0) ||
                    (j - 1) != (NCols - 1) || (j - 1) != 0)) {
          value_to_replace = PatchIDs[i + 1][j - 1];
          clashes = true;
        } else if (PatchIDs[i + 1][j] != center_value &&
                   PatchIDs[i + 1][j] != NoDataValue &&
                   (((i + 1) != (NRows - 1) || (i + 1) != 0) ||
                    (j) != (NCols - 1) || (j) != 0)) {
        ...
        // if clashes are present, loop over PatchIDs and fix the clash by
        // updataing the IDs of the clashing patches to match.
        if (clashes == true) {
          ...
          // reset initial scan through DEM until it makes it through clean with
          // no clashes
          i = 0;
          j = 0;
        }
  ...
  // Strip out any values that only occur below the minimum_segment_size param
  // value
  cout << "\tRemoving patches that have an area smaller than "
       << minimum_patch_size << " pixels." << endl;
  ...
  // loop over map, get vector of keys where value < user defined limit and
  // store patchIDs to be removed as vector
  ...
  if (!PatchesToRemove.empty()) {
    ...
    // loop over PatchIDs, checking each value for membership in the vector of
    // values to be removed, and if true, set cell value to NDV
    for (int i = 1; i < NRows - 1; ++i) {
      for (int j = 1; j < NCols - 1; ++j) {

        if (PatchIDs[i][j] != NoDataValue) {

          if (find(PatchesToRemove.begin(), PatchesToRemove.end(),
                   PatchIDs[i][j]) != PatchesToRemove.end()) {
  ...
  LSDIndexRaster Patches(NRows, NCols, XMinimum, YMinimum, DataResolution,
                         NoDataValue, PatchIDs, GeoReferencingStrings);
  return Patches;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Simple method to remove any values below a user supplied value from an
// LSDRaster.
//
...
LSDRaster LSDRaster::RemoveBelow(float Value) {
  ...
  LSDRaster Removed(NRows, NCols, XMinimum, YMinimum, DataResolution,
                    NoDataValue, Data, GeoReferencingStrings);
  return Removed;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Simple method to remove any values above a user supplied value from an
// LSDRaster.
//
...
LSDRaster LSDRaster::apply_mask(LSDIndexRaster &mask) {
  ...
  LSDRaster masked_raster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                          NoDataValue, masked_data, GeoReferencingStrings);
...
LSDIndexRaster LSDRaster::ConvertToBinary(int Value, int ndv) {
  ...
  LSDIndexRaster binmask(NRows, NCols, XMinimum, YMinimum, DataResolution,
                         NoDataValue, binary, GeoReferencingStrings);
  return binmask;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Method to merge data from two LSDRasters WITH SAME EXTENT together.  The data
// from the raster specified as an argument will be added (will overwrite the
// original raster if there is a conflict). FJC 30/09/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDRaster LSDRaster::MergeRasters(LSDRaster &RasterToAdd) {
  ...
  LSDRaster NewRaster(NRows, NCols, XMinimum, YMinimum, DataResolution,
                      NoDataValue, NewRasterData, GeoReferencingStrings);
  return NewRaster;
}
...
// Method to merge data from two LSDRasters WITH SAME EXTENT together.  The data
// from the raster specified as an argument will be added (will overwrite the
// original raster if there is a conflict). Overloaded function that overwrites
// the raster rather than creating a new one. FJC 07/04/17
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Function to get potential floodplain patches using a slope and relief
// threshold FJC 20/10/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDIndexRaster
LSDRaster::get_potential_floodplain_patches(LSDRaster &Relief, LSDRaster &Slope,
  ...
  LSDIndexRaster FloodplainRaster(NRows, NCols, XMinimum, YMinimum,
                                  DataResolution, NoDataValue, FloodplainArray,
                                  GeoReferencingStrings);
...
float LSDRaster::get_threshold_for_floodplain(float bin_width,
  ...
  calculate_histogram(raster_vector, bin_width, Midpoints, LLims, ULims, Count,
                      ProbabilityDensity);

  // get peaks from histogram
  vector<int> peak_indices;
  get_peak_indices(ProbabilityDensity, peak_threshold, peak_distance,
                   peak_indices);

  // using second peak as the floodplain: get the midpoint of the bin with the
  // peak
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Function to set the threshold value to use in floodplain extraction using QQ
// plots FJC 16/11/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
float LSDRaster::get_threshold_for_floodplain_QQ(string q_q_filename,
  ...
  vector<float> quantile_values, normal_variates, mn_values;
  int N_points = 10000; // values.size();
  quantile_quantile_analysis_defined_percentiles(
      raster_vector, quantile_values, normal_variates, mn_values, N_points,
    ...
    if (frac_diff < threshold_condition) {
      ...
      if (flag == 0) {
        ...
        if (count == min_length) {
          ...
          cout << "Quantile value at threshold: " << quantile_values[i]
               << " Normal variate: " << normal_variates[i] << endl;
        } else
          flag = 0;
      }
    }
...
// Get the lengths in spatial units of each part of the channel network, divided
// by strahler order. Returns a string containing the comma separated lengths
// SWDG 17/1/16
string LSDRaster::ChannelLengthByOrder(LSDIndexRaster &StreamNetwork,
  ...
  int q = 0;
  float length_sum = 0;
  map<int, float> Order_Length; // structure to hold pairs of length values with
                                // a basin ID as a key

  // Loop over every pixel and record it's stream length and order in two
  // vectors
  for (int i = 0; i < NRows; ++i) {
    for (int j = 0; j < NCols; ++j) {
  ...
  // sort our two vectors based on the Basin IDs: has the effect of grouping
  // each order together in 1D space
  matlab_int_sort(IDs, IDs_sorted, index_map);
  matlab_float_reorder(Lengths, index_map, SortedLengths);
  ...
  while (q < int(IDs_sorted.size())) {
    if (start_id == IDs_sorted[q]) {
      ...
      Order_Length[start_id] =
          (length_sum); // record the total length the order as a key
      length_sum = 0;
      start_id = IDs_sorted[q];
    }
  }
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
/// Populate a raster with random noise drawn from a gaussian distribution of
/// given mean and minimum values. SWDG 9/6/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
LSDRaster LSDRaster::PoupulateRasterGaussian(float minimum, float mean) {
  ...
  LSDRaster output(NRows, NCols, XMinimum, YMinimum, DataResolution,
                   NoDataValue, Gauss, GeoReferencingStrings);
  return output;
}
...
LSDRaster LSDRaster::PoupulateRasterSingleValue(float value) {
  ...
  LSDRaster output(NRows, NCols, XMinimum, YMinimum, DataResolution,
                   NoDataValue, Single, GeoReferencingStrings);
  return output;
}

//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Write CHT and hilltop gradient data to a *.csv file, coded by UTM coordinates
// as well as lat/long. SWDG 2/11/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void LSDRaster::HilltopsToCSV(LSDRaster &CHT, LSDRaster &CHT_gradient,
                              LSDRaster &gradient, int UTMZone, bool isNorth,
                              int eId, string filename) {
  ...
  // Tests if the sorted vector has NDV at the front or back. We need them at
  // the end. This can fail if you have  strange NDV value that is not a low or
  // high number.
  if (sorted[q] == NoDataValue) {
  ...
  while (sorted[q] != NoDataValue) {
    ...
    double UTMn =
        YMinimum + float(NRows - i) * DataResolution - 0.5 * DataResolution;
    ...
    stringstream ss;
    ss << setiosflags(ios::fixed) << setprecision(7) << q << "," << UTMe << ","
       << UTMn << "," << Lat << "," << Long << "," << CHT.get_data_element(i, j)
       << "," << gradient_flag << "," << gradient.get_data_element(i, j);
  ...
  WriteData << "_ID,Easting,Northing,Lat,Long,CHT,gradient_flag,gradient"
            << endl;
...
vector<vector<float>> LSDRaster::Sample_Along_Ridge(LSDRaster &Raster1,
  ...
  for (int q = 0; q < int(Flat.size()); ++q) { // switch to while sorted[q] <=
                                               // ID
  ...
  if (int(dists.size()) < threshold) {
    ...
    cout << "Threshold is larger than the number of pixels in the hilltop "
            "segment"
         << endl;
  }
