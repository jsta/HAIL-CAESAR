...
//------------------------------------------------------------------------------
vector<float> sample_without_replacement(vector<float> population_vector,
                                         int N) {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// This function gets the main peaks from a vector of y data.  User must set a
// y-threshold which the amplitude of the peak must be above, and a distance
// between peaks (smaller peaks very close to a large one will be removed). It
// returns the indices of the input vector which represent the peaks. Ported
// from the Python package PeakUtils https://pypi.python.org/pypi/PeakUtils The
// MIT License (MIT) Copyright (c) 2014 Lucas Hermann Negri

// FJC 10/11/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void get_peak_indices(vector<float> &y_data, float threshold, int distance,
  ...
  for (int i = 0; i < int(diff_rising.size()); i++) {
    if (diff_falling[i] < 0 && diff_rising[i] > 0 &&
        y_data[i] > threshold) // identifies peaks using neighbour differences
    {
      peak_indices_temp.push_back(i);
      peak_values_temp.push_back(y_data[i]);
      // cout << "Potential peak value: " << y_data[i] << " Peak index: " << i
      // << endl;
    }
  }
  ...
  if (int(peak_indices_temp.size()) > 1) {
    ...
    // loop through peaks from highest - lowest removing peaks within min
    // distance
    ...
    for (int i = 0; i < int(check_peaks.size()); i++) {
      if (check_peaks[i] == 1) {
        ...
        // cout << "Removed small peaks, Peak value: " << y_data[i] << " Peak
        // index: " << i << endl;
      }
    }
  } else {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// gets specified percentile, from pre-sorted vector, following same method as
// MS Excel.  Note that the percentile should be expressed as a percentage i.e.
// for median percentile = 50, NOT 0.5! DTM 14/04/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
// sorts data; produces quartile-quantile comparison against standard normal
// variate, returning a sorted subsample of N_points, their corresponding normal
// variate and the reference value from the standard normal distribution DTM
// 28/11/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void quantile_quantile_analysis(vector<float> &data, vector<float> &values,
                                vector<float> &standard_normal_variates,
                                vector<float> &mn_values, int N_points) {
  vector<size_t> index_map;
...
// variate, returning a sorted subsample of N_points, their corresponding normal
// variate and the reference value from the standard normal distribution DTM
// 28/11/2014 Modified by FJC 03/03/16 to get the percentiles for the normally
// distributed model as an argument.
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void quantile_quantile_analysis_defined_percentiles(
    vector<float> &data, vector<float> &values,
    vector<float> &standard_normal_variates, vector<float> &mn_values,
    int N_points, int lower_percentile, int upper_percentile) {
  vector<size_t> index_map;
  vector<float> data_sorted;
  matlab_float_sort(data, data_sorted, index_map);
  float quantile, x;
  ...
  cout << "q_lower_x: " << q_lower_x << " q_lower_y: " << q_lower_y
       << " q_upper_x: " << q_upper_x << " q_upper_y: " << q_upper_y << endl;
...
double erfi(double tau) {
  ...
  //    sum+= front_term*(exp(xloc_old*xloc_old) + 4*exp(xloc_mid*xloc_mid) +
  //    exp(xloc_new*xloc_new) );
  //  }
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Use the Marsaglia polar method to generate random numbers drawn from a normal
// distribution with a given mean and minimum values. Set allowNegative to false
// to stop output values from dropping below zero.
//
// Extreme values can fall below or above the boundaries in < 3 sigma of cases.
//
// Seed for random number will fail post 2038. I will instruct my firstborn to
...
// it returns a vector with the best fit values for m, b, r^2 and the
// durban_watson statistic (which is used to test if the residuals are
// autocorrelated it also replaces the residuals vector with the actual
// residuals from the best fit
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
vector<float> simple_linear_regression(vector<float> &x_data,
                                       vector<float> &y_data,
  ...
  if (n_x != n_y) {
    cout << "WARNING simple_linear_regression x and y vecs no the same size. "
            "Prepare for segmentation!"
         << endl;
  }
  if (n_x == 0) {
    cout << "WARNING simple_linear_regression I haven't got x data! Prepare "
            "for segmentation!"
         << endl;
  }
  ...
  if (solution.dim1() == 0) {
    cout << "WARNING simple_linear_regression Solution data is empty! Prepare "
            "for segmentation!"
         << endl;
    cout << "Dimensions are: n_x: " << n_x
         << " solution matrix: " << solution.dim1() << " " << solution.dim2()
         << endl;
  }
  ...
  for (int i = 0; i < n_rows; i++) {
    ...
    // cout << "RESIDUAL, i: " << i << " pred: " << predicted[i] << " data: " <<
    // y_data[i] << " resid: " << temp_residuals[i] << endl;
  }
...
// DTM 07/10/2014
void least_squares_linear_regression(vector<float> x_data, vector<float> y_data,
                                     float &intercept, float &gradient,
                                     float &R_squared) {
  float SS_xx = 0;
  float SS_yy = 0;
  float SS_xy = 0;
...
// This comes from
// davegiles.blogspot.co.uk/2014/11/orthogonal-regression-first-steps.html NOTE:
// THis is more generally called Total Least Squares
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
vector<float> orthogonal_linear_regression(vector<float> &x_data,
                                           vector<float> &y_data,
                                           float &intercept, float &gradient) {
  ...
  gradient = (SS_yy - SS_xx +
              sqrt((SS_xx - SS_yy) * (SS_xx - SS_yy) + 4 * SS_xy * SS_xy)) /
             2 * SS_xy;
  intercept = y_mean - gradient * x_mean;
...
// SMM 3/12/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
double interp1D_ordered(vector<double> &x, vector<double> &y,
                        double x_interp_loc) {
  ...
  if (n_nodes != y_nodes) {
    cout << "Trying to interpolate but the vectors are not the same size!"
         << endl;
    cout << "Defaulting to y[0]" << endl;
    y_interp = y[0];
  } else if (n_nodes == 1) {
    cout << "Trying to interpolate you have only given me a single datapoint"
         << endl;
    cout << "Defaulting to y[0]" << endl;
    y_interp = y[0];
  } else {
    if (x_interp_loc < x[0]) {
      cout << "Interpolation point is outside bounds (too small) of x vector!"
           << endl;
      cout << "Defaulting to y[0]" << endl;
      y_interp = y[0];
    } else if (x_interp_loc > x[n_nodes - 1]) {
      cout << "Interpolation point is outside bounds (too big) of x vector!"
           << endl;
      ...
      y_interp = y[i - 1] + (y[i] - y[i - 1]) *
                                ((x_interp_loc - x[i - 1]) / (x[i] - x[i - 1]));
    }
  }
  return y_interp;
}
...
vector<double> interp1D_ordered(vector<double> &x, vector<double> &y,
                                vector<double> x_interp_loc) {
...
// vector version of the linear interpolation
vector<float> interp1D_ordered(vector<float> &x, vector<float> &y,
                               vector<float> x_interp_loc) {
  // initiate the vector for holding interpolation
...
// SMM 3/12/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
vector<double> interp1D_spline_ordered(vector<double> &x_data,
...
vector<float> interp1D_unordered(vector<float> x, vector<float> y,
                                 vector<float> &x_interp_loc) {
  // initiate the sorted vectors
  vector<float> x_sorted;
  vector<float> y_sorted;
...
double interp1D_unordered(vector<double> x, vector<double> y,
                          double x_interp_loc) {
  // initiate the sorted vectors
...
vector<double> interp1D_unordered(vector<double> x, vector<double> y,
                                  vector<double> &x_interp_loc) {
  // initiate the sorted vectors
  vector<double> x_sorted;
  vector<double> y_sorted;
...
// SMM 3/12/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
vector<double> interp1D_spline_unordered(vector<double> x_data,
                                         vector<double> y_data,
                                         vector<double> &x_interp_locs) {
  // initiate the sorted vectors
  vector<double> x_sorted;
  ...
  vector<double> y_interp =
      interp1D_spline_ordered(x_sorted, y_sorted, x_interp_locs);
  return y_interp;
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
double interp2D_bilinear(vector<double> &x_locs, vector<double> &y_locs,
                         Array2D<double> data, double x_interp,
                         double y_interp) {
  ...
  if (data.dim1() != n_xlocs || data.dim2() != n_ylocs) {
    cout << "Trying to do bilinear interpolation but data is not the same size"
         << endl
    ...
    if (y_interp < y_locs[0]) {
      ...
      cout << "y[0]: " << y_locs[0]
           << " y_locs[n_ylocs-1]: " << y_locs[n_ylocs - 1] << endl;
      y_index = ndv_index;
    } else {
      int i = 0;
    ...
    // cout << "x_index is: " << x_index << " and y_index is: " << y_index <<
    // endl; cout << "x1: " << x1 << " x2: " << x2 << " y1: " << y1 << " y2: "
    // << y2 << endl; cout << "xib: " << xib << " xis: " << xis << " yib: " <<
    // yib << " yis: " << yis << endl;
    ...
    // cout << "Q11: " << Q11 << " Q21: " << Q21 << " Q12: " << Q12 << " Q22: "
    // << Q22 << endl;

    double R1 = ((x2 - x_interp) / (x2 - x1)) * Q11 +
                ((x_interp - x1) / (x2 - x1)) * Q21;
    double R2 = ((x2 - x_interp) / (x2 - x1)) * Q12 +
                ((x_interp - x1) / (x2 - x1)) * Q22;

    interpolated_point =
        ((y2 - y_interp) / (y2 - y1)) * R1 + ((y_interp - y1) / (y2 - y1)) * R2;
  }
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
float interp2D_bilinear(vector<float> &x_locs, vector<float> &y_locs,
                        Array2D<float> data, float x_interp, float y_interp) {
  ...
  if (data.dim1() != n_xlocs || data.dim2() != n_ylocs) {
    ...
    float R1 = ((x2 - x_interp) / (x2 - x1)) * Q11 +
               ((x_interp - x1) / (x2 - x1)) * Q21;
    float R2 = ((x2 - x_interp) / (x2 - x1)) * Q12 +
               ((x_interp - x1) / (x2 - x1)) * Q22;
...
// parameters used to fit splines to vectors of data. Should not be called
// directly.
//
// Ported from a python implementation from
// http://jayemmcee.wordpress.com/cubic-splines/
//
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// Spline plotting function which calls CalculateCubicSplines() to generate a
// pair of X and Y vectors containing coordinates to plot a spline curve from a
// given set of data.
//
// Call this with vectors of X and Y data, a resolution (used to get the number
// of points between each data point for the output curve) and Spline_X and
// Spline_Y, which are passed in by reference and are overwritten with the
// resulting sets of coordinates.
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void PlotCubicSplines(vector<float> X, vector<float> Y, int SplineResolution,
                      vector<float> &Spline_X, vector<float> &Spline_Y) {
  ...
  int n =
      Splines
          .dim1(); // This is one fewer than the number of points in the dataset

  // check there will be enough partitions between each pair of points for the
  // plotting
  ...
  for (int i = 0; i < n - 1; ++i) {
    ...
    x1 = Splines[i + 1][4];
    vector<float> x =
        linspace(x0, x1, perSpline); // vector of evenly spaced values between
                                     // the max and min given

    // calculate x and y values for points along the spline curve between each
    // set of points
    for (int q = 0; q != int(x.size()); ++q) {
      Spline_X.push_back(x[q]);
      h = (x[q] - x0);
      Spline_Y.push_back(Splines[i][0] + Splines[i][1] * h +
                         Splines[i][2] * (h * h) +
                         (Splines[i][3] * (h * h * h)));
    }
  }
  ...
  x1 = X[n];
  vector<float> x2 =
      linspace(x0, x1, perSpline); // vector of evenly spaced values between the
                                   // max and min given

  for (int q = 0; q != int(x2.size()); ++q) {
    Spline_X.push_back(x2[q]);
    h = (x2[q] - x0);
    Spline_Y.push_back(Splines[n - 1][0] + Splines[n - 1][1] * h +
                       Splines[n - 1][2] * (h * h) +
                       (Splines[n - 1][3] * (h * h * h)));
  }
}
...
// Usage vector<float> new_vec = slice_vector(original_vector.begin() + 1,
// original_vector.begin() + 5); DTM 30/10/2014
vector<float> slice_vector(vector<float>::iterator first,
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void partition_driver_to_screen(
    int k, int minimum_length) // this just prints partitions to screen
{
  int n = 2 * k;
  int t = 0;
  vector<int> p(k, 0);
...
vector<vector<vector<int>>> partition_driver_to_vecvecvec(int k,
                                                          int minimum_length) {
  int n = 2 * k;
  int t = 0;
  vector<int> p(k, 0);
...
// and
// Skiena, S. Implementing Discrete Mathematics: Combinatorics and Graph Theory
// with Mathematica. Reading, MA: Addison-Wesley, 1990. this is a further
// adaptation that only presents solution to the partition with segments of a
// minimum length
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void partitions_with_minimum_length(int n, int k, int t, int min_length,
                                    vector<int> &p) {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// a function for use with the permutations
// this assigns values into the vecvecvec that contains all the partitioning
// information
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
void partition_vecvecvec_print(vector<vector<vector<int>>> &partitions) {
  ...
  for (int i = 0; i < int(partitions.size()); i++) {
    ...
    // cout << "n_segments: " << i+1 << " and number of partitions of this n
    // segments: " << n_partitions_this_nsegments << endl;
...
void partition_vecvecvec_print_with_permutation(
    vector<vector<vector<int>>> &partitions) {
  cout << "n_possible_segments: " << partitions.size() << endl;
  vector<vector<int>> partition_vecvec;
  for (int i = 0; i < int(partitions.size()); i++) {
    ...
    int n_partitions_this_nsegments = partition_vecvec.size();
    cout << "n_segments: " << i + 1
         << " and number of partitions of this n segments: "
         << n_partitions_this_nsegments << endl;
    for (int j = 0; j < n_partitions_this_nsegments; j++) {
      vector<int> individual_partition = partition_vecvec[j];
...
// and sigma, the standard deviation of the measured data. This will be
// approxamately the error in the surface elevation, although it might have to
// be increased simply because likelihood will tend to zero if this is too
// small. sigma should also be considered to contain the 'noise' inherent in
// channel incision so perhaps 1-5 metres is appropriate the maximum segment
// length is an integer: it is the number of data points used. these data points
// from raw chi data are irregularly spaced so two segments of the same 'length'
// can have different lengths in chi space. One remedey for this is a
// preprocessor that places the zeta vs chi data along evenly spaced points.
//
// The routine generates three matrices. The row of the matrix is the starting
// node of the segment. The column of the matrix is the ending node of the
// segment. Thus the routine will generate a matrix that is dimension n x n
// where n is the number of data points.
//
// One potential future development is to implement this using a sparse matrix
// from the boost mtl library to reduce the memory usage.
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void calculate_segment_matrices(
    vector<float> &all_x_data, vector<float> &all_y_data,
    int minimum_segment_length, float sigma, Array2D<float> &like_array,
    Array2D<float> &m_array, Array2D<float> &b_array,
    Array2D<float> &rsquared_array, Array2D<float> &DW_array) {
  int n_data_points = all_x_data.size();
  if (minimum_segment_length > n_data_points) {
    cout << "LSDStatsTools find_linear_segments: your segment length is "
            "greater than half the number of data points"
         << endl;
    cout << "This means that there can only be overlapping segments. Changing "
            "segment length to maximum segment length "
         << endl;
    minimum_segment_length = n_data_points;
  }

  // set up the arrays
  // in the future I might consider using sparse arrays but for now we'll just
  // populate the empty spots with placeholders
  ...
  // populate the matrix.
  // the get segment row function is recursive so it moves down through all the
  // possible starting nodes
  // cout << "LINE 518, sigma is: " << sigma << endl;
  populate_segment_matrix(start_node, end_node, no_data_value, all_x_data,
                          all_y_data, minimum_segment_length, sigma, like_array,
                          m_array, b_array, rsquared_array, DW_array);
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
void populate_segment_matrix(int start_node, int end_node, float no_data_value,
  ...
  if (like_array[start_node][end_node] == no_data_value) {
    ...
    regression_results =
        simple_linear_regression(segment_x, segment_y, residuals);
    this_MLE = calculate_MLE_from_residuals(residuals, sigma);

    // cout << "LINE 584 doing start: " << start_node << " end: " << end_node <<
    // endl;
    ...
    // now loop through all the end nodes that are allowed that are not the
    // final node. that is the first end node is first plus the maximum length
    // -1 , and then the final end node before the end of the data is the last
    // node minus the maximum length of the segment
    for (int loop_end = start_node + minimum_segment_length - 1;
         loop_end < end_node - minimum_segment_length + 1; loop_end++) {
      if (like_array[start_node][loop_end] == no_data_value) {
        ...
        // cout << "LINE 612 doing start: " << start_node << " end: " <<
        // loop_end << endl;
        ...
        populate_segment_matrix(loop_end + 1, end_node, no_data_value,
                                ...
                                sigma, like_array, m_array, b_array,
                                rsquared_array, DW_array);
      }
    }
  }
}
...
// First, it returns
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void best_fit_driver_AIC_for_linear_segments(int minimum_segment_length,
                                             float sigma,
                                             vector<float> all_x_data,
                                             vector<float> all_y_data,
  ...
  Array2D<float> DW_array;       // array holding the durbin-watson statistic of
                                 // indiviudal segments
  ...
  calculate_segment_matrices(all_x_data, all_y_data, minimum_segment_length,
                             norm_sigma, like_array, m_array, b_array,
                             rsquared_array, DW_array);
  ...
  find_max_like_of_segments(minimum_segment_length, like_array, one_sig_max_MLE,
                            segments_for_each_n_segments);
  ...
  get_n_segments_for_various_sigma(
      sigma_values, one_sig_max_MLE, all_x_data, best_fit_AIC, best_fit_AICc,
      AIC_for_each_n_segments, AICc_for_each_n_segments);

  print_AIC_and_AICc_to_screen(
      sigma_values, segments_for_each_n_segments, best_fit_AIC, best_fit_AICc,
      AIC_for_each_n_segments, AICc_for_each_n_segments);
  ...
  get_properties_of_best_fit_segments(
      bestfit_segments_node, segments_for_each_n_segments, m_values, m_array,
      b_values, b_array, r2_values, rsquared_array, DW_values, DW_array);

  // now print this data
  cout << "sigma is: " << sigma_values[n_sigma_for_printing] << " one_sig MLE: "
       << one_sig_max_MLE[best_fit_AICc[n_sigma_for_printing]]
       << " and the number of segments is: "
       << best_fit_AICc[n_sigma_for_printing] + 1 << endl;
  for (int i = 0; i < best_fit_AICc[n_sigma_for_printing] + 1; i++) {
    cout << m_values[i] << " " << b_values[i] << " " << r2_values[i] << " "
         << DW_values[i] << endl;
  }
}

// this function takes the normalized MLE values (normalized with sigma = 1) and
// returns the best fit number of segments from both the AIC and the AICc
// measures. It also returns two vector of vectors which are the AIC values for
// the varius values of sigma passed to the function in the sigma values vector
void get_n_segments_for_various_sigma(
    vector<float> sigma_values, vector<float> one_sig_max_MLE,
    vector<float> &all_x_data, vector<int> &best_fit_AIC,
    vector<int> &best_fit_AICc, vector<vector<float>> &AIC_for_each_n_segments,
    vector<vector<float>> &AICc_for_each_n_segments) {
  ...
  for (int i = 0; i < n_sigma; i++) {
    ...
    calculate_AIC_of_segments_with_normalized_sigma(
        sigma_values[i], one_sig_max_MLE, all_x_data, AIC_of_segments,
        AICc_of_segments);
...
void print_AIC_and_AICc_to_screen(
    vector<float> sigma_values,
    vector<vector<int>> segments_for_each_n_segments, vector<int> best_fit_AIC,
    vector<int> best_fit_AICc, vector<vector<float>> AIC_for_each_n_segments,
    vector<vector<float>> AICc_for_each_n_segments) {
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// this takes a likelihood array that has been calcualted with a given sigma
// value and normalizes the sigma values as though sigma was equal to 1.
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
vector<float> normalize_like_vector_to_sigma_one(float sigma,
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// this takes a normalize likelihood array and updates the values to a new sigma
// value
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
Array2D<float>
change_normalized_like_matrix_to_new_sigma(float sigma,
...
vector<float>
change_normalized_like_vector_to_new_sigma(float sigma,
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// this function calcualtes the most likeley combination of segments given the
// liklihood of individual segments calcualted by the calculate_segment_matrices
// function
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void find_max_like_of_segments(
    int minimum_segment_length, Array2D<float> &like_array,
  ...
  if (minimum_segment_length > n_data_points) {
    cout << "LSDStatsTools find_max_AIC_of_segments: your segment length is "
            "greater than the number of data points"
         << endl;
    cout << "This means that there can only be overlapping segments. Changing "
            "segment length to minimum segment length "
         << endl;
    minimum_segment_length = n_data_points;
  }
  ...
  vector<vector<vector<int>>> partitions =
      partition_driver_to_vecvecvec(n_data_points, minimum_segment_length);
  // cout << "find_max_like_of_segments, got partitions" << endl;

  // now loop through the number of segments, calucalting the maximum likelihood
  // each time
  vector<vector<int>> partition_vecvec;
  float this_MLE;
  ...
  for (int n_elem = 0; n_elem < int(partitions.size()); n_elem++) {
    ...
    int n_partitions_this_nsegments = partition_vecvec.size();
    // cout << "n_segments: " << n_elem+1 << " and number of partitions of this
    // n segments: " << n_partitions_this_nsegments << endl;
    for (int n_partition = 0; n_partition < n_partitions_this_nsegments;
         n_partition++) {
      // cout << "number of partitions: " << n_partition+1 << " of " <<
      // n_partitions_this_nsegments << endl;
      ...
      do {
        ...
        for (int i = 0; i < n_elements; i++) {
          ...
          // cout << "start node: " << start_node << " " << " end node: " <<
          // end_node
          //      << " and like: " << like_array[start_node][end_node] << endl;
          this_MLE = this_MLE * like_array[start_node][end_node];
          start_node = end_node + 1;
        }
        // cout << "This MLE: " << this_MLE << " seg MLE: " <<
        // MLE_for_segments[n_elem] << endl;

        // now test if this MLE is better than the the best MLE so far for this
      ...
      } while (prev_permutation(individual_partition.begin(),
                                individual_partition.end()));
    }
  }
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void get_properties_of_best_fit_segments(
    int bestfit_segments_node,
    vector<vector<int>> &segments_for_each_n_segments, vector<float> &m_values,
    Array2D<float> &m_array, vector<float> &b_values, Array2D<float> &b_array,
    vector<float> &r2_values, Array2D<float> &rsquared_array,
    vector<float> &DW_values, Array2D<float> &DW_array) {

  int n_segments =
      bestfit_segments_node + 1; // this converts from 0-based indexing to
  ...
  vector<int> individual_partition =
      segments_for_each_n_segments[bestfit_segments_node];
  ...
  for (int i = 0; i < n_segments; i++) {
    ...
    cout << "start node: " << start_node << " " << " end node: " << end_node
         << " m: " << m_array[start_node][end_node]
         << " b: " << b_array[start_node][end_node]
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// this function calculates AIC and AICc of segments taking the maximum_MLE
// based on a sigma of one
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void calculate_AIC_of_segments_with_normalized_sigma(
    float sigma, vector<float> &one_sigma_max_MLE, vector<float> &all_x_data,
    vector<float> &AIC_of_segments, vector<float> &AICc_of_segments) {

  // recast the MLE_vector
  vector<float> new_sig_MLE =
      change_normalized_like_vector_to_new_sigma(sigma, one_sigma_max_MLE);
  ...
  for (int n_elem = 0; n_elem < int(one_sigma_max_MLE.size()); n_elem++) {
    ...
    // if the MLE is 0 or less, this will throw an error. This only happens if
    // the fit is terrible so in this case set AIC and AICc to a large positive
    // number
    ...
    // cout << "AIC: " << AIC[n_elem] << " and AICc: " << AICc[n_elem] << endl
    // << endl;
  }
  AIC_of_segments = AIC;
...
void print_to_screen_most_likeley_segment_lengths(
  ...
  for (int n_elem = 0; n_elem < int(segments_for_each_n_segments.size());
       n_elem++) {
    cout << "n elements: " << n_elem << " and MLE: " << MLE_for_segments[n_elem]
         << endl;
    ...
    if (int(individual_partition.size()) != n_elem + 1) {
      cout << "LINE 707 statstools something is wrong n partitions is incorrect"
           << endl;
    }
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// this function calcualtes the most likeley combination of segments given the
// liklihood of individual segfments calcualted by the
// calculate_segment_matrices function
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void find_max_AIC_of_segments(
    int minimum_segment_length, vector<float> &all_x_data,
    vector<float> &all_y_data, Array2D<float> &like_array,
  ...
  for (int n_elem = 0; n_elem < int(partitions.size()); n_elem++) {
    ...
    if (MLE_for_segments[n_elem] <= 0) {
      ...
      cout << "MLE: " << MLE_for_segments[n_elem]
           << " log: " << log(MLE_for_segments[n_elem])
           << " 2nd term: " << -2 * log(MLE_for_segments[n_elem]) << endl;
...
// this is a function that generates linear segments randomly. Each segment has
// a slope and intercept.
void generate_random_segments(float sigma, int minimum_n_nodes,
                              int mean_segment_length, int segment_range,
                              float dx, float offset_range, float m_range,
                              vector<float> &x_data, vector<float> &y_data,
                              vector<int> &segment_lengths,
                              vector<float> &slope, vector<float> &intercept) {

  if (segment_range > 2 * mean_segment_length) {
    cout << "generate_random_segments LSDStatstools.cpp, segment range will "
            "result in negative segment lengths"
         << endl;
  ...
  while (total_nodes < minimum_n_nodes) {
    ...
    cout << "this segment length is: " << this_segment_length
         << " and total nodes: " << total_nodes << endl;
  }
  ...
  for (int i = 0; i < n_segments; i++) {
    ...
    vector<float> residuals;
    vector<float> lr =
        simple_linear_regression(segment_x_data, segment_y_data, residuals);
    cout << "Imposed m: " << this_m << " and regressed m: " << lr[0]
         << " and b: " << lr[1] << endl;
  ...
  cout << endl << endl;
  cout << "n_nodes: " << total_nodes
       << " and in all_x_data: " << all_x_data.size() << endl;
...
// this function attepts to find linear segments in a series of x,y data using a
// brute force method it loop through segments of a given length. Right now the
// only option is to use a fixed number of nodes rather than a fixed distance in
// x space but this can be altered later. it then calculates the regression
// coefficients and the durban-watson statistic on the segments. the function
// creates a vector of the slope, intercept, and durbin watson statistic of the
// best fit these are then scanned once more adjacent segments that lie on the
// same 'true' linear segment will have m and b vlaues that are approximately
// equal. however between segments there will be a transition where the m or b
// values move gradually to the new value. These gradual movments should be the
// length of a segment. So to see if there are two segments you compare the m
// and b values for segments that are seperated by the number of nodes in the
// segment to see if they are different. Eventally this will involve a
// statistical test but for now you can just let it pass a threshold. the code
// then identifies breakpoints and redoes the calucaltion using the number of
// identified segments one strategy for finding the 'most linear' fit is to loop
// through decreasing thresholds. The greater the threshold, the more segments
// there should be. So one can preform an Aikake test to see where the improment
// in fit no longer outweighs the increased number of segments.
void find_linear_segments(vector<float> &all_x_data, vector<float> &all_y_data,
                          int segment_length) {
  ...
  float slope_threshold = 0.1; // the threshold difference in slopes, beyond
                               // which a breakpoint is identified
  float intercept_threshold = 0.1; // the threshold difference in intercept,
                                   // beyond which a breakpoint is identified
  float slope_difference;     // difference in slope across a span that is the
                              // length of the segment
  float intercept_difference; // difference in intercept across a span that is
                              // the length of the segment

  // set up the scanning. The number of regressions will be
  // n_data_points-segment_length+1
  ...
  // the first break point has to be beyond the end of the first segment, so
  // start looking there
  int start = 0;
  for (int segment = segment_length; segment < n_segments; segment++) {
    slope_difference = fabs(segment_slopes[segment] -
                            segment_slopes[segment - segment_length]);
    intercept_difference = fabs(segment_intercepts[segment] -
                                segment_intercepts[segment - segment_length]);

    if (slope_difference > slope_threshold ||
        intercept_difference > intercept_threshold) {
  ...
  for (int calib_segment = 0; calib_segment < n_calibrated_segments;
       calib_segment++) {
    ...
    cout << calib_segment << " " << regression_data[0] << " "
         << regression_data[1] << " ";
    cout << regression_data[2] << " " << regression_data[3] << endl;
  }
}
...
float calculate_MLE(vector<float> &measured, vector<float> &modelled,
                    vector<float> &sigma) {
  ...
  for (int i = 0; i < n_samples; i++) {
    // cout << "exp term: " <<
    // -0.5*(measured[i]-modelled[i])*(measured[i]-modelled[i])/
    //                sigma[i]*sigma[i] << endl;
    MLE_tot =
        MLE_tot * exp(-0.5 * (measured[i] - modelled[i]) *
                      (measured[i] - modelled[i]) / (sigma[i] * sigma[i]));
  }
  return MLE_tot;
}
...
float calculate_MLE(vector<float> &measured, vector<float> &modelled,
  ...
  float MLE_tot = 1;
  for (int i = 0; i < n_samples; i++) {
    MLE_tot = MLE_tot * exp(-0.5 * (measured[i] - modelled[i]) *
                            (measured[i] - modelled[i]) / (sigma * sigma));
  }
  return MLE_tot;
}
...
float calculate_MLE_from_residuals(vector<float> &residuals, float sigma) {
  ...
  for (int i = 0; i < n_samples; i++) {
    MLE_tot =
        MLE_tot * exp(-0.5 * (residuals[i] * residuals[i]) / (sigma * sigma));
  }
  return MLE_tot;
}
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Takes 2D arrays for two corresponding variables (e.g. drainage area and
// slope) and sorts into logarithmically spaced bins of a specified bin width.
// The inputs are:
...
//
// Original version of this module produced a bad_alloc memory error. Now it has
// been re-ported from the working LSDRaster original version and has now been
// tested and produces the same results as the original with no memory errors. -
// SWDG 30/8/13
//
// Updated to return the number of values per bin in a vector by reference -
// SWDG 11/11/13
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void log_bin_data(Array2D<float> &InputArrayX, Array2D<float> &InputArrayY,
                  float log_bin_width, vector<float> &MeanX_output,
                  vector<float> &MeanY_output, vector<float> &midpoints_output,
                  ...
                  vector<int> &num_observations, float NoDataValue) {
  ...
  for (int row = 0; row < NRows; row++) {
    for (int col = 0; col < NCols; col++) {
      ...
      if (InputArrayX[row][col] < min_X ||
          min_X == 0) // Cannot have take a logarithm of zero.
      {
  ...
  for (int bin_id = 0; bin_id < NBins; bin_id++) {
    if (number_observations[bin_id] != 0) {
      ...
      while (vec_iterator_X != binned_data_X[bin_id].end()) {
        ...
        StandardDeviationX[bin_id] +=
            (Xi - MeanX[bin_id]) * (Xi - MeanX[bin_id]);
        vec_iterator_X++;
      }
      ...
      while (vec_iterator_Y != binned_data_Y[bin_id].end()) {
        ...
        StandardDeviationY[bin_id] +=
            (Yi - MeanY[bin_id]) * (Yi - MeanY[bin_id]);
        vec_iterator_Y++;
      }
    }
  }
  ...
  for (int bin_id = 0; bin_id < NBins; bin_id++) {
    if (number_observations[bin_id] != 0) {
      StandardDeviationX[bin_id] =
          sqrt(StandardDeviationX[bin_id] / number_observations[bin_id]);
      StandardDeviationY[bin_id] =
          sqrt(StandardDeviationY[bin_id] / number_observations[bin_id]);
      StandardErrorX[bin_id] =
          StandardDeviationX[bin_id] / sqrt(number_observations[bin_id]);
      StandardErrorY[bin_id] =
          StandardDeviationY[bin_id] / sqrt(number_observations[bin_id]);
    }
  }
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void log_bin_data(vector<float> &InputVectorX, vector<float> &InputVectorY,
                  float log_bin_width, vector<float> &MeanX_output,
                  vector<float> &MeanY_output, vector<float> &midpoints_output,
                  vector<float> &StandardDeviationX_output,
  ...
  // cout << "LSDStatsTools line 1757, n_data_X: " << n_data << " and Y: " <<
  // InputVectorX.size() << endl;
  ...
  for (int i = 0; i < n_data; ++i) {
    ...
    if (Y != NoDataValue) {
      ...
      if (X > 0) {
        ...
        // cout << "LINE 1818, bin id: " << bin_id << " i: " << i << " XDsz: "
        // << X_data.size() << " YDsz: " << Y_data.size() << endl; cout << "LINE
        // 1819, bdxsz: " << binned_data_X.size() << " bdysz: " <<
        // binned_data_Y.size() << endl << endl;
        //  Store X and corresponding Y into this bin, for their respective
        //  vector<vector> object
        binned_data_X[bin_id].push_back(X);
        binned_data_Y[bin_id].push_back(Y);
...
//  The outputs are:
// FC 13/11/12; modified by DM 04/12/12
// WARNING - will not work on vectors with negative values (e.g. hilltop
// curvature). If using vector of negative values take the absolute values
// before passing to function.
//
// Modified by FC 30/09/13 to calculate the range of the 95th percentile for
// each bin - used for channel head prediction through chi segment fitting.
// Changed from log binning to regular binning. Modified by FC 13/01/13 to
// calculate the median of each bin and return the standard error.
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void bin_data(vector<float> &InputVectorX, vector<float> &InputVectorY,
              float bin_width, vector<float> &MeanX_output,
              vector<float> &MeanY_output, vector<float> &midpoints_output,
              ...
              vector<float> &StandardErrorX_output,
              vector<float> &StandardErrorY_output,
              vector<int> &number_observations_output, float &bin_lower_limit,
              float NoDataValue) {
  ...
  cout << "LSDStatsTools line 1757, n_data_X: " << InputVectorX.size()
       << " and Y: " << InputVectorY.size() << endl;
  ...
  cout << "Upper limit: " << upper_limit << " Lower limit: " << lower_limit
       << " NBins: " << NBins << endl;
  ...
  for (int i = 0; i < n_data; ++i) {
    ...
    if (Y != NoDataValue) {
      ...
      if (X != 0) {
        ...
        if (bin_id >= 0) {
          // cout << "LINE 1818, bin id: " << bin_id << " i: " << i << " XDsz: "
          // << X_data.size() << " YDsz: " << Y_data.size() << endl; cout <<
          // "LINE 1819, bdxsz: " << binned_data_X.size() << " bdysz: " <<
          // binned_data_Y.size() << endl << endl;
          //  Store X and corresponding Y into this bin, for their respective
          //  vector<vector> object
          binned_data_X[bin_id].push_back(X);
  ...
  for (int bin_id = 0; bin_id < NBins; bin_id++) {
    ...
    if (number_observations[bin_id] != 0) {
      MeanY[bin_id] = Y_data[bin_id] / number_observations[bin_id];
      MeanX[bin_id] = X_data[bin_id] / number_observations[bin_id];
      // cout << "No observations in bin: " << number_observations[bin_id] <<
      // endl;
    }
  }
...
// look for bins with very few (or no) data points output from the log binning
// function and removes them to avoid plotting several empty bins at 0,0 in some
// cases. pass in a threshold fraction *above* which all bins will be kept. Pass
// in 0 to remove only empty bins. SWDG 6/11/13
void RemoveSmallBins(vector<float> &MeanX_output, vector<float> &MeanY_output,
                     ...
                     vector<float> &StandardErrorX_output,
                     vector<float> &StandardErrorY_output,
                     vector<int> &number_observations, float bin_threshold) {
...
// DTM 20/11/2013
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void print_histogram(vector<float> input_values, float bin_width,
                     string filename) {
  // Finding max contributing area to use as upper limit for the bins
  ...
  for (int i = 0; i < NBins; ++i) {
    ofs << bin_midpoints[i] << " " << bin_lower_lim[i] << " "
        << bin_upper_lim[i] << " " << number_observations[i] << " "
...
// DTM 12/02/2015
void calculate_histogram(vector<float> input_values, float bin_width,
                         vector<float> &Midpoints, vector<float> &LLims,
                         vector<float> &ULims, vector<int> &Count,
...
void calculate_histogram_fixed_limits(vector<float> input_values,
                                      float bin_width, float lower_limit,
                                      float upper_limit,
...
// bin_data
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// This is a much simpler version of the binning software.  It takes two
// vectors, and sorts the values held within the first vector into bins
// according to their respective values in the second vector.  The output is a
// vector<vector> with the binned dataset. and a vector of bin midpoints.  These
// can then be analysed ahd plotted as desired. DTM 14/04/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void bin_data(vector<float> &vector1, vector<float> &vector2, float min,
              float max, float bin_width, vector<float> &mid_points,
              vector<vector<float>> &binned_data) {
...
// log_bin_data
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// This is a much simpler version of the log-binning software.  It takes two
// vectors, and sorts the values held within the first vector into bins
// according to their respective values in the second vector.  The output is a
// vector<vector> with the binned dataset. and a vector of bin midpoints.  These
// can then be analysed ahd plotted as desired. DTM 30/10/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void log_bin_data(vector<float> &vector1, vector<float> &vector2,
                  float log_bin_width, vector<float> &bin_mid_points,
                  vector<float> &bin_vector1_mean,
                  vector<float> &bin_vector2_mean,
                  vector<vector<float>> &binned_data, const float NoDataValue) {
...
void matlab_double_reorder(std::vector<double> &unordered,
...
void matlab_double_sort(vector<double> &unsorted, vector<double> &sorted,
                        vector<size_t> &index_map) {
...
// This implementation is O(n), but also uses O(n) extra memory
void matlab_float_reorder(std::vector<float> &unordered,
                          std::vector<size_t> const &index_map,
...
void matlab_float_sort(vector<float> &unsorted, vector<float> &sorted,
                       vector<size_t> &index_map) {
  // Original unsorted index map
  index_map.resize(unsorted.size());
  for (size_t i = 0; i < unsorted.size(); i++) {
  ...
  // Sort the index map, using unsorted for comparison
  sort(index_map.begin(), index_map.end(),
       index_cmp<std::vector<float> &>(unsorted));
  sorted.resize(unsorted.size());
  matlab_float_reorder(unsorted, index_map, sorted);
}

void matlab_int_sort(vector<int> &unsorted, vector<int> &sorted,
                     vector<size_t> &index_map) {
  // Original unsorted index map
  index_map.resize(unsorted.size());
  for (size_t i = 0; i < unsorted.size(); i++) {
  ...
  // Sort the index map, using unsorted for comparison
  sort(index_map.begin(), index_map.end(),
       index_cmp<std::vector<int> &>(unsorted));
  sorted.resize(unsorted.size());
  matlab_int_reorder(unsorted, index_map, sorted);
}

void matlab_float_sort_descending(vector<float> &unsorted,
                                  vector<float> &sorted,
                                  vector<size_t> &index_map) {
  // Original unsorted index map
  index_map.resize(unsorted.size());
  ...
  sort(index_map.rbegin(), index_map.rend(),
       index_cmp<std::vector<float> &>(unsorted));
  sorted.resize(unsorted.size());
  matlab_float_reorder(unsorted, index_map, sorted);
}
...
// This implementation is O(n), but also uses O(n) extra memory
void matlab_int_reorder(std::vector<int> &unordered,
                        std::vector<size_t> const &index_map,
...
// Overloaded function to take in an array of integers and return a vector of
// the unique values found in the array, excluding the passed in NoDataValue.
// SWDG 12/11/13
  ...
  for (int i = 0; i < Rows; ++i) {
    for (int j = 0; j < Cols; ++j) {
      ...
      if (Value != NoDataValue) {
        ...
        if (find(UniqueValues.begin(), UniqueValues.end(), Value) ==
            UniqueValues.end()) {
...
// Wrapper around the std library unique method which also resizes the output
// vector.
// SWDG - 22/7/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
// between a min and max range (inclusive). Equivalent to np.linspace() in
// python and linspace in Matlab.
//
// Found at:
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
// Convert a bearing with 0/360 degrees at north to radians with 0/2pi radians
// at east - used in Hilltop flow routing SWDG 6/9/13
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
float BearingToRad(float Bearing) { return rad((-1 * Bearing) + 90); }
...
float angle_between_two_vector_datasets(vector<float> &x1_data,
                                        vector<float> &y1_data,
                                        vector<float> &x2_data,
                                        vector<float> &y2_data) {

  // get the vector intercept and floats
  float v1_intercept, v1_gradient, v2_intercept, v2_gradient;
  vector<float> first_vector_mean =
      orthogonal_linear_regression(x1_data, y1_data, v1_intercept, v1_gradient);
  vector<float> second_vector_mean =
      orthogonal_linear_regression(x2_data, y2_data, v2_intercept, v2_gradient);
...
//
// 2Percentile 25Percentile median mean 75Percentile 98Percentile minimum
// maximum
//
// SWDG 12/11/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
void get_distribution_stats(vector<float> &y_data, float &mean, float &median,
                            float &UpperQuartile, float &LowerQuartile,
                            float &MaxValue) {
  // Mean
  int n_data_points = y_data.size();
  MaxValue = 0;
...
double get_QuadraticMean(vector<double> input_values, double bin_width) {
  ...
  for (int i = 0; i < NBins; i++) {
    QuadraticMean +=
        probability_density[i] * bin_midpoints[i] * bin_midpoints[i];
  }
...
// Method to get the index of the maximum value in a 2D array of floats - MDH
// 28/8/14
...
// Method to get the index of the maximum value in a 2D array of ints - MDH
// 28/8/14
...
// p is the p-value. In numerical recipes it is provided as a value subtracted
// from 1, this code has been modified to present value the without this
// subtraction so that it matches the result from the scipy implementation. SWDG
// 26/6/14
...
float MannWhitneyUTest(vector<float> &sampleA, vector<float> &sampleB) {
  ...
  all_samples_indices.insert(all_samples_indices.end(), keyB.begin(),
                             keyB.end());

  // for(int i = 0; i<int(all_samples.size()); i++)
  //{
  //   cout << "samp["<<i<< "]: " << all_samples[i] << " and indices: " <<
  //   all_samples_indices[i] << endl;
  // }
  ...
  // cout << "sum_ranks1: " << sum_sample1 << " sum_ranks2: " << sum_sample2 <<
  // endl;

  // now calculate the U values
  // see: http://en.wikipedia.org/wiki/Mann%E2%80%93Whitney_U_test
  float U_1 = float(n_sample_A * n_sample_B) +
              float(n_sample_A * (n_sample_A + 1)) * 0.5 - sum_sample1;
  float U_2 = float(n_sample_A * n_sample_B) +
              float(n_sample_B * (n_sample_B + 1)) * 0.5 - sum_sample2;
  ...
  // now we need to get the mean and standard deviation for the normal
  // approximation
  ...
  for (int g = 0; g < n_groups; g++) {
    group_sum =
        float(number_in_groups[g] * number_in_groups[g] * number_in_groups[g] -
              number_in_groups[g]) /
        12.0;
  }
  ...
  if (group_sum == 0) {
    std_dev_U = sqrt(
        (float(n_sample_A * n_sample_B) * float(total_samples + 1)) / 12.0);
  } else {
    float std_dev_U_first_term = float(n_sample_A * n_sample_B) /
                                 float(total_samples * (total_samples - 1));
    float std_dev_U_second_term =
        float(total_samples * total_samples * total_samples - total_samples) /
        12.0;
    std_dev_U =
        sqrt(std_dev_U_first_term * (std_dev_U_second_term - group_sum));
  }
...
// 20/11/2014
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void rank_vector_with_groups(vector<float> sorted_data, vector<float> &ranks,
                             vector<int> &number_in_groups) {
...
// SWDG 5/6/15
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
void Count_Instances(vector<int> Data, vector<int> Key_Values,
                     map<int, int> &DataMap) {

  // update empty map passed in to contain the contain the keys and 0 value to
  // begin with.
...
vector<string> ReadCSVHeader(string path, string fname) {
  ...
  if (ifs.fail()) {
    cout << "\nFATAL ERROR: Trying to load csv cosmo data file, but the file"
         << filename << "doesn't exist; LINE 245 LSDCosmoData" << endl;
    exit(EXIT_FAILURE);
  }
  ...
  while (ss.good()) {
    ...
    substr.erase(remove_if(substr.begin(), substr.end(), ::iscntrl),
                 substr.end());
...
void split_delimited_string(const string &s, char c, vector<string> &v) {
  ...
  while (j != string::npos) {
    // DV - Copies out the substring between the zeroth char and the occurrence
    // of delimeter
...
string FixPath(string PathtoFix) {
  ...
  if (lchar != slash) {
    // cout << "You forgot the frontslash at the end of the path. Appending." <<
    // endl;
...
string ReformatPath(string old_path) {
  ...
  // '/' path.replace( path.begin(), path.end(), "\\", "/"); // replace all '\\'
  // to '/'
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--==
// SOME USEFUL MATHEMATICAL FUNCTIONS THAT DON'T APPEAR TO BE IN THE STANDARD
// MATH.h LIBRARY
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--==
// INVERSE ERROR FUNCTIONS AND INVERSE COMPLEMENTARY ERROR FUNCTIONS
// DTM, Following Press et al.,2007; Numerical Recipes, the Art of Scientific
// Computing, CUP
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=--==
// Inverse Complementary error function.  Returns x such that erfc(x)=p within
// limits 0<p<2
float inverfc(float p) {
  float x, err, t, pp;
  if (p >= 2.0) {
    cout << "ERROR - arguement outside permitted range - returning arbitrary "
            "value = 100\n";
    return 100;
  }
  if (p <= 0.0) {
...
float inverf(float p) { return inverfc(1. - p); }
...
DSnode *DisjointSet::get_DSnode(int i) { return DSnodes[i]; }
...
int DisjointSet::ElementCount() { return elements; }

int DisjointSet::SetCount() { return sets; }
...
void DisjointSet::Reset() { elements = sets = 0; }
...
//
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// Implementing sinmap's SI code, ported from code provided
// here:http://hydrology.usu.edu/sinmap2 and available under a GNU GPL licence.
// Notation reflects the derivation of the stability index outlined in the
// appendix of the sinmap documentation: http://hydrology.usu.edu/sinmap2 SWDG -
// 13/6/16
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

float StabilityIndex(float s, float a, float c1, float c2, float t1, float t2,
                     float x1, float x2, float r1, float r2, float fs1,
                     float fs2) {
  ...
  /* Wetness is coded between 0 and 1.0 based on the lower T/q parameter
    (this is conservative).  If wetness based on lower T/q is > 1.0 and wetness
    based on upper T/q <1.0, then call it the "threshold saturation zone" and
    hard code it to 2.0.  Then if wetness based on upper T/q >1.0, then call it
    the "saturation zone" and hard code it to 3.0. Lower T/q correspounds to
    upper q/T = x = x2 ->  w2  */
  w2 = x2 * a / ss;
  w1 = x1 * a / ss;
  if (w2 > 1.0) {
  ...
  if (fs2 < 1) /* always unstable */
  {
    ...
    if (fs1 >= 1) /*  Always stable */
    {
      ...
      if (w1 == 1) /* region 1 */
      {
        si = 1 - f2s(c1 / ss, c2 / ss, cs * (1 - r1) / ss * t1,
                     cs * (1 - r2) / ss * t2, 1);
      } else {
        if (w2 == 1) /* region 2 */
        {
          ...
          cdf1 = (x2 * as - 1) / (x2 * as - x1 * as) *
                 f2s(c1 / ss, c2 / ss, cs * (1 - r1) / ss * t1,
                     cs * (1 - r2) / ss * t2, 1);
          cdf2 = (1 - x1 * as) / (x2 * as - x1 * as) *
                 f3s(c1 / ss, c2 / ss, y1, y2, t1, t2, 1);
          si = 1 - cdf1 - cdf2;
        } else /* region 3 */
        {
          as = a / ss;
...
float f3s(float x1, float x2, float y1, float y2, float b1, float b2, float z) {
  ...
  if (x2 < x1 || y2 < y1 || b2 < b1) {
    ...
    if (y1 == y2 ||
        b1 == b2) { /* degenerate on y or b - reverts to additive convolution */
      p = f2s(x1, x2, y1 * b1, y2 * b2, z);
    } else {
      if (x2 == x1) {
        p = fa(y1, y2, b1, b2, z - x1);
      } else {
        p = (fai(y1, y2, b1, b2, z - x1) - fai(y1, y2, b1, b2, z - x2)) /
            (x2 - x1);
      }
    }
  }
...
float fa(float y1, float y2, float b1, float b2, float a) {
  ...
  if (y1 * b2 >
      y2 * b1) { /* Invoke symmetry and switch so that y1*b2 < y2*b1 */
    t = y1;
    y1 = b1;
    b1 = t;
  ...
  adiv = (y2 - y1) * (b2 - b1);
  p = (y1 * b1 < a && a <= y1 * b2)
          ? (a * log(a / (y1 * b1)) - a + y1 * b1) / adiv
          : p;
  p = (y1 * b2 < a && a <= y2 * b1) ? (a * log(b2 / b1) - (b2 - b1) * y1) / adiv
                                    : p;
  p = (y2 * b1 < a && a < y2 * b2)
          ? (a * log((b2 * y2) / a) + a + b1 * y1 - b1 * y2 - b2 * y1) / adiv
          : p;
  p = a >= (b2 * y2) ? 1 : p;
...
float fai4(float y1, float y2, float b1, float b2, float a) {
  ...
  float adiv;
  adiv = (y2 - y1) * (b2 - b1);
  return (a * a * log(b2 * y2 / a) / 2 + 3 * a * a / 4 + b1 * y1 * a -
          b1 * y2 * a - b2 * y1 * a) /
         adiv;
}

float fai5(float y1, float y2, float b1, float b2, float a) { return a; }
...
float distbetween(int row1, int col1, int row2, int col2) {
  return sqrt(((row2 - row1) * (row2 - row1)) +
              ((col2 - col1) * (col2 - col1)));
}

// Normalize the values of an array terrain shape index values to between 0 and
// MaxValue. pass in percentiles eg 98 for the 98th percentile to truncate the
// data about the median. For no truncation pass in 0 and 100. SWDG 25/1/17
Array2D<float> normalize_terrain_index(Array2D<float> Data,
                                       float lower_percentile,
                                       float upper_percentile, float MaxValue,
                                       float NoDataValue) {
...
// a polygon.
// returns an integer counting the number of times a ray traced from the point
// (XCoord,YCoord) crosses the border of the polygon. An even return value (0 is
// even) means the point is outside the polygon, and an odd value means the
// point is inside the polygon.
//
// Adapted from: http://stackoverflow.com/a/2922778/1627162
// SWDG - 25/1/17
int PointInPolygon(int VertexCount, float XCoords[], float YCoords[],
  ...
  for (int i = 0; i < VertexCount; ++i) {
    if (((YCoords[i] > YCoord) != (YCoords[j] > YCoord)) &&
        (XCoord < (XCoords[j] - XCoords[i]) * (YCoord - YCoords[i]) /
                          (YCoords[j] - YCoords[i]) +
                      XCoords[i])) {
      c++;
    }
