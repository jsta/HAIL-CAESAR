...
namespace LSDUtils {
void parse_line(std::ifstream &infile, std::string &parameter,
                std::string &value) {
  char c;
  char buff[256];
  int pos = 0;
  ...
  while (infile.get(c)) {
    if (pos >= 256) {
      std::cout << "Buffer overrun, word too long in parameter line: "
                << std::endl;
      std::string line;
      getline(infile, line);
      std::cout << "\t" << buff << " ! \n" << line << std::endl;
...
std::string RemoveControlCharactersFromEndOfString(std::string toRemove) {
  ...
  if (len != 0) {
    if (iscntrl(toRemove[len - 1])) {
      // cout << "Bloody hell, here is another control character! Why Microsoft?
      // Why?" << endl;
...
// test
  ...
  std::cout << "Hello! My name is OpenMP. I like to do LSD (Land Surface "
               "Dynamics) in parallel!"
            << std::endl;
  std::cout << "Your system has: " << num_procs << " PROCESSORS available and "
            << num_threads << " THREADS to use!" << std::endl;
  std::cout
...
#pragma omp critical
    {
      std::cout << "Yo, whaddup! From thread number..." << thread_id
                << std::endl;
    }
  }
