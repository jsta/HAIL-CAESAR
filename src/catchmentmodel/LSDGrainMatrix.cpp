...
{
  std::cout << "You are trying to create an LSDGrainMatrix object with no
supplied files or parameters." << std::endl << "Exiting..." << std::endl;
  exit(EXIT_FAILURE);
}*/
...
void LSDGrainMatrix::write_grainMatrix_to_ascii_file(
    std::string filename, std::string fname_extension) {
  ...
  if (fname_extension == "asc") {
    ...
    for (int i = 1; i <= NRows; ++i) {
      for (int j = 1; j <= NCols; ++j) {
        ...
        if (rasterIndex[i][j] != NoData) {
          // Write the first part of the output file line (x,y location and
          // index)
    ...
    std::cout << "You did not enter and approprate extension!" << std::endl
              << "You entered: " << fname_extension << ", options are: asc"
              << std::endl;
    exit(EXIT_FAILURE);
  }
}
