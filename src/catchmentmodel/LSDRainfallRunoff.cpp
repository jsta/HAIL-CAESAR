...
void rainGrid::create() {
  std::cout << "You have tried to create an empty RainfallRunoff object. "
            << std::endl
            << "Please try again with some parameters" << std::endl;
  exit(EXIT_FAILURE);
}
    ...
    for (int j = 1; j < jmax; j++) {
      for (int rf = 1; rf < rf_num; rf++) {
        if (hydroindex[i][j] == rf) {
          rainfallgrid2D[i][j] =
              rain_data[current_rainfall_timestep][rf - 1]; // added -1
        }
      }
    }
  }
...
void rainGrid::create(TNT::Array3D<double> &rain_data,
                      int current_raindata_timestep, int imax, int jmax) {
...
void rainGrid::write_rainGrid_to_raster_file(double xmin, double ymin,
                                             double cellsize,
...
void runoffGrid::create(int imax, int jmax) {
  ...
  // This is all that happens, use calculate_runoff() to fill in with proper
  // values.
}

// Same as abouve but calculates values for runoff matrices given raingrid and a
// timestep
...
void runoffGrid::write_runoffGrid_to_raster_file(
    double xmin, double ymin, double cellsize, std::string RUNOFFGRID_FNAME,
    ...
    for (int n = 1; n < jmax; n++) {
      ...
      if (elevations[m][n] > -9999) {
        ...
        if (current_rainGrid.get_rainfall(m, n) > 0) {
          // std::cout << "Rainfall is: " << current_rainGrid.get_rainfall(m, n)
          // << std::endl;
          //  Divide by 1000 to get m/hr, then 3600 for m/sec
          local_rainfall_rate =
              rain_factor *
              ((current_rainGrid.get_rainfall(m, n) / 1000) / 3600);
        }

        // If case is zero, we still need to calculate the amount of saturation
        // decay for this time step (TOPMODEL based)
        if (local_rainfall_rate == 0) {
          j_array[m][n] =
              jo_array[m][n] / (1 + ((jo_array[m][n] * local_time_step) / M));

          new_j_mean_array[m][n] =
              M / local_time_step *
        ...
        if (local_rainfall_rate > 0) {
          // std::cout << "Cell Rainfall Rate is: " << local_rainfall_rate <<
          // std::endl;

          j_array[m][n] =
              local_rainfall_rate /
              (((local_rainfall_rate - jo_array[m][n]) / jo_array[m][n]) *
                   std::exp((0 - local_rainfall_rate) * local_time_step / M) +
               1);

          new_j_mean_array[m][n] =
              (M / local_time_step) *
              std::log(
                  ((local_rainfall_rate - jo_array[m][n]) +
                   jo_array[m][n] *
                       std::exp((local_rainfall_rate * local_time_step) / M)) /
                  local_rainfall_rate);
        }
